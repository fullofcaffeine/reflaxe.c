...
class CBodyLowering {
	...
	/**
	 * Retain runtime-backed carriers required by emitted type declarations.
	 *
	 * Function-body operations already contribute their own requirements. A
	 * reachable record, enum payload, or class field can still require a runtime
	 * header even when the selected entry point never constructs that value.
	 * Reading the prepared source types here and reconciling them independently
	 * against validated HxcIR prevents C header emission from outrunning the
	 * runtime artifact plan.
	 */
	static function collectDeclarationRuntimeRequirements(output:Array<CBodyRuntimeRequirement>, aggregates:Array<CPreparedBodyAggregate>,
	...
	static function collectDeclarationTypeRuntimeRequirements(output:Array<CBodyRuntimeRequirement>, type:HxcIRTypeRef, source:HxcSourceSpan,
			position:Null<Position>, surface:String):Void {
		switch type {
			case IRTString:
				output.push(new CBodyRuntimeRequirement("string-literal", "type-carrier", surface, source, position, "runtime-representation"));
			case IRTManagedString:
				output.push(new CBodyRuntimeRequirement("string", "type-carrier", surface, source, position, "runtime-representation"));
			case IRTPointer(pointee, _) | IRTNullable(pointee, _) | IRTFixedArray(pointee, _, _) | IRTSpan(pointee, _):
				collectDeclarationTypeRuntimeRequirements(output, pointee, source, position, surface);
			case IRTFunction(parameters, result):
				for (parameter in parameters)
					collectDeclarationTypeRuntimeRequirements(output, parameter, source, position, surface);
				collectDeclarationTypeRuntimeRequirements(output, result, source, position, surface);
			case _:
		}
	}
	...
	**/
	...
	/**
		Decide once whether this program needs the owner-capable String plan.

		A literal-only program keeps the smaller allocation-free lifecycle: its
		String views point at compiler-owned bytes and need no cleanup. The first
		reachable scalar constructor or String concatenation changes the whole
		program's private String carrier policy because a runtime-created value can
		later cross an ordinary function, record, enum, optional, or Array boundary.
		Choosing the policy before type preparation keeps those independently built
	...
	**/
	...
	public static function isStringFromCharCode(expression:TypedExpr):Bool {
		return switch expression.expr {
			case TField(_, FStatic(reference, field)): final owner = reference.get(); owner.pack.length == 0 && owner.name == "String" && field.get()
					.name == "fromCharCode";
			case TParenthesis(inner) | TMeta(_, inner) | TCast(inner, _):
...
private class FunctionBuilder {
	...
	/**
		...
		HxcIR parameters are immutable incoming values. Ordinary reassignment and
		stack-closure capture instead share one automatic local initialized from
		that incoming value, so every later read observes the latest write without
		teaching C emission a special parameter-mutation rule.
	**/
	final parameterShadowPlaces:Map<Int, LoweredPlace> = [];
	...
	final freshManagedStringValueIds:Map<String, Bool> = [];
	final freshManagedStringValueRoles:Map<String, String> = [];
	...
	public function new(context:CompilationContext, prepared:PreparedBodyFunction, functionsById:Map<String, PreparedBodyFunction>,
		...
		prepareMutableParameterPlaces();
		prepareClosureCapturePlaces();
	}

	/**
		Materialize direct primitive parameters that the current function mutates.

		The discovery walk runs before body lowering because a read may precede the
		first assignment in source order. Each admitted parameter therefore starts
		as an initialized local at function entry. Nested function bodies retain
		the separate capture analysis: their shared outer variables are materialized
		when the closure takes their address.
	**/
	function prepareMutableParameterPlaces():Void {
		final parameterIds:Map<Int, Bool> = [];
		for (parameter in prepared.parameters)
			if (parameter.compilerId >= 0)
				parameterIds.set(parameter.compilerId, true);
		final mutations = directParameterMutations(prepared.bodyExpression, parameterIds);
		for (parameter in prepared.parameters) {
			if (parameter.compilerId < 0)
				continue;
			final mutation = mutations.get(parameter.compilerId);
			if (mutation == null)
				continue;
			switch parameter.mapping.kind {
				case CBVKPrimitive(primitive):
					switch primitive.irType {
			...
			}
			{
				...
				context.symbols.register(localRequest);
				localRequests.set(localId, localRequest);
				appendInstruction(null, IRIOInitialize(IRPLocal(localId), parameter.ir.id, IRISUninitialized, IRISInitialized), source,
					"mutable-parameter-initialize");
				parameterShadowPlaces.set(parameter.compilerId, {
					place: IRPLocal(localId),
					mapping: parameter.mapping,
					mutable: true
				});
				directMutableParameterIds.set(parameter.compilerId, true);
			}
		}
	}

	/**
		Find the first mutation of every parameter in this function's own body.

		Assignment, compound assignment, and prefix/postfix updates all require an
		addressable place. A nested `TFunction` has its own builder and capture
		plan, so this walk deliberately does not claim mutations inside it. One
		shared traversal keeps discovery proportional to body size rather than
		rescanning a large function once per parameter.
	**/
	static function directParameterMutations(body:TypedExpr, parameterIds:Map<Int, Bool>):Map<Int, TypedExpr> {
		final found:Map<Int, TypedExpr> = [];
		function visit(expression:TypedExpr):Void {
			switch expression.expr {
				case TFunction(_):
				case TBinop(OpAssign, target, _) | TBinop(OpAssignOp(_), target, _):
					recordDirectParameterMutation(found, parameterIds, target, expression);
					TypedExprTools.iter(expression, visit);
	...
	/** Retain the first source mutation for one admitted parameter ID. */
	static function recordDirectParameterMutation(found:Map<Int, TypedExpr>, parameterIds:Map<Int, Bool>, target:TypedExpr, mutation:TypedExpr):Void {
		final compilerId = directParameterPlaceCompilerId(target);
		if (compilerId != null && parameterIds.exists(compilerId) && !found.exists(compilerId))
			found.set(compilerId, mutation);
	}

	/** Recover the compiler ID from the exact local-place spelling admitted by `lowerPlace`. */
	static function directParameterPlaceCompilerId(expression:TypedExpr):Null<Int> {
		return switch expression.expr {
			case TLocal(variable): variable.id;
			case TParenthesis(inner) | TMeta(_, inner): directParameterPlaceCompilerId(inner);
			case _: null;
		};
	}
	...
	**/
	...
	public function build():BuiltBodyFunction {
		...
		if (freshManagedStringValueIds.keys().hasNext()) {
			final ids = [for (id in freshManagedStringValueIds.keys()) id];
			ids.sort((left, right) -> left < right ? -1 : left > right ? 1 : 0);
			final id = ids[0];
	...
	**/
	...
	function lowerStatement(expression:TypedExpr):Void {
		...
		switch expression.expr {
			...
			case TBinop(OpAssign, left, right) if (lowerManagedStringFieldInitializer(left, right)):
				// The field takes one runtime-created String owner. A borrowed
				// constructor argument is retained; a fresh expression transfers.
	...
	**/
	...
	function lowerManagedStringFieldInitializer(left:TypedExpr, right:TypedExpr):Bool {
		...
		if (initializedManagedStringFields.exists(fieldName))
			return false;
		final self = selfValue;
		if (self == null)
	...
	**/
	function lowerManagedDirectFieldInitializer(left:TypedExpr, right:TypedExpr):Bool {
		...
		if (initializedManagedDirectFields.exists(fieldName))
			return false;
		final self = selfValue;
		if (self == null)
	...
	/**
		Initialize one final primitive field in its declaring constructor.

		Haxe represents the legal constructor-only write as the same typed `=`
		node used for later assignment. A primitive `Bool`, integer, or float owns
		no nested storage, so construction needs only one typed store into the
		already zero-initialized object. This recognizer accepts the first direct
		`this.field = value` for a prepared final primitive field; subsequent writes
		fall through to the ordinary immutable-place diagnostic.
	**/
	function lowerFinalPrimitiveFieldInitializer(left:TypedExpr, right:TypedExpr):Bool {
		switch prepared.role {
			case PBRConstructor(_):
			case _:
				return false;
		...
		if (initializedFinalPrimitiveFields.exists(fieldName))
			return false;
		final self = selfValue;
		if (self == null)
		...
		if (field == null || field.mutable || field.type.primitiveMapping() == null || field.type.irType == IRTVoid)
			return false;
		final value = coerce(lowerValue(right, field.type), field.type, right.pos, 'TField($fieldName:final-primitive-initializer)');
		appendInstruction(null, IRIOStore(IRPField(IRPDereference(self.id), fieldName), value.id), sourceSpan(left.pos), "initialize-final-primitive-field");
		initializedFinalPrimitiveFields.set(fieldName, true);
		return true;
	}
	...
	**/
	...
	function lowerOwnedClassInitializer(left:TypedExpr, right:TypedExpr):Bool {
		...
		for (index in 0...argumentExpressions.length) {
			...
			argument = stabilizeFreshManagedAggregate(argument, argumentExpression.pos, 'owned-field-constructor-argument-$index');
	...
	**/
	...
	function lowerVariable(variable:TVar, initializer:Null<TypedExpr>, position:Position, compilerFlowCarrier:Bool = false):Void {
		...
		final value:Null<LoweredValue> = switch initializer {
			case null if (managedFlowCarrier || directFlowCarrier || tracedAggregateFlowCarrier):
				...
				// instead zero-initialized below so an intervening collection cannot
				// observe indeterminate pointer fields.
				null;
		...
		if (localMapping.irType == IRTManagedString && !managedFlowCarrier) {
			...
			if (!transferredFreshOwner) {
				...
				runtimeRequirements.push(new CBodyRuntimeRequirement("string", "retain", "ordinary Haxe String local alias", source, position));
			}
			final cleanupId = 'string-local.$ordinal.release';
			constructionCleanupActions.push({
				id: cleanupId,
				idempotence: IRCExactlyOnce,
				kind: IRCARelease(IRPLocal(localId), IRIRuntime("string")),
				source: source
			});
			normalCleanupActionIds.push(cleanupId);
	...
	function lowerConstructedVariable(variable:TVar, expression:TypedExpr, construction:BodyNewExpression, position:Position, ordinal:Int,
		...
		for (index in 0...argumentExpressions.length) {
			...
			argument = stabilizeFreshManagedAggregate(argument, argumentExpression.pos, 'constructor-argument-$index');
	...
	**/
	...
	function lowerSuperCall(expression:TypedExpr, callArguments:Array<TypedExpr>):Void {
		...
		for (index in 0...argumentExpressions.length) {
			...
			converted = stabilizeFreshManagedAggregate(converted, argument.pos, 'super-constructor-argument-$index');
	...
	**/
	...
	/**
	 * Prove that one flow-carrier arm either publishes a value or cannot rejoin.
	 *
	 * A `throw`/`return` edge does not need to initialize the carrier because it
	 * never reaches the later read. This is stricter than merely finding an
	 * assignment somewhere in the arm: nested branches must establish the same
	 * fact for every path that can continue.
	 */
	function definitelyAssignsOrTerminatesLocal(expression:TypedExpr, compilerId:Int):Bool {
		return switch expression.expr {
			...
			case TIf(_, whenTrue, whenFalse): whenFalse != null && definitelyAssignsOrTerminatesLocal(whenTrue,
					compilerId) && definitelyAssignsOrTerminatesLocal(whenFalse, compilerId);
			case TSwitch(subject, cases, defaultExpression): switchArmsAssignLocal(subject, cases, defaultExpression, compilerId);
			case TParenthesis(inner) | TMeta(_, inner): definitelyAssignsOrTerminatesLocal(inner, compilerId);
			case _: false;
		};
	}
	...
	function switchArmsAssignLocal(subject:TypedExpr, cases:Array<TypedSwitchArm>, defaultExpression:Null<TypedExpr>, compilerId:Int):Bool {
		if (defaultExpression == null) {
			if (!isExhaustiveEnumAbstractSwitch(subject, cases) && !isExhaustiveEnumValueSwitch(subject, cases)) {
				return false;
			}
		} else if (!definitelyAssignsOrTerminatesLocal(defaultExpression, compilerId)) {
			return false;
		}
		for (item in cases) {
			if (!definitelyAssignsOrTerminatesLocal(item.expr, compilerId)) {
				return false;
			}
		}
		return true;
	...
	/** Rebuild Haxe enum exhaustiveness from the prepared case table. */
	function isExhaustiveEnumValueSwitch(subject:TypedExpr, cases:Array<TypedSwitchArm>):Bool {
		final enumSubject = enumIndexSubject(subject);
		if (enumSubject == null)
			return false;
		final mapping = bodyValueType(enumSubject.t, enumSubject.pos, "TSwitch(flow-carrier-enum-subject)");
		final value = mapping.enumValue();
		return value != null && enumSwitchCoveredCaseCount(cases, value) == value.cases.length;
	}
	...
	function lowerReturn(value:Null<TypedExpr>, position:Position):Void {
		...
		if (prepared.returnMapping.irType == IRTManagedString) {
			var transferredCleanupId:Null<String> = null;
			var returnedValueId = lowered.id;
			if (!freshManagedStringValueIds.remove(lowered.id)) {
				final returnedLocal = directLocalCompilerId(value);
				if (returnedLocal != null)
					transferredCleanupId = stringCleanupActionIdsByCompilerId.get(returnedLocal);
				if (transferredCleanupId == null) {
					final ownerLocalId = createFlowLocal(prepared.returnMapping, lowered.id, source, "returned-string-owner");
					appendInstruction(null, IRIORetain(IRPLocal(ownerLocalId), IRIRuntime("string")), source, "retain-string-return");
					runtimeRequirements.push(new CBodyRuntimeRequirement("string", "retain", "ordinary Haxe String borrowed return", source, value.pos));
					returnedValueId = loadPlace({place: IRPLocal(ownerLocalId), mapping: prepared.returnMapping, mutable: false}, value.pos,
	...
	function lowerValue(expression:TypedExpr, ?expectedMapping:CBodyValueType):LoweredValue {
		...
		return switch expression.expr {
			...
			case TField(receiver, FInstance(_, _, fieldReference))
				if (fieldReference.get().name == "length"
	...
	function lowerAggregateLiteral(expression:TypedExpr, fields:Array<{name:String, expr:TypedExpr}>, expectedMapping:Null<CBodyValueType>):LoweredValue {
		...
		for (index => field in fields) {
			...
			/*
				...
				HxcIR roots the nested field. A pointer to automatic C storage cannot:
				the record could later be returned, stored, or passed onward after
	...
	**/
	function captureManagedValue(value:LoweredValue, mapping:CBodyValueType, position:Position, role:String):LoweredValue {
		...
		if (mapping.irType == IRTManagedString) {
			...
			appendInstruction(null, IRIORetain(IRPLocal(ownerLocalId), IRIRuntime("string")), source, "retain-record-string-field");
			runtimeRequirements.push(new CBodyRuntimeRequirement("string", "retain", "managed String captured by a closed record", source, position));
	...
	function lowerAggregateField(expression:TypedExpr, receiver:TypedExpr, fieldName:String):LoweredValue {
		...
		var receiverValue = coerce(lowerValue(receiver, receiverType), receiverType, receiver.pos, 'TField($fieldName:receiver)');
		// A returned managed record owns its nested values even when this
		// expression reads only one direct field. Give that fresh receiver stable
		// automatic storage before projecting from it; otherwise the projection
		// would leave the outer owner unconsumed and a managed child could outlive
		// the bytes it borrows.
		receiverValue = stabilizeFreshManagedAggregate(receiverValue, receiver.pos, 'record-field-$fieldName-receiver');
		final result:HxcIRResult = {id: nextValueId(), type: field.type.irType};
	...
	function lowerStatementConditional(expression:TypedExpr, condition:TypedExpr, whenTrue:TypedExpr, whenFalse:Null<TypedExpr>):Void {
		...
		// The condition's Bool is copied into an IR value before control splits.
		// Release any managed temporary used to compute it here; neither arm owns
	...
	function lowerStatementSwitch(expression:TypedExpr, subject:TypedExpr, cases:Array<TypedSwitchArm>, defaultExpression:Null<TypedExpr>):Void {
		final optionalSubject = bodyValueType(subject.t, subject.pos, "TSwitch(optional-subject-discovery)").optionalValue();
		if (optionalSubject != null && optionalSubject.payload.primitiveMapping() != null) {
			lowerStatementOptionalPrimitiveSwitch(expression, subject, optionalSubject, cases, defaultExpression);
			return;
		}
		final enumSubject = enumIndexSubject(subject);
		if (enumSubject != null) {
	...
	**/
	function lowerStatementOptionalPrimitiveSwitch(expression:TypedExpr, subject:TypedExpr, optional:CPreparedBodyOptional, cases:Array<TypedSwitchArm>,
		...
		final primitive = optional.payload.primitiveMapping();
		if (primitive == null)
		...
		final needsExit = defaultBlock == null || openEnds.length > 0;
		final exitBlock = needsExit ? createGeneratedBlock("optional-switch-exit", source) : null;
		if (exitBlock != null)
			for (end in openEnds)
				end.terminator = {kind: IRTJump(edge(exitBlock.id)), source: source};
	...
	function lowerValueSwitch(expression:TypedExpr, subject:TypedExpr, cases:Array<TypedSwitchArm>, defaultExpression:Null<TypedExpr>,
		...
		final initialResultId:Null<String> = if (managedStringResult) {
			...
			case _:
				...
				registerDefaultStringRequirement(resultMapping, source, expression.pos);
				initialResult.id;
		};
		...
		for (index in 0...cases.length) {
			...
			if (value != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, cases[index].expr.pos, "switch-case-acquire");
				else
		...
		if (defaultExpression == null) {
			...
			if (defaultValue != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, defaultExpression.pos, "switch-default-acquire");
				else
		...
		currentBlock = joinBlock;
		if (managedStringResult)
			return moveManagedStringCarrier(resultLocalId, resultMapping, source, "switch-managed-result");
	...
	**/
	function lowerValueStringSwitch(expression:TypedExpr, subject:TypedExpr, subjectMapping:CBodyValueType, cases:Array<TypedSwitchArm>,
		...
		final resultLocalId = if (managedStringResult) {
			final localId = declareFlowLocal(resultMapping, source, "string-switch-managed-result");
			appendInstruction(null, IRIODeclareManagedCarrier(IRPLocal(localId), IRIRuntime("string")), source, "string-switch-managed-result-declare");
			localId;
		} else {
			createFlowLocal(resultMapping, initialResultId, source, "string-switch-result");
		}
		final dispatchBlock = currentBlock;
		final caseBlocks:Array<MutableBodyBlock> = [];
		for (index in 0...cases.length)
		...
		for (index in 0...cases.length) {
			...
			if (value != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, cases[index].expr.pos, "string-switch-case-acquire");
				else
		...
		if (defaultExpression == null) {
			...
			if (defaultValue != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, defaultExpression.pos, "string-switch-default-acquire");
				else
		...
		currentBlock = joinBlock;
		if (managedStringResult)
			return moveManagedStringCarrier(resultLocalId, resultMapping, source, "string-switch-managed-result");
	...
	function lowerValueEnumSwitch(expression:TypedExpr, subject:TypedExpr, cases:Array<TypedSwitchArm>, defaultExpression:Null<TypedExpr>,
		...
		final resultLocalId = if (managedStringResult) {
			...
			registerDefaultStringRequirement(resultMapping, source, expression.pos);
			createFlowLocal(resultMapping, initialResult.id, source, "enum-switch-result");
		}
		final dispatchBlock = currentBlock;
		final caseBlocks:Array<MutableBodyBlock> = [];
		for (index in 0...cases.length)
		...
		for (index in 0...cases.length) {
			...
			if (value != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, cases[index].expr.pos, "enum-switch-case-acquire");
				else
		...
		if (defaultExpression != null && defaultBlock != null) {
			...
			if (defaultValue != null) {
				...
				if (managedStringResult)
					appendManagedCarrierAcquire(resultLocalId, coerced, resultMapping, null, source, defaultExpression.pos, "enum-switch-default-acquire");
				else
	...
	**/
	...
	function stringSwitchMapping(expression:TypedExpr):Null<CBodyValueType> {
		...
		return (mapping.irType == IRTString || mapping.irType == IRTManagedString)
			&& mapping.staticStringIdentity() != null ? mapping : null;
	}
	...
	**/
	...
	/** Find stable caller storage for one captured variable, reusing a mutable parameter place when present. */
	function closureCaptureSourcePlace(capture:PreparedStackClosureCapture, position:Position):LoweredPlace {
		...
		final existing = parameterShadowPlaces.get(capture.compilerId);
		if (existing != null)
			return existing;
	...
	**/
	...
	function lowerAssignment(expression:TypedExpr, left:TypedExpr, right:TypedExpr):LoweredValue {
		...
		if (target.mapping.irType == IRTManagedString) {
			...
			appendInstruction(null, IRIORelease(stableTarget.place, IRIRuntime("string")), sourceSpan, "release-string-assignment-target");
	...
	function lowerCompoundAssignment(expression:TypedExpr, operation:Binop, left:TypedExpr, right:TypedExpr):LoweredValue {
		...
		if (operation == OpAdd && target.mapping.irType == IRTManagedString) {
			// Preserve the original owner while evaluating the right side. That
			// expression may call user code, reassign the same place, or move
			// lowering into another basic block. The retained owner protects the
			// bytes; the staged value separately gives the eventual join block a
			// locally available HxcIR carrier for those bytes.
			final rightCreatesFlow = expressionCreatesFlow(right);
			final retainedOldValue = retainManagedStringTemporary(oldValue, left.pos, "compound-string-left");
			final stagedOldValue = stageFlowValue(retainedOldValue, left, rightCreatesFlow, "compound-string-left-flow");
			final stagedTarget = stageFlowPlace(target, left.pos, rightCreatesFlow, "compound-string-target");
			var rightValue = coerce(lowerValue(right, target.mapping), target.mapping, right.pos, "TBinop(OpAssignOp:String-right)");
			rightValue = stabilizeFreshManagedString(rightValue, right.pos, "compound-string-right");
			final stableOldValue = restoreStagedLoweredValue(stagedOldValue, "compound-string-left-flow-load");
			...
			runtimeRequirements.push(new CBodyRuntimeRequirement("string", "cleanup-release", "ordinary Haxe managed String cleanup", source, expression.pos));
			// The target now owns the fresh concatenation result.
			freshManagedStringValueIds.remove(nextValue.id);
			return nextValue;
		}
		final rightCreatesFlow = expressionCreatesFlow(right);
	...
	/**
		Lower `String.fromCharCode` to one checked, owned UTF-8 scalar.

		The runtime turns invalid Unicode scalar values into U+FFFD. HxcIR marks
		the result as a fresh owner so a local, return, or container can move it
		without an unnecessary retain.
	**/
	function lowerStringFromCharCode(expression:TypedExpr, arguments:Array<TypedExpr>):LoweredValue {
		...
		final argumentMapping = bodyValueType(arguments[0].t, arguments[0].pos, "TCall(String.fromCharCode:argument-type)");
		...
		appendInstruction(result, IRIOCall({
			...
			failure: managedArrayFailure()
		}), source, "string-from-scalar");
		registerValueTemporary(result.id, "string-from-scalar-result");
		freshManagedStringValueIds.set(result.id, true);
		freshManagedStringValueRoles.set(result.id, "String.fromCharCode");
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "from-scalar", "ordinary Haxe String.fromCharCode", source, expression.pos));
		return {id: result.id, type: result.type, mapping: resultMapping};
	}

	/** Concatenate two immutable String values into one fresh owned result. */
	...
	**/
	...
	function lowerManagedStringConcatValues(expression:TypedExpr, leftValue:LoweredValue, rightValue:LoweredValue, resultMapping:CBodyValueType,
		...
		appendInstruction(result, IRIOCall({
			...
			arguments: [leftValue.id, rightValue.id],
			returnType: IRTManagedString,
			failure: managedArrayFailure()
		}), source, role);
		registerValueTemporary(result.id, "string-concat-result");
		freshManagedStringValueIds.set(result.id, true);
		freshManagedStringValueRoles.set(result.id, role);
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "concat", "ordinary Haxe String concatenation", source, expression.pos));
		return {id: result.id, type: result.type, mapping: resultMapping};
	}
	...
	function lowerBinary(expression:TypedExpr, operation:Binop, left:TypedExpr, right:TypedExpr):LoweredValue {
		if (operation == OpAdd) {
			...
			if (resultMapping.irType == IRTManagedString)
				return lowerManagedStringConcat(expression, left, right, resultMapping);
		}
		if (operation == OpEq || operation == OpNotEq) {
			...
			final importedEnumComparison = lowerImportedEnumEquality(expression, operation, left, right);
			if (importedEnumComparison != null)
				return importedEnumComparison;
		}
		if (operation == OpEq || operation == OpNotEq) {
			...
			if (leftMapping != null && leftMapping.optionalValue() != null || rightMapping != null && rightMapping.optionalValue() != null) {
				return isNullConstantExpression(left)
					|| isNullConstantExpression(right) ? lowerOptionalNullEquality(expression, operation, left, right, leftMapping,
						rightMapping) : lowerOptionalScalarEquality(expression, operation, left, right, leftMapping, rightMapping);
			}
	...
	/**
		Compare two values of the same header-owned C enum.

		Imported enums are nominal direct values, not Haxe tagged unions and not
		primitive integers. Treating them as records rejects valid `==`/`!=`, while
		erasing them to `Int` would lose the reviewed ABI type. This path preserves
		the imported instance in HxcIR, evaluates operands from left to right, and
		emits one structural C enum comparison. Struct imports and differently
		named enum types remain rejected.
	**/
	function lowerImportedEnumEquality(expression:TypedExpr, operation:Binop, left:TypedExpr, right:TypedExpr):Null<LoweredValue> {
		final leftMapping = isNullConstantExpression(left) ? null : bodyValueType(left.t, left.pos, "TBinop(c-import-enum-equality:left-type)");
		final rightMapping = isNullConstantExpression(right) ? null : bodyValueType(right.t, right.pos, "TBinop(c-import-enum-equality:right-type)");
		final leftImport = leftMapping == null ? null : leftMapping.importedValue();
		final rightImport = rightMapping == null ? null : rightMapping.importedValue();
		final hasImportedEnum = leftImport != null && leftImport.kind == CITEnum || rightImport != null && rightImport.kind == CITEnum;
		if (!hasImportedEnum)
			return null;
		if (leftMapping == null
			|| rightMapping == null
			|| leftImport == null
			|| rightImport == null
			|| leftImport.kind != CITEnum
			|| rightImport.kind != CITEnum
			|| leftImport.instanceId != rightImport.instanceId)
			return unsupported(expression, "TBinop(c-import-enum-equality-requires-matching-enums)");

		final leftValue = coerce(lowerValue(left, leftMapping), leftMapping, left.pos, "c-import-enum-equality-left");
		final stagedLeft = stageFlowValue(leftValue, left, expressionCreatesFlow(right), "c-import-enum-equality-left");
		final rightValue = coerce(lowerValue(right, rightMapping), rightMapping, right.pos, "c-import-enum-equality-right");
		final stableLeftId = restoreStagedValue(stagedLeft, "c-import-enum-equality-left");
		final boolMapping = bodyValueType(expression.t, expression.pos, "TBinop(c-import-enum-equality:result-type)");
		if (boolMapping.irType != IRTBool)
			return unsupported(expression, "TBinop(c-import-enum-equality-result-not-Bool)");
		final result:HxcIRResult = {id: nextValueId(), type: IRTBool};
		appendInstruction(result,
			IRIOBinary(operation == OpEq ? "haxe.c-import-enum.equal" : "haxe.c-import-enum.not-equal", stableLeftId, rightValue.id, IRIStatic),
			sourceSpan(expression.pos), "c-import-enum-equality");
	...
	**/
	function lowerStaticStringEquality(expression:TypedExpr, operation:Binop, left:TypedExpr, right:TypedExpr, leftMapping:Null<CBodyValueType>,
		...
		if (leftIsNull != rightIsNull) {
			...
			var value = coerce(lowerValue(valueExpression, target), target, valueExpression.pos, "TBinop(String-null-equality:value)");
			value = stabilizeFreshManagedString(value, valueExpression.pos, "string-null-equality");
		...
		var leftValue = coerce(lowerValue(left, target), target, left.pos, "TBinop(String-equality:left)");
		leftValue = stabilizeFreshManagedString(leftValue, left.pos, "string-equality-left");
		final stagedLeft = stageFlowValue(leftValue, left, expressionCreatesFlow(right), "string-equality-left");
		var rightValue = coerce(lowerValue(right, target), target, right.pos, "TBinop(String-equality:right)");
		rightValue = stabilizeFreshManagedString(rightValue, right.pos, "string-equality-right");
	...
	function lowerOptionalScalarEquality(expression:TypedExpr, operation:Binop, left:TypedExpr, right:TypedExpr, leftMapping:Null<CBodyValueType>,
		...
		final optionalOnRight = rightMapping != null && rightMapping.optionalValue() != null;
		if (optionalOnLeft == optionalOnRight)
			return unsupported(expression, "TBinop(direct-optional-scalar-equality-requires-one-optional)");
		final optionalMapping = optionalOnLeft ? leftMapping : rightMapping;
		...
		final optionalValue = coerce(lowerValue(optionalExpression, optionalMapping), optionalMapping, optionalExpression.pos,
			"TBinop(direct-optional-scalar-equality:optional)");
		final scalarValue = coerce(lowerValue(scalarExpression, optional.payload), optional.payload, scalarExpression.pos,
			"TBinop(direct-optional-scalar-equality:scalar)");
		...
		// present-value edge. This also keeps the source's left-to-right order.
		final optionalLocal = createFlowLocal(optionalMapping, optionalValue.id, source, "optional-scalar-equality-optional");
		final scalarLocal = createFlowLocal(optional.payload, scalarValue.id, source, "optional-scalar-equality-scalar");
		final absent:HxcIRResult = {id: nextValueId(), type: IRTBool};
		appendInstruction(absent, IRIOConstant(IRCBool(operation == OpNotEq)), source, "optional-scalar-absent-result");
		final resultLocal = createFlowLocal(boolMapping, absent.id, source, "optional-scalar-equality-result");
		final present:HxcIRResult = {id: nextValueId(), type: IRTBool};
		final optionalForPresence = loadPlace({place: IRPLocal(optionalLocal), mapping: optionalMapping, mutable: false}, optionalExpression.pos,
			"optional-scalar-presence-load");
		appendInstruction(present, IRIOUnary("haxe.direct-optional.is-not-null", optionalForPresence.id, IRIStatic), source, "optional-scalar-present");
		final compareBlock = createGeneratedBlock("optional-scalar-compare", source);
		final joinBlock = createGeneratedBlock("optional-scalar-join", source);
		currentBlock.terminator = {kind: IRTBranch(present.id, edge(compareBlock.id), edge(joinBlock.id)), source: source};
		currentBlock = compareBlock;
		final optionalForCompare = loadPlace({place: IRPLocal(optionalLocal), mapping: optionalMapping, mutable: false}, optionalExpression.pos,
			"optional-scalar-compare-optional-load");
		final scalarForCompare = loadPlace({place: IRPLocal(scalarLocal), mapping: optional.payload, mutable: false}, scalarExpression.pos,
			"optional-scalar-compare-scalar-load");
		appendInstruction(null, IRIONullCheck(optionalForCompare.id, IRNCPCheckedAbort(Std.string(context.profile), Std.string(context.buildMode))), source,
			"optional-scalar-present-proof");
		final payload:HxcIRResult = {id: nextValueId(), type: optional.payload.irType};
		...
		currentBlock.terminator = {kind: IRTJump(edge(joinBlock.id)), source: source};
		currentBlock = joinBlock;
		return loadPlace({place: IRPLocal(resultLocal), mapping: boolMapping, mutable: true}, expression.pos, "optional-scalar-result-load");
	}
	...
	**/
	...
	function lowerConditional(expression:TypedExpr, condition:TypedExpr, whenTrue:TypedExpr, whenFalse:Null<TypedExpr>,
		...
		final resultLocalId = if (managedCarrierResult) {
			...
			appendInstruction(null, IRIODeclareManagedCarrier(IRPLocal(localId), destroyImplementation), source, "conditional-managed-result-declare");
			localId;
		} else if (branchInitializesResult && !tracedDirectResult) {
			...
			localId;
		} else if (tracedDirectResult) {
			createFlowLocal(resultMapping, null, source, "conditional-traced-result");
		} else {
		...
		if (managedCarrierResult) {
			appendManagedCarrierAcquire(resultLocalId, trueValue, resultMapping, managedEnumResult, source, whenTrue.pos, "conditional-true-acquire");
		} else {
		...
		if (managedCarrierResult) {
			appendManagedCarrierAcquire(resultLocalId, falseValue, resultMapping, managedEnumResult, source, falseExpression.pos, "conditional-false-acquire");
		} else {
			if (optionalResult != null && optionalResult.managedLifetime)
		...
		currentBlock = joinBlock;
		final loaded:LoweredValue = if (!managedCarrierResult) {
		...
		if (managedStringResult)
			freshManagedStringValueRoles.set(loaded.id, "conditional-result");
		if (optionalResult != null && optionalResult.managedLifetime)
	...
	/**
	 * Give one managed branch or switch arm to its outer join carrier.
	 *
	...
	function appendManagedCarrierAcquire(localId:String, value:LoweredValue, mapping:CBodyValueType, managedEnum:Null<CPreparedBodyEnumInstance>,
		...
		final fresh = if (mapping.irType == IRTManagedString) {
			...
			throw new CBodyEmissionError('managed carrier `$role` lost its supported lifetime type');
		};
		final acquisition = if (fresh) {
			IRMCAMoveFresh;
		} else {
			IRMCARetainBorrowed(managedCarrierImplementation(mapping, managedEnum, true));
		}
	...
	/** Select the exact retain or destroy implementation for one managed carrier. */
	 ...
	 * need active-branch transfer and join cleanup. An imported struct is admitted
	...
	function lowerCall(expression:TypedExpr, materializeResult:Bool):Null<LoweredValue> {
		...
		if (isAbstractMethod(call.callee, "c.StructInit", "zero")) {
			return lowerImportedStructZero(expression, call.arguments);
		}
		...
		if (target.returnMapping.irType == IRTManagedString)
			freshManagedStringValueIds.set(result.id, true);
		if (target.returnMapping.irType == IRTManagedString)
			freshManagedStringValueRoles.set(result.id, "direct-call-result");
	...
	function lowerIndirectFunctionCall(expression:TypedExpr, calleeExpression:TypedExpr, argumentExpressions:Array<TypedExpr>, callableMapping:CBodyValueType,
		...
		for (index in 0...argumentExpressions.length) {
			...
			argument = stabilizeFreshManagedString(argument, argumentExpression.pos, 'indirect-call-argument-$index');
		...
		if (signature.result.bytesValue() != null)
			freshManagedBytesValueIds.set(result.id, true);
		if (signature.result.irType == IRTManagedString)
			freshManagedStringValueIds.set(result.id, true);
		if (signature.result.irType == IRTManagedString)
			freshManagedStringValueRoles.set(result.id, "indirect-call-result");
	...
	function lowerImportedStructZero(expression:TypedExpr, arguments:Array<TypedExpr>):LoweredValue {
		...
		final imported = mapping.importedStructValue();
		if (imported == null)
			return unsupported(expression, "TCall(c.StructInit.zero:result-must-be-imported-struct)");
		final result:HxcIRResult = {id: nextValueId(), type: mapping.irType};
		appendInstruction(result, IRIOZeroAggregate(imported.instanceId), sourceSpan(expression.pos), "zero-imported-struct");
		registerValueTemporary(result.id, "zero-imported-struct-result");
		return {id: result.id, type: result.type, mapping: mapping};
	}
	...
	**/
	...
	/** Resolve the one release owner for a managed by-value class field. */
	static function managedDirectFieldRelease(type:CBodyValueType):Null<HxcIRImplementation> {
		if (type.bytesValue() != null)
			return IRIRuntime("bytes");
		final enumValue = type.enumValue();
		if (enumValue != null && enumValue.managedLifetime) {
			final id = enumValue.destroyImplementationId();
			return id == null ? null : IRIProgramLocal(id);
		}
		final aggregate = type.aggregateValue();
		if (aggregate != null && aggregate.managedLifetime) {
			final id = aggregate.destroyImplementationId();
			return id == null ? null : IRIProgramLocal(id);
		}
		final optional = type.optionalValue();
		if (optional != null && optional.managedLifetime) {
			final id = optional.destroyImplementationId();
			return id == null ? null : IRIProgramLocal(id);
		}
		return null;
	}
	...
	/**
		...
		An entry, `*-join`, or `*-exit` block continues in the function's ordinary
		lifetime. A join is emitted after its structured `if`/switch rather than
		inside one arm, so an owner declared there is visible to function-exit
		cleanup just like an owner declared in the entry block.
	...
	**/
	static function managedArrayElementOwnerHasCleanupBoundary(block:MutableBodyBlock):Bool {
		...
		return role == null
			|| StringTools.endsWith(role, "-join")
			|| StringTools.endsWith(role, "-exit")
			|| role == "while-body"
			|| role == "do-body"
			|| role == "if-true"
			|| role == "if-false"
			|| role == "conditional-true"
			|| role == "conditional-false"
			|| role == "short-circuit-rhs"
			|| StringTools.startsWith(role, "string-switch-")
	...
	**/
	function lowerStringCall(expression:TypedExpr, access:reflaxe.c.lowering.CBodyDispatch.CBodyInstanceCallAccess, arguments:Array<TypedExpr>):LoweredValue {
		...
		for (index => argumentExpression in arguments) {
			...
			if (typeKey(argumentMapping.irType) != typeKey(IRTInt(32, true)))
				return unsupported(argumentExpression, 'TCall(String.$method:argument-$index-requires-Haxe-Int)');
			loweredArguments.push(coerce(lowerValue(argumentExpression, argumentMapping), argumentMapping, argumentExpression.pos,
	...
	**/
	...
	/** True for both literal-only and runtime-owned immutable String carriers. */
	...
	**/
	function lowerStringLength(expression:TypedExpr, receiverExpression:TypedExpr):LoweredValue {
		...
		registerValueTemporary(result.id, "string-length-result");
		runtimeRequirements.push(new CBodyRuntimeRequirement("string-scalar", "length", "ordinary Haxe String.length with Unicode-scalar indexing", source,
			expression.pos));
		return {id: result.id, type: result.type, mapping: resultMapping};
	}
	...
	**/
	 ...
	 * `hxc_io_println` for the duration of that call. If the expression created a
	...
	function lowerSysPrintln(expression:TypedExpr, arguments:Array<TypedExpr>):Null<LoweredValue> {
		if (arguments.length == 1 && stringLiteral(arguments[0]) != null)
			return lowerLiteralOutput(expression, arguments, "sys-println-literal", "Sys.println(String literal)", false);
		if (prepared.role != PBRFunction)
		...
		final source = sourceSpan(expression.pos);
		appendInstruction(null, IRIOCall({
			dispatch: IRCDRuntime("io", "sys-println-string"),
			arguments: [value.id],
			returnType: IRTVoid,
			failure: {
				kind: IRFNativeStatus,
	...
	**/
	...
	/**
		Give a fresh runtime-created String a caller-owned lifetime around a call.

		Haxe passes String values as immutable borrows: the callee may read the
		bytes for the duration of the call, and it retains a separate owner only
		if it stores the value. A nested expression such as
		`consume(String.fromCharCode(code))` has no source local to own the fresh
		buffer, so this compiler local keeps it alive through the call and releases
		it afterward. Array element reads already have a scoped owner and therefore
		pass through without another retain.
	**/
	function stabilizeFreshManagedString(value:LoweredValue, position:Position, role:String):LoweredValue {
		...
		constructionCleanupActions.push({
			id: cleanupId,
			idempotence: IRCExactlyOnce,
			kind: IRCARelease(IRPLocal(ownerLocalId), IRIRuntime("string")),
			source: source
		});
		normalCleanupActionIds.push(cleanupId);
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "cleanup-release", "ordinary Haxe managed String cleanup", source, position));
	...
	/**
		Turn a borrowed String view into one transferable owner when it can escape.

		`charAt` and `substring` reuse bytes from their receiver. If those bytes
		belong to a runtime-created String, returning or storing the view without a
		retain would leave it pointing at freed memory after the receiver is
		released. This helper uses the typed managed-carrier protocol to retain the
		borrow and move that owner into a new semantic value. Keeping the handoff in
		HxcIR lets a later conditional prove that this value is fresh without
		trusting lowering-only bookkeeping or guessing from emitted C.
		Literal-backed views have no runtime owner, so they stay allocation-free.
	**/
	function ownBorrowedStringResult(value:LoweredValue, position:Position, role:String):LoweredValue {
		if (value.mapping.irType != IRTManagedString)
			return value;
		final source = sourceSpan(position);
		final ownerLocalId = declareFlowLocal(value.mapping, source, role + "-owner");
		appendInstruction(null, IRIODeclareManagedCarrier(IRPLocal(ownerLocalId), IRIRuntime("string")), source, role + "-owner-declare");
		appendInstruction(null, IRIOAcquireManagedCarrier(IRPLocal(ownerLocalId), value.id, IRMCARetainBorrowed(IRIRuntime("string"))), source,
			role + "-owner-acquire");
		final result:HxcIRResult = {id: nextValueId(), type: value.mapping.irType};
		appendInstruction(result, IRIOMoveManagedCarrier(IRPLocal(ownerLocalId)), source, role + "-owner-move");
		registerValueTemporary(result.id, role + "-owned-result");
		final owned:LoweredValue = {id: result.id, type: result.type, mapping: value.mapping};
		freshManagedStringValueIds.set(result.id, true);
		freshManagedStringValueRoles.set(result.id, role + "-borrowed-view");
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "retain", "borrowed ordinary Haxe String view escaping its receiver expression",
			source, position));
		return owned;
	}

	/**
		Retain a borrowed String when later evaluation may replace its current owner.

		This differs from `stabilizeFreshManagedString`: a borrowed value does not
		arrive with an owner the current expression may consume, so the temporary
		first acquires one retain. Compound assignment uses this to preserve the
		left value across arbitrary calls on the right.
	**/
	function retainManagedStringTemporary(value:LoweredValue, position:Position, role:String):LoweredValue {
		...
		normalCleanupActionIds.push(cleanupId);
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "retain", "borrowed ordinary Haxe String preserved across evaluation", source,
			position));
		runtimeRequirements.push(new CBodyRuntimeRequirement("string", "cleanup-release", "ordinary Haxe managed String cleanup", source, position));
		return loadPlace({place: IRPLocal(ownerLocalId), mapping: value.mapping, mutable: false}, position, role + "-borrow");
	}
	...
	**/
			...
			switch mapping.kind {
				case CBVKAggregate(_):
					appendInstruction(null, IRIODefaultInitialize(IRPLocal(localId), IRISUninitialized, IRISInitialized), source, role
						+ "-default-initialize");
				case CBVKEnum(value) if (value.collectorPayload && !value.managedLifetime):
					appendInstruction(null, IRIODefaultInitialize(IRPLocal(localId), IRISUninitialized, IRISInitialized), source, role
						+ "-default-initialize");
				case _:
	...
	function defaultConstant(type:HxcIRTypeRef, expression:TypedExpr, owner:String):HxcIRConstant {
		return switch type {
			...
			case IRTManagedString: IRCString("", 0);
	...
	/**
		Record the runtime carrier needed by a compiler-created empty String.

		Conditionals and switches sometimes need a safe value before their branches
		assign the real result. HxcIR represents that defensive value with the same
		empty UTF-8 String constant used for source literals. The runtime analyzer
		therefore sees a `string-literal/static-value` intent even though the user did
		not write a literal at that exact span. Registering the reason here keeps the
		typed source plan and reachable HxcIR in one-to-one agreement.
	**/
	function registerDefaultStringRequirement(mapping:CBodyValueType, source:HxcSourceSpan, position:Position):Void {
		switch mapping.irType {
			case IRTString | IRTManagedString:
				runtimeRequirements.push(new CBodyRuntimeRequirement("string-literal", "static-value", mapping.cSpelling, source, position,
					"direct-string-value"));
			case _:
		}
	}
		...
		if (sourceInterface != null || targetInterface != null) {
			if (sourceInterface != null && targetInterface != null) {
				...
				appendInstruction(result, IRIOUpcastInterface(value.id, sourceInterface.instanceId, targetInterface.instanceId, tables), sourceSpan(position),
					"interface-upcast");
				registerValueTemporary(result.id, "interface-upcast-value");
