...
/**
	...
	64-word schedule plus the final one or two padded blocks. The schedule and
	round constants use ordinary integer arrays because Eval can access an array
	word directly; storing each word in `Bytes` made every round call byte-buffer
	read/write helpers even though the data was already a host integer. Direct
	`Int` addition is correct here because Eval integers have the 32-bit
	wraparound needed by SHA-256. This is an execution optimization only:
	known-vector and standard-library differential tests own the cryptographic
	result.
**/
private class EvalSha256 {
	static final ROUND_CONSTANTS:Array<Int> = [
		0x428A2F98,
		0x71374491,
		0xB5C0FBCF,
		0xE9B5DBA5,
	...
	final schedule:Array<Int> = [for (_ in 0...64) 0];
	...
	function compress(input:Bytes, offset:Int):Void {
		...
		for (index in 0...64) {
			...
			final first = h + sigma1 + choose + ROUND_CONSTANTS[index] + schedule[index];
