...
class CASTPrinter {
  ...
  public function new(?indentUnit:String = "  ") {
    if (indentUnit == "") throw "C AST indent unit must not be empty";
  ...
  public function printTranslationUnit(unit:CTranslationUnit):String {
    ...
    for (include in includes) {
      if (lastKind != null && lastKind != include.kind) out.push("");
    ...
    if (includes.length > 0 && unit.declarations.length > 0) out.push("");

    for (index in 0...unit.declarations.length) {
      out.push(printDecl(unit.declarations[index]));
      if (index + 1 < unit.declarations.length) out.push("");
    }
    return out.join("\n") + "\n";
  }
  ...
  public function printDecl(decl:CDecl):String {
    return switch decl {
      ...
      case DPrototype(storage, returnType, declarator, attributes):
        joinTokens([printStorage(storage), printType(returnType), printDeclarator(declarator), printAttributes(attributes)]) + ";";
      case DFunction(function): printFunction(function);
    }
  }
  ...
  */
  public function printDeclarator(declarator:CDeclarator):String {
    return switch declarator {
      ...
      case DArray(inner, size, isStatic, qualifiers):
        ...
        for (qualifier in qualifiers) parts.push(cast(qualifier, String));
        if (size != null) parts.push(printExpr(size, 2));
      ...
      case DFunction(inner, params, variadic):
        ...
        if (variadic) values.push("...");
        if (values.length == 0) values.push("void");
  ...
  function printFunction(function:CFunctionDef):String {
    final header = joinTokens([
      printStorage(function.storage),
      printType(function.returnType),
      printDeclarator(function.declarator),
      printAttributes(function.attributes)
    ]);
    return header + "\n" + printControlledBody(function.body);
  }
  ...
  function printBlock(statements:Array<CStmt>):String {
    ...
    indent++;
    for (statement in statements) out.push(printStmt(statement));
    indent--;
    out.push(line("}"));
    return out.join("\n");
  }
  ...
  function printSwitch(value:CExpr, cases:Array<CCase>):String {
    ...
    for (caseData in cases) {
      if (caseData.isDefault) out.push(line("default:"));
      for (caseValue in caseData.values) out.push(line('case ${printExpr(caseValue, 3)}:'));
      indent++;
      for (statement in caseData.body) out.push(printStmt(statement));
      indent--;
    }
    indent--;
    out.push(line("}"));
  ...
  function binaryPrecedence(op:CBinaryOp):Int {
    return switch op {
      ...
      case LogicalAnd: 5;
      case LogicalOr: 4;
      case Assign | AddAssign | SubtractAssign | MultiplyAssign | DivideAssign | ModuloAssign
        | ShiftLeftAssign | ShiftRightAssign | BitAndAssign | BitXorAssign | BitOrAssign: 2;
      case Comma: 1;
    }
  }
  ...
  function binaryToken(op:CBinaryOp):String {
    return switch op {
      ...
      case Assign: "="; case AddAssign: "+="; case SubtractAssign: "-=";
      case MultiplyAssign: "*="; case DivideAssign: "/="; case ModuloAssign: "%=";
      case ShiftLeftAssign: "<<="; case ShiftRightAssign: ">>=";
      case BitAndAssign: "&="; case BitXorAssign: "^="; case BitOrAssign: "|=";
      case Comma: ",";
    }
  }
  ...
  function isRightAssociative(op:CBinaryOp):Bool {
    return switch op {
      case Assign | AddAssign | SubtractAssign | MultiplyAssign | DivideAssign | ModuloAssign
        | ShiftLeftAssign | ShiftRightAssign | BitAndAssign | BitXorAssign | BitOrAssign: true;
      case _: false;
    }
  }

  function printStorage(storage:Array<CStorage>):String return storage.map(value -> cast(value, String)).join(" ");
  function printAttributes(attributes:Array<String>):String return attributes.join(" ");
  function includeKindOrder(kind:CIncludeKind):Int return switch kind { case System: 0; case Local: 1; }
  function joinTokens(tokens:Array<String>):String return tokens.filter(token -> StringTools.trim(token) != "").join(" ");

  function line(value:String):String return indentPrefix() + value;

  function indentPrefix():String {
    final out = new StringBuf();
    for (_ in 0...indent) out.add(indentUnit);
    return out.toString();
  }
  ...
  function quote(value:String):String {
    ...
    for (byte in utf8Bytes(value)) {
      switch byte {
        case 0x22: escaped.add('\\"');
        case 0x3F: escaped.add('\\?'); // Prevent C11 trigraph formation.
        case 0x5C: escaped.add('\\\\');
        case 0x07: escaped.add('\\a');
        case 0x08: escaped.add('\\b');
        case 0x09: escaped.add('\\t');
        case 0x0A: escaped.add('\\n');
        case 0x0B: escaped.add('\\v');
        case 0x0C: escaped.add('\\f');
        case 0x0D: escaped.add('\\r');
        case _ if (byte >= 0x20 && byte < 0x7F): escaped.addChar(byte);
        case _: escaped.add('\\' + octalByte(byte));
      }
    }
    escaped.add('"');
  ...
  function printChar(value:Int):String {
    if (value < 0 || value > 0xFF) throw 'C byte character literal out of range: $value';
    return switch value {
      case 0x27: "'\\\''";
      case 0x3F: "'\\?'";
      case 0x5C: "'\\\\'";
  ...
  function utf8Bytes(value:String):Array<Int> {
    ...
    while (index < value.length) {
      ...
      if (first >= 0xD800 && first <= 0xDBFF) {
        if (index >= value.length) throw "Unpaired high surrogate in C string literal";
        final second = codeUnitAt(value, index++);
        if (second < 0xDC00 || second > 0xDFFF) throw "Unpaired high surrogate in C string literal";
  ...
  function codeUnitAt(value:String, index:Int):Int {
    ...
    if (code == null) throw 'String index out of range while printing C literal: $index';
    return code;
  }
