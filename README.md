...
| `constructor-lowering` | `implemented` | Concrete non-generic Haxe classes support bounded local and immediate-receiver stack construction, validated factories and other returned references through stable collector storage, final inline or traced child ownership, by-value closed-record, literal-backed nominal String, and unmanaged-enum arguments, plus call-only or retained interface arguments with exact order and cleanup. |
...
| `literal-string-output` | `implemented` | Hosted Sys.println accepts statically typed String values, while default trace accepts compiler-known literals; both preserve exact UTF-8/NUL bytes and explicit output failure handling. |
