...
| `hxc-ir` | `implemented` | Schema-20 HxcIR structurally records values, tagged cases, concrete and header-owned nominal types, reachable class/interface layouts, slots and tables, distinct caller-owned class and interface borrows, receiver-tied read-only span returns, exact native dispatch/constants, binary32 rounding/binary64 widening, strings, ordering, control flow, failures, cleanup, managed roots, unmanaged branch carriers, managed-enum ownership joins, and runtime intent. |
