...
def validate_generated(ledger: dict[str, object]) -> None:
    ...
    if statuses["conformant"] != 1 or statuses["partial"] != 24:
        raise StdlibLedgerFailure(
            "evidence should mark exactly Std.int conformant plus ten bounded Bytes APIs, eight bounded String APIs, four bounded StringBuf APIs, and literal Sys.println/trace partial; broader stdlib parity is not yet proven"
        )
