...
def validate_generated(ledger: dict[str, object]) -> None:
    ...
    if statuses["conformant"] != 1 or statuses["partial"] != 27:
        raise StdlibLedgerFailure(
            "evidence should mark exactly Std.int conformant plus the bounded Array.copy, Array<String>.join, Std.string(Bool/Int/String identity), ten Bytes, eight String, four StringBuf, and literal Sys.println/trace slices partial; broader stdlib parity is not yet proven"
        )
