...
class ControlFlowPlanCacheProbe {
	...
	**/
	public static macro function run():Expr {
		...
		CBodyControlFlowPlanCache.beginRequest(true);
		final changedFunction = CBodyControlFlowPlanCache.resolve("function.other", "hxcir schema=21\nfunction demo\n", () -> build("function"));
		require(!changedFunction.reused && builds == 3, "changed function identity reused stale evidence");
		CBodyControlFlowPlanCache.completeRequest();

		CBodyControlFlowPlanCache.beginRequest(true);
		final abortedText = CBodyControlFlowPlanCache.resolve("function.other", "hxcir schema=21\nfunction changed\n", () -> build("text"));
		require(!abortedText.reused && builds == 4, "changed function text reused stale evidence");
		CBodyControlFlowPlanCache.abortRequest();

		CBodyControlFlowPlanCache.beginRequest(true);
		final afterAbort = CBodyControlFlowPlanCache.resolve("function.other", "hxcir schema=21\nfunction demo\n", () -> build("unexpected"));
		...
		final disabled = CBodyControlFlowPlanCache.resolve("function.other", "hxcir schema=21\nfunction demo\n", () -> build("disabled"));
