...
RUNTIME_SOURCES = (
    ...
    ROOT / "runtime/hxrt/src/bytes_string.c",
)
...
def validate_bytes_flow_carrier(hxcir: str, function_id: str) -> None:
    """Prove one compiler-generated Bytes join has an exact owner lifecycle."""
    function = hxcir_function(hxcir, function_id)
    required_once = (
        'declare-managed-carrier place=local("local.1") destroy=runtime("bytes")',
        'ownership=retain-borrowed(runtime("bytes"))',
        "ownership=move-fresh",
        'move-managed-carrier place=local("local.1")',
        "managed-flow-owner-initialize",
        'implementation=runtime("bytes")',
    )
    for marker in required_once:
        if function.count(marker) != 1:
            raise BytesRuntimeFailure(
                f"{function_id} expected exactly one {marker!r}"
            )
    if "flow-carrier-default" in function:
        raise BytesRuntimeFailure(
            f"{function_id} fabricated a default Bytes owner"
        )
...
def validate_generated_project(output: Path, hxcir: str) -> None:
    for marker in (
        ...
        'runtime(feature="bytes-string",operation="get-string-utf8")',
    ...
    neighboring_operations = {
        ...
        for feature in ("array", "array-join", "bytes-string", "string")
    }
...
def run_negative_cases(root: Path) -> None:
    expected = {
        ...
        "raw_native_encoding": ("HXC1001", "TCall(Bytes.getString:encoding-not-UTF8)"),
...
def main(argv: Iterable[str] = ()) -> int:
    ...
    print(
        "bytes-runtime: OK: "
        f"{families}; {mode}; C11 O0/O2 and C++17 header consumers, aliasing, overlap, bounds, checked UTF-8 snapshots, call-scoped mutable C text, allocation rollback, ownership, sanitizers, and selective symbols passed"
    )
    return 0
