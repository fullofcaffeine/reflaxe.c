...
class ToolchainShardTests(unittest.TestCase):
    ...
    def test_runtime_piloscript_change_selects_its_generic_owner(self) -> None:
        focused_paths = (
            "examples/caxecraft/runtime-piloscript-c.hxml",
            "examples/caxecraft/src/caxecraft/pilot/RuntimePilotScript.hx",
            "examples/caxecraft/test/caxecraft/qa/RuntimePilotScriptProbe.hx",
            "examples/caxecraft/test/native/runtime_piloscript_harness.c",
            "examples/caxecraft/pilots/active.piloscript",
        )
        for path in focused_paths:
            with self.subTest(path=path):
                plan = self.route_selector.build_test_plan((path,))
