...
void hxc_Main_main(void)
{
  ...
  {
    if (hxc_io_println((hxc_string){ (const uint8_t *)"conditional string failure", 26, true, NULL }) != HXC_STATUS_OK)
    {
      abort();
    }
  }
  if (hxc_io_println((hxc_string){ (const uint8_t *)"\303\251\360\237\231\202", 6, true, NULL }) != HXC_STATUS_OK)
  {
    abort();
  }
  if (hxc_io_println((hxc_string){ (const uint8_t *)"embedded\000NUL", 12, true, NULL }) != HXC_STATUS_OK)
  {
    abort();
  }
  if (hxc_io_println((hxc_string){ (const uint8_t *)"Main.hx:9: traced", 17, true, NULL }) != HXC_STATUS_OK)
  {
    abort();
  }
  return;
