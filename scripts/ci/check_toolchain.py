...
def validate(root: Path, *, require_tools: bool) -> list[str]:
    ...
    expected_string_char_at_script = "python3 test/differential/string-char-at/run.py"
    expected_string_null_script = "python3 test/differential/string-null/run.py"
    ...
    expected_caxecraft_runtime_schemas_script = (
    ...
    expected_caxecraft_resolved_level_plan_script = (
        "python3 examples/caxecraft/run_haxe_c_test.py resolved-level-plan"
    )
    expected_caxecraft_content_generation_script = (
        "python3 examples/caxecraft/run_haxe_c_test.py content-generation"
    )
    expected_caxecraft_runtime_level_loader_script = (
    ...
    expected_caxecraft_runtime_content_publication_script = (
    ...
    expected_toolchain_script = (
        ...
        "npm run test:arithmetic-semantics && npm run test:primitive-differential && npm run test:span-lowering && npm run test:project-layout && npm run test:caxecraft-localization && npm run test:caxecraft-content-pack && npm run test:caxecraft-content-json && npm run test:caxecraft-runtime-schemas && npm run test:caxecraft-actor-composition && npm run test:caxecraft-package-store && npm run test:caxecraft-resolved-level-plan && npm run test:caxecraft-content-generation && npm run test:caxecraft-runtime-level-loader && npm run test:caxecraft-runtime-content-generation && npm run test:caxecraft-runtime-content-publication && npm run test:caxecraft-water && npm run test:caxecraft-aquatics && npm run test:caxecraft-session && npm run test:caxecraft-app-screen && npm run test:caxecraft-presentation && npm run test:caxecraft-terrain-chunks && npm run test:caxecraft-inventory && npm run test:caxecraft-gameplay && npm run test:caxecraft-pilot && npm run test:caxecraft-scenario-model && npm run test:caxecraft-caxeflow && npm run test:caxecraft-editor && npm run test:caxecraft-scenario-determinism && npm run test:caxecraft-domain:full && npm run snapshots:catalog"
    )
    if (
    ...
    if (
        not isinstance(scripts, dict)
        or scripts.get("test:int-map") != expected_int_map_script
    ):
        errors.append("package.json must retain the typed IntMap runtime gate")
    if (
        not isinstance(scripts, dict)
    ...
    if (
        not isinstance(scripts, dict)
        or scripts.get("test:string-char-at") != expected_string_char_at_script
    ):
        errors.append("package.json must retain the ordinary String.charAt differential gate")
    if (
        not isinstance(scripts, dict)
        or scripts.get("test:string-null") != expected_string_null_script
    ):
        errors.append("package.json must retain the legacy-nullable String differential gate")
    if (
        not isinstance(scripts, dict)
    ...
    if (
        not isinstance(scripts, dict)
        or scripts.get("test:caxecraft-runtime-schemas")
        != expected_caxecraft_runtime_schemas_script
    ):
    ...
    if (
        not isinstance(scripts, dict)
        or scripts.get("test:caxecraft-package-store")
