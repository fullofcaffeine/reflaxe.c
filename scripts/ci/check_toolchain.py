...
def validate(root: Path, *, require_tools: bool) -> list[str]:
    ...
    expected_toolchain_script = (
        ...
        "npm run test:runtime-features && npm run test:array-runtime && npm run test:string-map && npm run test:string-char-at && npm run test:bytes-runtime && npm run test:gc-runtime && npm run test:string-runtime && npm run test:string-output && npm run test:hello && npm run test:hxc-ir && npm run test:primitive-semantics && "
