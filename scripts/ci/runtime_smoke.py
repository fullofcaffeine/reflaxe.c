...
INT_MAP = ROOT / "test/differential/int-map/run.py"
...
def run_toolchain(toolchain: Toolchain, build: Path) -> tuple[str, ...]:
    ...
    run_command(
        ...
        label=f"{family} IntMap runtime contract",
    )
    lanes.append("int-map-runtime-contract")
    ...
    run_command(
        ...
        label=f"{family} fixed-length Bytes runtime contract",
    )
    lanes.append("bytes-runtime-contract")
