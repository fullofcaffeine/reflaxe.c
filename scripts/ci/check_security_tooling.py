...
def validate_config_and_hooks() -> None:
    ...
    for hook_name in (
        ...
        if "BEADS INTEGRATION v1.1.0" not in hook:
            raise SecurityToolingFailure(
                f"{hook_name} must retain the stock Beads managed integration"
            )
        if f"bd hooks run {hook_name}" not in hook:
            raise SecurityToolingFailure(
                f"{hook_name} must delegate to stock bd"
            )
    ...
    for required in (
        ...
        if required not in beads_scan:
            raise SecurityToolingFailure(
                f"current Beads record scan lost required behavior: {required}"
            )
    if "resolve-reviewed" in beads_scan or "HXC_DOLT_BIN" in beads_scan:
        raise SecurityToolingFailure(
            "current Beads scan must use stock bd without custom resolvers"
        )
    safe_push = read_text(ROOT / "scripts/beads/push-safe.sh")
    if (
        "scripts/security/run-beads-gitleaks.sh" not in safe_push
        or 'bd -C "$ROOT_DIR" dolt push "$@"' not in safe_push
    ):
        raise SecurityToolingFailure(
            "Beads publication must scan current decoded records before Dolt push"
        )
    ...
    if "bd --version" not in installer or EXPECTED_BEADS_VERSION not in installer:
        raise SecurityToolingFailure("hook installer must require stock Beads 1.1.2")
    ...
    expected_scripts = {
        ...
        "security:beads-current": "bash scripts/security/run-beads-gitleaks.sh",
...
def main() -> int:
    try:
        ...
        print(
            ...
            f"stock Beads {EXPECTED_BEADS_VERSION}, "
            "staged/Git/current-Beads gates, fail-closed hook delegation, "
