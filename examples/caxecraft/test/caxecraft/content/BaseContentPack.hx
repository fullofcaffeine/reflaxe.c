...
/**
 * Keeps selected built-in mechanics available to older focused regressions.
 *
 * Production code reads the packaged JSON through `RuntimeContentRegistry`.
 * This module is deliberately test-only and remains bounded by
 * `haxe_c-xge.20.4.3.8.3`, which owns replacing its duplicated facts.
 */
...
/**
 * Supplies fixed expectations to older focused mechanics tests.
 *
 * This test-only fixture is not game content and is never compiled into the
 * playable application. The game reads `content.json` through
 * `RuntimeContentRegistry`; these manually reviewed values remain only while
 * the older actor and level-plan regressions move to that runtime model.
 * Keeping the fixture under `test/` prevents a second product authority while
 * preserving useful compiler diagnostics during that migration.
 */
