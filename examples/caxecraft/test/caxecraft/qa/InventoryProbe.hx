...
final class InventoryProbe {
	static function main():Void {
		...
		require(Inventory.SLOT_COUNT == 9
			&& Inventory.itemAt(3) == ItemKind.Haxeforge
			&& Inventory.itemAt(7) == ItemKind.Lantern
			&& Inventory.itemAt(8) == ItemKind.SandBlock,
			"closed item order includes collectible sand without moving existing slots");
		require(Inventory.selectedBlock(state) == BlockKind.Grass, "selected block mapping");

		state = Inventory.cycle(state, -1);
		require(state.selected == 8, "negative cycle wraps");
		...
		state = Inventory.collectBlock(state, BlockKind.Sand);
		require(state.sand == 1, "mined sand returns one typed sand block");
		...
		Sys.println("caxecraft-inventory: 9 typed slots; selection, wrap, consume, lossless collect, empty, and full bounds passed");
	}
