...
the validated CaxeMap; it advances water on the fixed clock, draws translucent
surfaces, swims, applies Adventure drowning damage, fades the underwater camera,
and shows fixed-tick breath bubbles. `WaterSnapshot` preserves mutable water for
the future complete save format. Tideweave pickup/equipment, audio, actor/rule
composition, and native runtime map loading remain later work.
