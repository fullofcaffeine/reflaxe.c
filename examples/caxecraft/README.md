...
grass top, grass side, soil, stone, or foundation-rock cells for each outward
face. Its focused Eval probe checks those choices and the half-pixel texture
inset through `npm run test:caxecraft-gameplay`.

The C renderer scans the complete current 32 by 32 world, submits only faces
that touch air, and places them inside one texture-bound `rlgl` quad region.
...
That one region removes the old moving nine-block render radius, so blocks no
...
frame and a bounded exit. This full-world scan is intentionally proportional
to the small feasibility map. Larger editor-authored maps will earn cached
chunk meshes when measurement shows the need; gameplay storage and atlas names
do not depend on that later optimization.
...
manifest, empty runtime plan, generated program header, inventory HUD, terrain
renderer, and Raylib application module. The check rejects generated `goto`, `hxrt`, and
allocation calls; requires direct window, input, texture, and draw calls; and
pins six load/check/unload ownership sites, one typed world-billboard borrow,
