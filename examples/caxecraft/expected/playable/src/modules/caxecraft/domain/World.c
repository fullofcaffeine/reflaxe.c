...
void hxc_caxecraft_domain_World_prepareSpawnMeadow(uint8_t *hxc_cells, size_t hxc_length)
{
  uint8_t *hxc_borrow = hxc_cells;
  ...
  while (1)
  {
    ...
    while (1)
    {
      ...
      while (1)
      {
        ...
        enum hxc_caxecraft_domain_BlockKind hxc_kind = hxc_caxecraft_domain_BlockKind_Air;
        if (hxc_y == 0)
        {
          hxc_kind = hxc_caxecraft_domain_BlockKind_Bedrock;
        }
        else
        {
          if (hxc_y < 4)
          {
            hxc_kind = hxc_caxecraft_domain_BlockKind_Dirt;
          }
          else
          {
            if (hxc_y == 4)
            {
              hxc_kind = hxc_caxecraft_domain_BlockKind_Grass;
            }
          }
        }
        int32_t hxc_tmp_load_result_n10 = hxc_x;
        int32_t hxc_tmp_load_result_n11 = hxc_y;
        struct hxc_caxecraft_domain_BlockCoord hxc_tmp_call_result_n13 = hxc_caxecraft_domain_World_coord(hxc_tmp_load_result_n10, hxc_tmp_load_result_n11, hxc_z);
        hxc_caxecraft_domain_World_replace(hxc_cells, hxc_length, hxc_tmp_call_result_n13, hxc_kind);
        hxc_y = hxc_i32_add_wrapping(hxc_y, 1);
      }
      hxc_x = hxc_i32_add_wrapping(hxc_x, 1);
    }
