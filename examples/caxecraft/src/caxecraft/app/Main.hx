...
final class Main {
	...
	static function main():Void {
		...
		var player:PlayerState = PlayerPhysics.recoverSpawn(cells, PlayerPhysics.player(16.5, spawnY, 16.5));
		var inventory:InventoryState = Inventory.starter();
		var guide:GuideState = GuideNpc.start(cells, 17.5, 13.5);
		var mossling:MosslingState = Mossling.start(cells, 14.5, 11.5);
		var lookX = 0.0;
		var lookY = -0.18;
		var lookZ = -1.0;
		var accumulator = 0.0;
		...
		while (!quit && !Raylib.WindowShouldClose()) {
			...
			final minePressed = PilotScript.minePressed(pilotAction);
			...
			final minePressed = frameInput.minePressed;
			...
			if (!onTitle && !paused && interactPressed && GuideNpc.isInRange(guide, player.x, player.z)) {
			...
			if (captured && !recapturedThisFrame && hit.hit && minePressed) {
				final removedKind = World.query(cells, World.coord(hit.cellX, hit.cellY, hit.cellZ));
				if (World.remove(cells, World.coord(hit.cellX, hit.cellY, hit.cellZ)) && selectedMode == GameMode.Adventure)
			...
			if (onTitle) {
				...
				drawHud(renderCounters.visible, renderCounters.drawCalls, frameCount, updateCount, paused, captured, placementBlockedFrames > 0, hit,
					player.x, player.z, selectedMode, language, inventory, guide, mossling, hudTexture, hudTextureReady, itemTexture, itemTextureReady);
			}
			Raylib.EndDrawing();
			#if caxecraft_pilot
	...
	static function drawActors(guide:GuideState, mossling:MosslingState):Void {
	...
	static function drawHud(visible:Int, drawCalls:Int, frames:Int, updates:Int, paused:Bool, captured:Bool, placementBlocked:Bool, hit:RaycastHit,
		...
		Raylib.DrawText("WASD MOVE  1-8/WHEEL ITEMS  SPACE JUMP  E TALK  LMB MINE  RMB USE  ESC PAUSE  Q QUIT", 20, height - 22, 14, text);
