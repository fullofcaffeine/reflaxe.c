...
/**
	...
	This stateless module owns a character body's axis-aligned collision box and
	the order in which X, Z, and Y motion is resolved. `Aquatics` calculates
...
function canPlaceAt(state:CharacterBody, coord:BlockCoord):Bool {
	...
	final bodyMaximumX = state.x + HALF_WIDTH;
	final bodyMinimumY = state.y;
	final bodyMaximumY = state.y + HEIGHT;
	final bodyMinimumZ = state.z - HALF_WIDTH;
	final bodyMaximumZ = state.z + HALF_WIDTH;
	var separated = false;
	if (bodyMaximumX <= coord.x)
		separated = true;
	if (bodyMinimumX >= coord.x + 1)
		separated = true;
	if (bodyMaximumY <= coord.y)
		separated = true;
	if (bodyMinimumY >= coord.y + 1)
		separated = true;
	if (bodyMaximumZ <= coord.z)
		separated = true;
	if (bodyMinimumZ >= coord.z + 1)
		separated = true;
	return separated;
}
