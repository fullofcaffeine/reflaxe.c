...
import caxecraft.domain.PlayerPhysics.FIXED_SECONDS;
import caxecraft.domain.PlayerPhysics.HEIGHT;
import caxecraft.domain.PlayerPhysics.input as landInput;
import caxecraft.domain.PlayerPhysics.resolveVelocity;
import caxecraft.domain.PlayerPhysics.step as stepOnLand;
...
/**
	...
	ordinary player collision box, then update breath from the final head sample.
...
function start(capability:AquaticProfile):PlayerAquaticState {
	return {
		medium: Dry,
		submersion: 0.0,
		headSubmerged: false,
...
function input(moveX:Float, moveZ:Float, ascend:Bool, descend:Bool):AquaticInput {
	return {
		moveX: moveX,
		moveZ: moveZ,
		jump: ascend,
...
/**
	...
	player's center column; collision continues to use the complete horizontal
	player box.
**/
function observe(cells:WorldCells, player:PlayerState):PlayerImmersion {
	final feetY = player.y;
	final headY = player.y + HEIGHT;
	var submergedHeight = 0.0;
	var cellY = floorToInt(feetY);
	final lastY = floorToInt(headY - EPSILON);
	while (cellY <= lastY) {
		final top = waterSurface(cells, player.x, cellY, player.z);
		if (top > cellY) {
	...
	final eyeY = player.y + HEAD_SAMPLE;
	final eyeSurface = waterSurface(cells, player.x, floorToInt(eyeY), player.z);
	final cameraDepth = eyeSurface - eyeY;
	return {
		submersion: clamp(submergedHeight / HEIGHT, 0.0, 1.0),
		feetWet: pointIsWet(cells, player.x, player.y + FEET_SAMPLE, player.z),
		bodyWet: pointIsWet(cells, player.x, player.y + BODY_SAMPLE, player.z),
		headWet: pointIsWet(cells, player.x, eyeY, player.z),
...
function classifyMedium(previous:PlayerMedium, immersion:PlayerImmersion):PlayerMedium {
...
function step(cells:WorldCells, original:PlayerState, aquatic:PlayerAquaticState, command:AquaticInput, capability:AquaticProfile):PlayerAquaticStep {
...
function canMine(state:PlayerAquaticState, capability:AquaticProfile):Bool {
...
private function advanceBreath(previous:PlayerAquaticState, immersion:PlayerImmersion, capability:AquaticProfile):BreathStep {
