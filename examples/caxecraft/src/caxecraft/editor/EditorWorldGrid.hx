...
/**
	...
	This module has no persistent grid object: each function receives all of its
	input and returns a new world or an error. Module functions express that
	stateless ownership more directly than a class containing only static
	methods.

	The grid operations must be module-public because `EditorCommandReducer`
	imports them from another source file. They are nevertheless internal
	building blocks, not the supported editor API: calling them directly would
	skip the snapshot isolation, history, and validation coordinated by
	`EditorSession`. Haxe's `@:noCompletion` metadata keeps each annotated
	function out of ordinary editor and IDE completion lists so application
	authors are guided toward that safer session boundary. It does not make a
	function private, weaken type checking, authorize access, or add a runtime
	check. Explicit internal imports still compile normally, and haxe.c emits no
	special C code for the metadata.
**/
@:noCompletion
function resize(world:ScenarioWorld, nextSize:VoxelSize):EditorWorldResult {
...
@:noCompletion
function paintMany(world:ScenarioWorld, points:Array<VoxelPoint>, paletteCode:Int):EditorWorldResult {
	for (point in points)
...
/** Rewrite every voxel in one validated rectangular selection. */
@:noCompletion
function fill(world:ScenarioWorld, bounds:VoxelBounds, paletteCode:Int):EditorWorldResult {
...
/** True when one voxel coordinate lies inside the finite world. */
@:noCompletion
function containsPoint(size:VoxelSize, point:VoxelPoint):Bool
	return point.x >= 0 && point.y >= 0 && point.z >= 0 && point.x < size.width && point.y < size.height && point.z < size.depth;

/** True when a positive rectangular region fits completely inside the world. */
@:noCompletion
function containsBounds(size:VoxelSize, bounds:VoxelBounds):Bool
...
/** Return the exact voxel count for a size already admitted by editor bounds. */
@:noCompletion
inline function volume(size:VoxelSize):Int
	return size.width * size.height * size.depth;

private function validSize(size:VoxelSize):Bool {
	if (size.width <= 0
...
private inline function remaining(edge:Int, origin:Int):Int {
...
private inline function index(size:VoxelSize, x:Int, y:Int, z:Int):Int
