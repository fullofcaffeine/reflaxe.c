...
Twenty-six rows are `partial`: literal-only `Sys.println` and default
`haxe.Log.trace`, ten bounded `haxe.io.Bytes` entries, eight bounded `String`
entries, four bounded `StringBuf` entries, one `Array<String>.join` entry, and
one `Std.string` entry covering only typed `Bool`, `Int`, and String identity.
The other 2,284 rows are `planned`. A partial row means only its named operation
