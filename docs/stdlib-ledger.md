...
Twenty-two rows are `partial`: literal-only `Sys.println` and default
`haxe.Log.trace`, ten bounded `haxe.io.Bytes` entries, six bounded `String`
entries, and four bounded `StringBuf` entries. The other 2,288 rows are
