...
The class-layout suite adds private nominal structs, embedded base prefixes,
explicit nullable-reference checks, inherited field access, identity, safe
...
keep the same semantics. It also switches over a String-backed enum abstract
stored in a record, proving one-time subject evaluation, ordered content
comparison, a default branch, and a fail-stop edge for forged values in a
default-free exhaustive switch. Eval, HxcIR, strict C11, C++17 header
consumers, layout/discovery determinism, and sanitizers cover the positive
...
`test/class_layout` is the focused E3.T04
positive/negative/snapshot/runtime suite. Ordinary non-generic classes receive
private concrete C structs with source-order own fields and a real first base
subobject; empty roots receive only the strict-C anchor needed to make storage
complete. HxcIR records nullable class pointers, same-block checked null proofs,
inherited field places, identity operations, and explicit representation-safe
upcasts. Generated structural assertions compile under strict C11, while an
independent C provider and C++17 consumer compare size, alignment, offsets,
base paths, field behavior, null preservation, and identity at O0/O2 under
GCC/G++ and Clang/Clang++. Production roots are deterministic and runtime-free
across portable, metal, and explicit runtime-none policies, retain no public
ABI exports, and fail closed on interfaces, generic classes, and downcasts.
See [concrete class instance layouts](class-layout.md).

`test/constructor_lowering` is the focused E3.T05
positive/negative/snapshot/runtime suite. It compares pinned-Haxe Eval order
with generated strict C, checks default automatic storage and ordinary private
constructor calls, proves trivial-chain elision, and validates explicit status
propagation plus reverse partial-initialization cleanup. Repeated, reversed,
portable, metal, and runtime-none builds remain byte-identical and runtime-free;
escape, cycle, conditional, native-layout, and generic boundaries fail closed.
...
plus exhaustive tag-switch edges in schema-19 HxcIR. Recursive values use
...
policies visible in schema-19 HxcIR, executes mutation and iteration at O0/O2,
...
schema-19 HxcIR; checks exact byte lengths, runtime root reasons, stdlib reachability,
