...
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,aggregate_lowering,arithmetic_semantics,body_lowering,c_import,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-21 semantic core with production consumers | Schema 21 distinguishes immutable block-local values from mutable places and records ordered instructions, typed block/tag-switch edges, operation/dispatch kinds, ABI integer/nullability identities, explicit binary32 rounding/binary64 widening, arrays/spans, receiver-tied read-only fixed-array span returns, call-scoped mutable C-string buffers, aggregates, enums, classes, caller-owned class parameter borrows, embedded-child borrow operations and automatic borrow aliases, header-owned nominal imports/constants, distinct Haxe and borrowed C string literals, exact native dispatch, absent/virtual/runtime header intent, hierarchy layouts/slots/tables, checked construction of object-plus-interface-table values, failure successors, fixed-array zero initialization, explicit branch-assigned unmanaged carriers, managed-enum carriers with fresh-move or borrowed-retain acquisition, initialization, explicit retain/release, lifetime, cleanup, and exact collector-managed parameter/result roots including typed paths through direct records, active enum payloads, and present optionals. The validator proves repository-relative spans, reference/type/order invariants, borrow no-escape and alias-reload rules, exact receiver-field origin and immediate-use rules for returned read-only spans, single-native-consumer lifetime for mutable C-string buffers, exact import literal invariants, interface object/table compatibility, virtual override representations, bounded fixed-array automatic storage, unmanaged branch assignment before reads, exact one-owner acquire-before-move flow for managed enum joins, fresh-value provenance, managed aggregate/enum lifecycle types, managed-root representation/path/definition ownership, and dispatch-plan consistency. E2.T05-T09 consume bounded primitive/output/span/static families; E3 consumes records, enums, classes, construction, and dispatch; the bounded E6 slice consumes exact direct imports, Float32, and call-scoped mutable text. Reordered dumps, stable negative diagnostics, generated root-lifetime/layout evidence, and no-implicit-runtime assertions pass. Interface-value C emission and safe casts remain owned by E3.T07; broader semantic analyses and runtime use remain. |
...
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/{function_lowering,generic_specialization,evaluation_order,string_output}/**`, canonical function/generic/evaluation-order/string-output cases | Compile-verified production sequenced primitive and closed-generic executable slice with direct optional/default completion and literal output | The typed reachable direct-static graph is collected cycle-safely from the captured program; signatures, globals, default values, and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private declarations precede definitions. Proven direct static and direct instance calls append omitted trailing typed defaults before HxcIR, so C retains one fixed-arity function and explicit null remains distinct from omission. The bounded constructor slice reuses the same completion rule for its independently admitted parameter families. Closed generic instances use the same graph with applied signatures and private specialization identities. Closed recursive cycles in the remaining unconditional single-block subset are compiler-proven C11 `_Noreturn`; multi-block bodies are excluded conservatively. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other closed-cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Rest arguments, indirect calls, unresolved virtual/interface omission, and unsupported nullable representations fail with scoped source-positioned `HXC1001`. Primitive and closed-generic portable, metal, and explicit runtime-none roots emit byte-identical runtime-free strict C; compiler-known literal output emits the exact selective runtime closure. Broader dispatch, collection/pattern control flow, objects, open/reference generics, nonliteral strings, exports, and standard library remain fail-closed. |
...
| Direct C imports (`direct-c-import`) | `interop/CImportRegistry.hx`, `std/c/{CString,CStringBufferRef,Float32}.hx`, `docs/typed-c-authoring.md`, `test/{c_import,differential/bytes-runtime}/**`, `test/native/pointlib/**` | Compile-verified bounded runtime-free and selective-runtime C-library consumption | Reached hand-authored externs with exact `@:c.name` and authoritative includes lower fixed-arity C calls, scalars including exact binary32 `float`, typedef identities, fieldless enum/constants, by-value imported structs/fields, allocation-free literal borrowed C strings, call-scoped mutable scalar references, and one-call mutable text buffers through schema-21 HxcIR and structural C AST. `c.CStringBufferRef.to(bytes)` keeps Bytes as the owner, selects only the existing Bytes runtime closure, obtains a checked `char *`, and permits exactly one direct native consumer; imports that do not use managed storage remain runtime-free. Reachable Float32 emits target-side format assertions; explicit narrowing/widening retains rounding intent. The neutral build plan deduplicates reached include/library/pkg-config/framework facts with source owners; generated C never recreates header-owned definitions. Independent compiled probes verify pointlib size, alignment, offsets, constants, Float32 boundaries, mutable-text capacity and aliasing, and behavior under strict C11/C++17, O0/O2, and sanitizers. Callbacks, variadics, general pointers and retained lifetimes, opaque ownership, unions, packing, C++, public exports, full bindgen, and stable ABI fail closed. |
