...
A present file is not evidence that its full capability is implemented. For example, the presence of `CASTPrinter.hx` means a printer seed exists; it does not mean all C11 declarators or Haxe lowering semantics are complete.
...
| Structural C AST and printer (`c-ast-printer`) | `ast/CAST.hx`, `ast/CASTPrinter.hx`, `test/all_sources/**` | Compile-verified seed | The substantial declaration/expression/statement model and deterministic printer seed type-check under the pinned toolchain, including a minimal empty-unit execution probe. Declarator completeness, precedence, attributes, dialect gates, provenance, comments/source maps, invariants, and a compile-backed golden corpus remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each compiles/runs hosted C11, compiles the freestanding path, links/runs a C++17 public-header consumer, and exercises independent C-library and opaque-handle C++-shim fixtures. Sanitizers, generated-Haxe output, broader platform matrices, ownership/failure paths, and installed external consumers remain later gates. |
