...
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/hxc_ir/**` | Compile-verified deterministic semantic core | Schema 1 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, all dispatch kinds, conversions, allocation/implementation intent, initialization/lifetime transitions, failure successors, and reverse inner-to-outer cleanup execution. Repository-relative source spans, reordered-input canonical dumps, stable `HXC1001` unsupported-node and `HXC9000` invariant negatives, and no-implicit-runtime assertions pass. Typed-module collection/normalization is now present; production Haxe-to-HxcIR lowering, later semantic analyses, HxcIR-to-C lowering, runtime-plan finalization, and generated-program evidence remain. |
