...
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{aggregate-lowering,arithmetic-semantics,body-lowering,class-layout,constructor-lowering,enum-lowering,evaluation-order,function-lowering,generic-specialization,hxc-ir,span-lowering,static-initialization,string-runtime,virtual-dispatch}.md`, `test/{typed_ast,all_sources,aggregate_lowering,arithmetic_semantics,body_lowering,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified fail-closed sequenced primitive/span/closed-record/class-reference/bounded-constructor/virtual-dispatch/bounded-enum/closed-generic/literal-output executable lowering | `CCompiler` consumes one normalized request, plans initialization, and collects the reachable direct-static, constructor, generic, and instance-method graph before typed HxcIR lowering. The admitted slice includes sequenced primitive control flow and operations, local arrays/spans, records, concrete class references/fields/upcasts, bounded nonescaping `new`, ordinary instance methods, minimal reachable class virtual dispatch, bounded enums, closed generic static functions/enums, and compiler-known literal output. Final/private/metadata/`super` instance calls stay direct; ordinary overridable calls use representation-validated hierarchy slots and conditional `hxc.dispatch.json` evidence. Constructors retain exact pinned-Haxe order/status/cleanup and bind selected program-local tables before user code. Unsupported nodes/signatures emit source-positioned `HXC1001`, while unorderable static cycles emit `HXC1002`; both leave no output. Fully admitted graphs emit owned private header/source projects with analyzed sidecars and no public exports. Primitive, record, class-reference, constructor, virtual-dispatch, enum, and closed-generic programs remain runtime-free; literal output alone packages `runtime-base + status + string-literal + io`. General allocation, interfaces, closures, dynamic/reflection, open generics, and broader stdlib/runtime semantics remain fail-closed. |
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,aggregate_lowering,arithmetic_semantics,body_lowering,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-7 semantic core with production consumers | Schema 7 distinguishes immutable block-local values from mutable places and records ordered instructions, typed block/tag-switch edges, operation/dispatch kinds, ABI integer/nullability identities, arrays/spans, aggregates, enums, classes, absent/virtual/runtime header intent, hierarchy layouts/slots/tables, table binding, receiver-bearing virtual calls, safe upcasts, failure successors, initialization, lifetime, and cleanup. The validator proves repository-relative spans, reference/type/order invariants, exact override representations, applicable table implementations, and root/table/bind consistency. E2.T05-T09 consume the bounded primitive, output, span, and deferred-global families; E3.T01-E3.T05 consume records, enums, class layouts, and bounded construction; E3.T06 consumes reachable minimal class dispatch. Reordered dumps, stable `HXC1001`/`HXC9000` negatives, and no-implicit-runtime assertions pass. Broader semantic analyses and runtime use remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `examples/hello/**`, `test/runtime/runtime-feature-graph/**`, `test/{aggregate_lowering,arithmetic_semantics,c_ast,class_layout,constructor_lowering,virtual_dispatch,declaration_plan,differential/array-runtime,project_emitter,body_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/aggregate/class-reference/constructor/dispatch/generic/span/literal-output/hello projects | Local auto mode reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ warning-as-error lanes. Generated projects run at O0/O2; aggregate/class/constructor/dispatch/enum suites also compile independent C++17 private-header consumers. The dispatch lane proves runtime-free indirect behavior, minimal tables, typed adapters, and preserved direct calls. Runtime packages, sanitizer-eligible arithmetic/array cases, span/class/constructor zero-`hxrt` inspection, literal output, and exact hello output retain their bounded evidence. Independent `.cpp` consumers prove only C-header compatibility; they are not generated C++ output or public ABI evidence. Broader platforms, stable exports, and installed consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, `examples/hello/case.json`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic snapshots through the virtual-dispatch and hello vertical slices | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus evaluation order/control flow, arithmetic, fixed-array/span, static initialization, closed generic specialization, bounded constructor and virtual-dispatch lowering, E2.T07 literal output, and the declared E2.T10 hello example are mapped explicitly. Every expected artifact is registry-owned and reproducible only through the diffing updater, which cannot bless in CI. Direct AST/IR, generated programs, independent runtime/ABI seeds, oracle fixtures, and product examples retain their limited claims. Additional examples require schema-valid declared assertions and may not become implicit tests. |
