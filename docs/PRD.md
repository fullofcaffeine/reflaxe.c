...
| Structural validation and archive integrity (`structural-validation`) | `scripts/ci/check_prd.py`, `structure_check.py`, `verify_manifest.py`, all-source HXML, runtime smoke, CI workflow, package manifest | Verified native and Haxe scaffold | Local gates verify PRD/registry/plan synchronization, structure, reference evidence, the complete current owned Haxe graph and macro branches, fail-closed production activation, native fixtures, and package checksums. Generated-output, sanitizer, full platform, and release gates remain explicitly unverified. |
