...
- capturing the complete Haxe module-type set before Reflaxe callback filters,
  then deterministically classifying selected classes, enums, typedefs,
  abstracts, externs, metadata, entry-point facts, and source-module ownership;
...
out/
  ...
  hxc.specializations.json       # only when closed generic instances exist
  hxc.abi.json
  hxc.symbols.json
...
ADR 0008 fixes the primitive representation/conversion subset. Exact native
integers use the corresponding `<stdint.h>` typedef; `size_t`, `ptrdiff_t`,
`intptr_t`, and `uintptr_t` remain target-ABI identities and never inherit a
host `long` spelling or width. Ordinary Haxe mappings are identical in portable
and metal. Exact, wrapping, checked, saturating, and nullable conversions remain
distinct in HxcIR; checked and nullable-unwrap conversions carry failure edges.
For the Haxe-unspecified non-finite/out-of-range domain of `Std.int`, this target
defines NaN as zero and positive/negative overflow or infinity as the applicable
`int32_t` endpoint, using a program-local helper before the final safe C
conversion. Primitive lowering selects no `hxrt` feature.
...
The M0 implementations began as method-free compiler contracts except for the
compile-time-only `c.StaticAssert.require`. E2.T08 and its bounded storage
extension now admit operations only on `CArray`, `Span`, and `ConstSpan`: direct
nonempty literal initialization, positive compiler-known `CArray.zero` products
within a 65,536-byte per-array automatic-storage policy, local borrowing,
checked indexing, and compiler-lowered span iteration. Those methods have no
wrapper allocation or runtime representation; every other operational surface
remains fail-closed. Empty `c.Lib`, `c.Build`, or `c.Export` marker types are not
admitted: build and export intent uses validated declaration metadata until a
concrete typed value API earns its surface. `CArray<T, N>` preserves `N` as the
type-level length identity while deriving a literal or validated compile-time
element count from the typed AST; exact witness-generation ergonomics remain E3
work.
...
The implemented E1.T08 M1 seed is deliberately narrower than the complete E8
backend: one typed schema-1 non-installing executable plan emits optional CMake
and Meson views, and an independent argument-array consumer can ignore both.
The structural project plus both C AST/printer corpora build and run through all
three lanes with adversarial quoting. Library/export targets, configurable
target names, direct/Ninja orchestration, toolchain/sysroot/cross files, and CLI
...
The exact machine status vocabulary is: Implemented | Scaffold-only | Experimental | Unsupported.

- **Implemented** means the specifically bounded scope has executable checkout
  evidence; it does not imply adjacent language or product support.
- **Scaffold-only** means a contract, seed, fixture, or plan exists, but no
  user-program capability may be claimed from it.
- **Experimental** means an explicit opt-in surface exists without stability or
  supported-release guarantees.
- **Unsupported** means the surface is absent, deliberately fails closed, or
  has not passed the evidence needed for a product claim.

A seed is an implementation input, not a completed feature.

### 32.2 Inventory

The current-checkout inventory is `docs/specs/bootstrap-inventory.json`; its
schema and `scripts/ci/check_capability_manifest.py` enforce stable capability
IDs, the four statuses, evidence paths, owning Beads tasks, hard no-claim
gates, the generated README view, and the future `hxc doctor` consumption
contract. This table supplies the longer design explanation. Asset or status
changes must update both surfaces in one reviewed change.

| Area | Existing assets | Status | What Codex must understand |
| --- | --- | --- | --- |
| Bootstrap capability manifest (`bootstrap-capability-manifest`) | `docs/specs/bootstrap-inventory{,.schema}.json`, `scripts/ci/check_capability_manifest.py`, governance tests | Implemented | CI validates evidence paths, Beads owners, exact status/disposition pairs, hard M0 claim gates, README derivation, and the absent `hxc doctor` contract. The manifest reports evidence and changes no compiler behavior. |
| Typed Haxe boundary governance (`typed-boundary-governance`) | `docs/specs/typed-boundaries{,.schema}.json`, `scripts/ci/check_typed_boundaries.py`, `docs/typed-boundaries.md`, governance tests | Implemented | Every repository-owned Haxe file is scanned for `Dynamic`, `Any`, `Reflect`, `untyped`, open-ended carriers, casts, private access, and raw injection. The source-exact allowlist contains only the validated ownership-JSON ingress, one diagnostic negative, and explicit real-`Dynamic` semantic/negative fixtures; no untyped carrier enters semantic compiler logic. This guard does not implement user-language Dynamic or reflection semantics. |
...
| Target activation (`target-activation`) | `c/Init.hx`, `TargetPlatform.hx`, `BuildDetection.hx`, `CompilerBootstrap.hx`, `CompilerInit.hx`, `test/bootstrap/**` | Production typing carrier verified; lowering fail-closed | Cold, package, and back-to-back compiler-server probes exercise real `--custom-target c=<output>` activation, target-owned platform configuration, derived/matching output transport, non-C isolation, exactly-once counts, public/internal define visibility, source-anchored conflicts, and typed upstream scalar-Unicode branches. Full Reflaxe registration reaches exact source-positioned `HXC1001` at the bootstrap fixture's first unsupported call with no artifact. Legacy Cross remains an executable `HXC0003` negative; Eval is only an oracle/non-C host. |
| Profiles, runtime policy, environment, and build mode (`configuration-policies`) | `CProfile.hx`, `CRuntimePolicy.hx`, `CEnvironment.hx`, `CBuildMode.hx`, `ProfileResolver.hx`, `BuildModeResolver.hx`, `docs/configuration.md` | Scaffold-only with compile-backed build and literal-output runtime slices | `portable|metal`, `auto|minimal|none`, environment presets, and `debug|release|minsizerel` are represented. The span bounds-policy matrix proves build mode cannot weaken semantics and invalid direct values fail with `HXC0003`. Literal output proves portable `auto + summary`, metal `minimal + warn`, explicit diagnostic-off, runtime-none rejection, and hosted-only environment selection without changing generated behavior. No project configuration schema/template exists, and complete precedence, capability matrices, canonical reports, and provenance-aware manifest integration remain incomplete. |
| Per-build compiler state (`per-build-context`) | `CompilationContext.hx`, `test/{typed_ast,symbol_registry}/**`, `test/runtime/runtime-feature-graph/**` | Compile-verified typed-input/naming/runtime-plan state seed | A fresh context owns one normalized `TypedProgramInput`, one empty per-compilation `CSymbolRegistry`, and at most one typed runtime plan; adversarial rich-to-small compiler-server requests match cold output without module/context leakage, while reversed registry discovery is byte-identical. The admitted executable slice now reconciles runtime reasons against reachable HxcIR per request. Full resolved configuration, type/layout registries, broader pass state, diagnostics, outputs, full typed-AST symbol request collection, and later pipeline caches still need implementation. |
| Reflaxe adapter (`reflaxe-adapter`) | `CReflaxeCompiler.hx`, `frontend/Typed{ProgramInput,AstNormalizer,AstInventory}.hx`, `emit/**`, `docs/typed-ast-input.md`, `docs/project-emission.md`, `test/{typed_ast,project_emitter,function_lowering,generic_specialization,evaluation_order,string_output}/**`, `test/all_sources/**` | Compile-verified deterministic typed-input and output-ownership adapter | `filterTypes` captures and normalizes complete request input before Reflaxe filters or preprocesses callbacks; modules, declarations, primary/secondary ownership, externs, abstracts, typedefs, metadata, fields, original field expressions, and entry-point facts enter a raw-object-retaining model. Unordered module/declaration sets are sorted while field, enum, metadata, and expression-root order is retained explicitly. Reordered-input, repeated-cold, and compiler-server inventories are byte-identical after normalizing Reflaxe's exact transient build-cache hook. The manual output adapter validates typed content-addressed files, prior Reflaxe ownership, unowned collisions, and descendant symlinks before delegating every write/stale deletion. The admitted graph reaches production-owned private header/source-set/sidecars, closed generic static-function/enum instances, and exact selectively packaged literal-output runtime files; broader callbacks, open/reference generics, public output, and other runtime-using projects remain. |
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `interop/**`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{aggregate-lowering,arithmetic-semantics,body-lowering,class-layout,constructor-lowering,enum-lowering,evaluation-order,function-lowering,generic-specialization,hxc-ir,span-lowering,static-initialization,string-runtime,typed-c-authoring,virtual-dispatch}.md`, `test/{typed_ast,all_sources,aggregate_lowering,arithmetic_semantics,body_lowering,c_import,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified fail-closed sequenced primitive/span/closed-record/class-reference/bounded-constructor/virtual-dispatch/bounded-enum/closed-generic/literal-output/direct-import executable lowering | `CCompiler` consumes one normalized request, plans initialization and exact imports, and collects the reachable direct-static, constructor, generic, instance-method, and native-call graph before typed HxcIR lowering. The admitted slice includes sequenced primitive control flow and operations, explicit binary64/binary32 C-boundary conversions, literal and bounded compile-time-sized zero-initialized local arrays/spans, records, concrete class references/fields/upcasts, bounded nonescaping `new`, ordinary instance methods, minimal reachable class virtual dispatch, bounded enums, closed generic static functions/enums, compiler-known literal output, and exact fixed-arity scalar/typedef/enum/constant/by-value-struct C imports with literal borrowed C strings. Unsupported nodes/signatures emit source-positioned diagnostics and leave no output. Fully admitted graphs emit owned private header/source projects with analyzed sidecars and no public exports. Primitive, fixed-array/span, direct class-reference/constructor, virtual-dispatch, enum, closed-generic, and direct-import programs remain runtime-free when their settled value graph requires no managed storage. A class graph retained by another class, interface field, or `Array<Class>` instead selects the exact nonmoving collector closure; literal output alone packages `runtime-base + status + string-literal + io`. Callbacks, variadics, unproved pointer lifetimes, closures, dynamic/reflection, open generics, and broader stdlib/runtime semantics remain fail-closed. |
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,aggregate_lowering,arithmetic_semantics,body_lowering,c_import,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-23 semantic core with production consumers | Schema 23 distinguishes immutable block-local values from mutable places and records ordered instructions, typed block/tag-switch edges, operation/dispatch kinds, ABI integer/nullability identities, explicit binary32 rounding/binary64 widening, arrays/spans, receiver-tied read-only fixed-array span returns, call-scoped mutable C-string buffers, aggregates, enums, classes, caller-owned class parameter borrows, embedded-child borrow operations and automatic borrow aliases, header-owned nominal imports/constants, structural zero initialization of wholly unmanaged imported structs, distinct Haxe and borrowed C string literals, exact native dispatch, absent/virtual/runtime header intent, hierarchy layouts/slots/tables, checked construction of object-plus-interface-table values, closed-world child-interface to parent-interface table remapping, failure successors, fixed-array zero initialization, explicit branch-assigned unmanaged carriers, reference-counted managed-enum carriers with fresh-move or borrowed-retain acquisition, zero-initialized collector-bearing enum carriers, initialization, explicit retain/release, lifetime, cleanup, and exact collector-managed parameter/result roots including typed paths through direct records, active enum payloads, and present optionals. The validator proves repository-relative spans, reference/type/order invariants, borrow no-escape and alias-reload rules, exact receiver-field origin and immediate-use rules for returned read-only spans, single-native-consumer lifetime for mutable C-string buffers, exact import literal invariants, interface object/table compatibility, complete same-class parent-interface table mappings, virtual override representations, bounded fixed-array automatic storage, unmanaged branch assignment before reads, exact one-owner acquire-before-move flow for reference-counted enum joins, safe default initialization plus tag-aware roots for collector-bearing enum joins, fresh-value provenance, managed aggregate/enum lifecycle types, managed-root representation/path/definition ownership, and dispatch-plan consistency. E2.T05-T09 consume bounded primitive/output/span/static families; E3 consumes records, enums, classes, construction, and dispatch; the bounded E6 slice consumes exact direct imports, Float32, and call-scoped mutable text. Reordered dumps, stable negative diagnostics, generated root-lifetime/layout evidence, and no-implicit-runtime assertions pass. General checked interface downcasts remain owned by E3.T07; broader semantic analyses and runtime use remain. |
| Primitive semantic contract (`primitive-semantics`) | `semantics/CPrimitive{Types,Semantics,TypeMapper,Contract}.hx`, ADR 0008, `docs/{arithmetic-semantics,primitive-semantics}.md`, `docs/specs/primitive-semantics{,.schema}.json`, `test/{arithmetic_semantics,primitive_semantics}/**` | Compile-verified schema-3 typed contract with independent and generated arithmetic/Float32 proof | Real pinned-compiler typed fields map Void/Bool/Int/UInt/Float, `c.Float32`, exact/ABI integers, and scalar/reference/native-pointer nullability identically in portable and metal. The registered deterministic contract fixes safe conversions plus 26 representative operation decisions, edge refinements, direct/helper ownership, and zero runtime features. Independent strict C11 probes cover representation/conversion algorithms including binary32 ties, subnormals, underflow, overflow, NaN, infinity, and signed zero; generated-Haxe/import GCC/Clang O0/O2 projects retain C `float` fields/prototypes and explicit narrowing/widening. The broader conversion and primitive surface remains later work. |
| Primitive body lowering (`body-lowering`) | `lowering/{CBodyLowering,CBodyControlFlow,CBodyEmitter,CPrimitiveHelperEmitter,HaxeSourceSpan}.hx`, `docs/{arithmetic-semantics,body-lowering,control-flow-structuralization,evaluation-order}.md`, `test/{arithmetic_semantics,body_lowering,evaluation_order}/**`, canonical body/evaluation/arithmetic cases | Compile-verified real TypedExpr-to-HxcIR/C sequenced UB-safe body slice | Real pinned-Haxe primitive functions lower constants, parameters, locals/globals, direct calls/conversions, lazy/value and statement conditionals, pre/post-test and pinned range-loop graphs, innermost loop jumps, `Int` statement/value switches, typed arithmetic/comparisons, compound assignment, prefix/postfix updates, masked shifts, bit operations, zero-safe division/modulo, `Std.int`, blocks, and returns through validated HxcIR to structural strict C11. A separately verified region plan turns reducible HxcIR edges into structural C conditionals, loops, switches, returns, and lexical break/continue; continuing switch arms end in ordinary C `break`, compiler-exposed value-switch carriers require structurally complete assignments plus defensive initialization, and every helper is request-local, registry-named, and static inline. Safe UInt fast paths remain direct C, `fmod` contributes only the exact `m` build fact, and no operation selects `hxrt`. Repeated/reversed/profile renders match; required GCC/Clang lanes execute runtime-free C at O0/O2 against Eval and under eligible UBSan. |
| Closed anonymous-record lowering (`closed-anonymous-record-lowering`) | `lowering/{CBodyAggregate,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/aggregate-lowering.md`, `test/aggregate_lowering/**`, canonical aggregate cases | Compile-verified runtime-free direct value-record slice | Closed anonymous records of admitted primitive or nested record fields deduplicate across aliases by canonical UTF-8 structural shape and emit dependency-first private named C structs. HxcIR retains source-order object-literal evaluation before canonical named construction, explicit by-value copies, immutable projections, and direct local field places. Proven single-use private local/field reads may be coalesced into one later expression only inside the same block and across no observable, lifetime, cleanup, failure, or alias barrier. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private size/alignment/offset agreement under required GCC/G++ and Clang/Clang++ O0/O2 lanes. `Void` fields, identity/equality, mutation requiring alias analysis, open/reflective/Dynamic shapes, public ABI, and metal packed typed-C layouts remain fail-closed. |
| Concrete class instance layouts (`class-layout`) | `lowering/{CBodyClass,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/class-layout.md`, `test/class_layout/**`, canonical class-layout cases | Compile-verified private class-reference/layout slice with representation-selected runtime use | Ordinary concrete non-generic classes emit nominal private C structs with source-order own fields, an embedded first base subobject, and a strict-C anchor only for an otherwise empty root. HxcIR retains nullable instance pointers, explicit profile/build null proofs, inherited field places, identity operations, absent-unless-required header intent, null-preserving derived-to-base representation conversions, and one settled storage representation across each inheritance chain. A graph with no retained class edge keeps caller-owned direct storage and no runtime; a retained concrete-class edge promotes the owner, target, and inheritance closure to precise nonmoving collector storage with exact tracing. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private size/alignment/offset/base paths and field/reference behavior under required GCC/G++ and Clang/Clang++ O0/O2 lanes. Production ABI reports retain no public exports. E3.T06 may select one root-only program-local table pointer for a reachable polymorphic hierarchy; unrelated and final-direct hierarchies remain header-free. Dynamic dispatch, generic classes, reflection/type tests, broader unproved ownership shapes, and public ABI remain fail-closed. |
| Bounded constructor lowering (`constructor-lowering`) | `lowering/{CBodyConstructor,CBodyLowering,CBodyEmitter,CStaticFunctionGraph}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/constructor-lowering.md`, `test/constructor_lowering/**`, canonical constructor-lowering cases | Compile-verified bounded construction with selective managed parameter ownership | Direct nonescaping local `new` expressions for concrete non-generic classes in the function's outer statement sequence receive default-initialized automatic storage and ordinary private C constructor functions. A validated earlier guard may return before construction: that return captures no object cleanup, while every later exit carries the registered initialized-object cleanup. Construction inside one branch, loop body, or switch arm remains fail-closed because that object would need destruction before the nested body rejoins. Proven `new` and `super` calls append omitted trailing typed defaults before HxcIR; explicit null remains distinct from omission, non-managed tagged record optionals pass by value, and the IR validator sees only exact fixed-arity calls. Literal-backed String parameters preserve nominal abstract identity in constructor symbols and pass as immutable views; a first final field initialization copies the program-long literal view without allocation or ownership operations. Fieldless enum parameters preserve their exact instance identity and pass as nominal C tags. Tagged payload enums whose finalized graph has no managed lifetime also pass by value with their active payload intact; a first final field initialization copies either unmanaged enum representation without lifetime work. Constructor symbol admission forces enum lifecycle finalization first. Prepared `Array<T>` constructor parameters retain their exact specialization and remain call borrows; a first final `this.field = parameter` initialization retains one field owner for reference-counted Arrays, collector-backed Arrays use exact roots and tracing, and fresh literal arguments receive a caller cleanup owner. A fresh closed record with managed fields similarly receives one caller-owned automatic local before named, immediate-receiver, inline-child, `super`, or collector-managed construction. The constructor borrows that by-value record and retains an independent field owner only when it stores the value; success and failure edges destroy the caller and partial-object owners exactly once each. A retained concrete-class or interface field promotes the reachable graph to exact collector storage; constructor-only first initialization may store `this` in the object's own managed field, and native pressure verifies that self-cycle after collection. HxcIR preserves pinned-Haxe argument/field/`super`/body order, non-null `self`, trivial-chain elision, explicit infallible versus `status(exception)` conventions, nested propagation, and reverse partial/initialized cleanup. Embedded final children may use a validated non-owning automatic alias, including across lazy Boolean blocks, without gaining an independent lifetime; fallible inline children propagate status and clean the initialized prefix. Eval, repeated/reversed/two-profile/runtime-none where eligible, strict GCC/Clang O0/O2 C11, sanitizers, intentional fail-stop, and C++17 header evidence pass. E3.T06 may bind a selected program-local table before constructor code. Runtime-created or owned Strings, managed or recursive payload-enum constructor values, returned or otherwise unproved escaping references, retention outside the constructing object's own field, branch/loop/switch-local automatic construction, managed Array field replacement or storage through another object, constructor-dependency or direct by-value layout cycles, native layouts, generic/interface construction, unproven managed/callable parameter families, heap allocation beyond admitted traced graphs, catch/finally, dynamic dispatch, reflection/type metadata, and public ABI remain fail-closed. |
| Closed-world virtual dispatch (`virtual-dispatch`) | `lowering/{CBodyDispatch,CDispatchReport,CBodyLowering,CBodyEmitter,CStaticFunctionGraph}.hx`, `emit/{CProjectEmitter,CStaticFunctionProjectEmitter,GeneratedFile}.hx`, `docs/virtual-dispatch.md`, `docs/specs/dispatch-report.schema.json`, `test/virtual_dispatch/**`, canonical virtual-dispatch cases | Compile-verified deterministic runtime-free minimal class-dispatch slice | Reachable ordinary instance methods use explicit typed `self`; final/private/inline/`@:nonVirtual`/final-receiver/`super` calls remain direct. Ordinary overridable calls select UTF-8-ordered root slots and tables only for reachable constructed dynamic classes. Schema-7 HxcIR validates one root layout, exact table applicability/order/binding, and representation-identical arguments/results; more-derived receivers use typed program-local adapters. Source-level variance that would create incompatible C function pointers fails with exact source-positioned `HXC1001` and no output. Conditional schema-1 `hxc.dispatch.json` explains every call, slot, table, adapter, and empty runtime set. Cold/reversed/locale/profile/runtime-none/warm-server builds, strict GCC/Clang C11 O0/O2 execution, and C++17 private-header consumption pass. Interfaces, closures, dynamic/generic methods/classes, escaping allocation, general exceptions, reflection, public layouts, and stable method ABI remain fail-closed. |
| Haxe enum lowering (`tagged-enum-lowering`) | `lowering/{CBodyEnum,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/enum-lowering.md`, `test/enum_lowering/**`, canonical enum cases | Compile-verified bounded enum slice with selective runtime ownership | Fieldless concrete Haxe enums use private native C enums where representation permits, and ordinary equality/inequality compares their tags directly. Payload enums use readable discriminant enums, constructor payload structs, a union, and an outer value struct. HxcIR retains source-order construction, validated fieldless-enum equality, checked payload projection, exhaustive tag-switch edges, and explicit retain/release. Reference-counted conditional results move fresh owners or retain borrowed values through one checked carrier. A tagged enum containing collector-managed class references instead joins through a zero-initialized direct carrier with exact tag-aware roots, so either arm may allocate before assignment without exposing inactive union storage. Closed primitive generic instances participate in the shared E3.T03 specialization key/reason/limit/report contract. Recursive values use allocator-backed uniquely owned trees; copies clone, calls borrow, returns transfer or retain, and cleanup recursively destroys. Closed records and Array elements compose the same tag-aware lifecycle with rollback. Unmanaged programs remain runtime-free; managed programs select only their dependency-closed Array/allocator features. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private tags, equality, size/alignment/offsets, construction, and recursive layout under required GCC/G++ and Clang/Clang++ O0/O2 lanes; generated layouts also run under sanitizers. Public ABI, broader generic class/reference arguments, unsupported references, payload-enum identity/equality, reflection, and cycle-capable graphs remain unsupported. |
| Generic specialization (`generic-specialization`) | `lowering/{CGenericSpecialization,CGenericSpecializationContract,CGenericSpecializationReport,CStaticFunctionGraph,CBodyEnum,CBodyLowering}.hx`, `emit/{CProjectEmitter,GeneratedFile}.hx`, `docs/generic-specialization.md`, `docs/specs/generic-specialization-report.schema.json`, `test/generic_specialization/**`, canonical generic-specialization cases | Compile-verified deterministic bounded monomorphization slice | Reachable direct static generic calls infer closed non-null Bool/Int/UInt/Float, admitted enum, or selected direct-record arguments, expand aliases, keep finite same-nominal nesting distinct, and share full length-prefixed UTF-8 semantic keys. A transparent non-core abstract in a record field contributes its nominal path, closed arguments, and proven carrier key; an unsupported carrier remains fail-closed. SHA-256 is only a compact instance/symbol suffix and every registry rejects a full-key collision. Request-local worklist registration closes recursive discovery. Hard 64-function, 64-type, and 524,288 estimated-C-byte limits fail with source-positioned `HXC1001` rather than boxing. Schema-2 `hxc.specializations.json` records normalized arguments, sorted source reasons, recursion, isolated function bytes/hashes, reachable specialized-enum declaration/assertion bytes, and total generated payload; unrelated ordinary enums do not consume the monomorphization budget. Repeated roots, reversed modules, locale/profile/policy changes, and rejected warm-server requests are byte-identical where required; exact Dynamic/open/unsupported-carrier/65th-function/expanding-type/code-size negatives leave no output. The primitive/enum reference executable remains runtime-free; a specialized record composes only the already-admitted runtime plan of its fields. Strict GCC/Clang C11 O0/O2 reference executables and strict split/package/unity nominal-abstract record executables pass. Generic classes/references, general containers, descriptor bodies, public ABI, reflection, closures, and dynamic dispatch remain unsupported. |
| Fixed-array and span lowering (`span-lowering`) | `std/c/{CArray,Span,ConstSpan}.hx`, `ir/HxcIRFixedArrayPolicy.hx`, `lowering/{CBodyLowering,CBodyEmitter}.hx`, `docs/span-lowering.md`, `test/span_lowering/**`, canonical span cases | Compile-verified bounded local fixed-array/span runtime-free slice | Direct nonempty primitive literals and positive compiler-known `CArray.zero` products become structural automatic C arrays; zero storage uses `{ 0 }` without allocation or `memset`. The shared typed policy admits exact-size integer/binary32/binary64 elements up to 65,536 bytes per zero-initialized array. Mutable/const borrows become typed pointer-plus-`size_t` views, and exact-width span `for` becomes guarded HxcIR with no iterator object. Static out-of-range indexes fail before C; dynamic indexes retain signed/element-count fail-stop checks tagged with profile/build provenance. A 32 × 16 × 32 `UInt8` fixture proves ordinary-Haxe linearization, repeated mutation, borrowing, and iteration under strict GCC/Clang O0/O2. Exact-span size/bounds/escape negatives, repeated/reversed two-profile/three-build renders, and runtime-none no-allocation/no-`hxrt` inspection pass. General arrays, empty arrays, dynamic/heap/global storage, escaping views, broader elements/operations, whole-frame stack analysis, and stable public ABI remain fail-closed. |
| Static initialization (`static-initialization`) | `plan/CStaticInitialization*.hx`, `lowering/{CStaticFunctionGraph,CBodyLowering,CBodyEmitter}.hx`, `emit/{CStaticFunctionProjectEmitter,CProjectEmitter}.hx`, `docs/static-initialization.md`, `test/static_initialization/**`, canonical static-initialization cases | Compile-verified deterministic runtime-free primitive executable phase | The captured typed program produces a source-positioned dependency graph that recursively follows inheritance, interfaces, type/static/constructor references, and called static bodies. UTF-8-sorted roots and edges yield dependency-first type order; class `__init__` bodies run first, static fields run in type/source order, and one private hosted wrapper runs exactly once before Haxe entry for a non-empty plan; empty plans elide the wrapper, symbol, and call. Explicit fields are zero-initialized C storage with deferred HxcIR `():Void` functions containing one `initialize-global`. The schema-1 `hxc.initialization-plan.json` records reasons and order; reversed discovery is byte-identical; unorderable cross-type cycles fail with canonical `HXC1002`; the pinned Haxe JavaScript oracle and strict GCC/Clang O0/O2 harness agree without `hxrt`. Lazy/thread-safe initialization, registration, library init/shutdown, source-uninitialized/non-primitive statics, and non-hosted entry emission remain. |
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/{function_lowering,generic_specialization,evaluation_order,string_output}/**`, canonical function/generic/evaluation-order/string-output cases | Compile-verified production sequenced primitive and closed-generic executable slice with direct optional/default completion and bounded String output | The typed reachable direct-static graph is collected cycle-safely from the captured program; signatures, globals, default values, and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private declarations precede definitions. A directly reassigned Bool, Int, or Float parameter uses one initialized automatic HxcIR local for subsequent reads and writes, including compound Float assignment, while read-only parameters remain direct incoming values. Proven direct static and direct instance calls append omitted trailing typed defaults before HxcIR, so C retains one fixed-arity function and explicit null remains distinct from omission. The bounded constructor slice reuses the same completion rule for its independently admitted parameter families. Closed generic instances use the same graph with applied signatures and private specialization identities. Closed recursive cycles in the remaining unconditional single-block subset are compiler-proven C11 `_Noreturn`; multi-block bodies are excluded conservatively. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other closed-cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Rest arguments, indirect calls, unresolved virtual/interface omission, mutable managed or otherwise unproved parameter representations, and unsupported nullable representations fail with scoped source-positioned `HXC1001`. Primitive and closed-generic portable, metal, and explicit runtime-none roots emit byte-identical runtime-free strict C; compiler-known literals and statically typed runtime Strings passed to `Sys.println` emit the exact selective runtime closure and cleanup. Broader dispatch, collection/pattern control flow, objects, open/reference generics, unsupported String operations, exports, and standard library remain fail-closed. |
| Structural C AST and printer (`c-ast-printer`) | `ast/CAST.hx`, `ast/CASTPrinter.hx`, `test/all_sources/**`, `test/c_ast/**`, `scripts/test/c_fixture_harness.py`, canonical AST case | Compile-verified owned single/multi-file AST corpus with reproducible native records | Base specifiers and grammar-level pointer/array/function/parenthesized/abstract declarators are structural; typed nodes cover C11 array bounds, prototypes, native/exact types, atomics, alignment, anonymous aggregates, incomplete types, dialect-gated attributes, validated numeric literals, generic selections, and line anchors. The Reflaxe-owned fixture compiler emits difficult declarations, the literal 6×6 ordered expression-family matrix, every operator and statement family, designated initializers, adversarial escaping, a GNU-attribute unit, and a guarded header with separate implementation/consumer units. Unrelated-root renders match byte-for-byte. Required GCC and Clang lanes compile/link/run every runtime-free project; the normalized schema-1 report retains compiler family/version, strict flags, argument arrays, input hashes, exits, stdout, and stderr without host paths. Direct AST evidence still does not prove Haxe/HxcIR lowering, source-map policy, or whole-AST semantic invariants. |
| Declaration and header planning (`declaration-planner`) | `plan/CDeclarationPlan.hx`, `plan/CDeclarationPlanner.hx`, typed contract schema 2, `test/declaration_plan/**` | Compile-verified deterministic planning core | Complete edges drive stable in-header order or local includes; pointer-only cycles use typed forward declarations; external opaque declarations propagate their authoritative includes; build includes retain declaration owners and source reasons; public/private complete-type leakage and invalid graphs fail with HXC5002. Portable path-derived guards, reversed-input goldens, independently compiled repeated-include headers, and a combined runtime-free C consumer pass strict GCC/Clang lanes. Name finalization is now supplied structurally by `TypedCNameFinalizer`; real Haxe declaration lowering, umbrella/runtime-feature headers, production multi-file emission, exported ABI policy, and C++ header compatibility remain with later E1/E7 work. |
| Deterministic symbol registry (`symbol-registry`) | `naming/CSymbol{Request,Registry,RegistryError}.hx`, `naming/TypedCNameFinalizer.hx`, `docs/symbol-naming.md`, `test/{symbol_registry,project_emitter}/**` | Compile-verified schema-2 production naming boundary and structural sidecar | Per-compilation batch finalization covers packages/modules/types/fields/methods/locals/temporaries, overloads, generic specializations, anonymous ordinals, compiler tables/descriptors/initializers, exports, and runtime-private roles against real C namespaces. Full semantic keys remain in reports while collision-free generated C uses readable source paths and short `hxc_`-prefixed local/member words; SHA-256 suffixes appear only for genuine collisions, anonymous identity, or length limits. The prefix also prevents included-header object macros from rewriting generated locals or members, with a strict native standard-header probe. Exact `@:c.name` values are validated without rewriting. Reversed discovery, both-origin HXC5002 diagnostics, typed-contract-to-planner handoff, host-path rejection, snapshot ownership, zero-runtime effects, production lowering, and Caxecraft generated-C evidence pass. Stable export ABI remains with E7. |
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,CStaticFunctionProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `runtime/RuntimeFeaturePackager.hx`, `docs/project-emission.md`, `test/{arithmetic_semantics,c_import,project_emitter,function_lowering,generic_specialization,virtual_dispatch,evaluation_order,string_output,differential/{array-runtime,bytes-runtime}}/**`, `test/runtime/runtime-feature-graph/**` | Compile-verified adversarial deterministic structural emitter with sequenced production consumer | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and explicit runtime/ABI/stdlib states. Primitive executables consume positive no-runtime proofs; direct imports retain reached native build facts; generics and dispatch add their content-addressed reports. Literal output consumes exactly foundation/status/literal/I/O, the bounded ordinary-Haxe Array slice consumes exactly foundation/status/allocator/array, and bounded Bytes consumes exactly foundation/status/allocator/literal/bytes. The packager reads nothing for an empty plan and has no writer. Broader unproven plans remain rejected. The writer retains adversarial validation and deterministic Reflaxe-managed output; production roots compile as strict GCC/Clang C. |
| Build adapter seeds (`build-adapter-seeds`) | `emit/{CBuildPlan,CBuildAdapterEmitter,CProjectEmitter}.hx`, `docs/project-emission.md`, `test/project_emitter/**`, checksum-pinned Meson CI requirements | Compile-verified executable-only neutral plan and adapters | One typed schema-1 plan records strict C mode, owned sources/headers, include directories, source-provenance-bearing header requirements, typed definitions, logical libraries, pkg-config packages, and frameworks. Optional CMake and Meson files are content-addressed derived views; a raw argument-array consumer can ignore both. Required GCC/Clang lanes build and run the structural project and both C AST/printer corpora through all three consumers from adversarial paths and definition values. Library/export targets, configurable target names, direct/Ninja orchestration, toolchain/sysroot/cross configuration, and CLI integration remain E7/E8 work; this structural evidence does not prove general Haxe lowering or a public ABI. |
| Target-facing C API (`target-c-api`) | `std/c/**`, `TypedCContractMacro`, typed-C, span, direct-import, and Bytes-runtime fixtures | Compile-verified contract seed plus bounded fixed-array/span and direct-import operation slices | Pointer, span, ownership, result, allocator, syntax, calling-convention, visibility, integer, Float32, and volatile abstractions type-check as compiler contracts. Namespaced declaration/build metadata produces a deterministic schema-2 typed snapshot whose merged build facts retain sorted declaration provenance; negative fixtures prove source-positioned validation and empty runtime effects. The fixed-array slice operationally lowers direct nonempty primitive `CArray` literals, bounded compiler-known zero initialization, and local `Span`/`ConstSpan`; the bounded E6 slice lowers exact reached scalar/typedef/enum/constant/by-value-struct extern calls, explicit `Float`/`c.Float32` conversion, literal borrowed `CString`, scalar `c.Ref`, and one-call `CStringBufferRef` borrowing from managed Bytes. Other `c.*` operations, production public-header emission, broad native layout derivation, unsafe operation, and stable public ABI are not implemented. |
| Direct C imports (`direct-c-import`) | `interop/CImportRegistry.hx`, `std/c/{CString,CStringBufferRef,Float32,StructInit}.hx`, `docs/typed-c-authoring.md`, `test/{c_import,differential/bytes-runtime}/**`, `test/native/pointlib/**` | Compile-verified bounded runtime-free and selective-runtime C-library consumption | Reached hand-authored externs with exact `@:c.name` and authoritative includes lower fixed-arity calls plus zero-optional-argument calls to explicitly marked variadic declarations, scalars including exact binary32 `float`, typedef identities, fieldless enum/constants, by-value imported structs/fields, complete object-literal or structural-zero imported struct construction, allocation-free literal borrowed C strings, call-scoped mutable scalar references, and one-call mutable text buffers through schema-23 HxcIR and structural C AST. `c.StructInit.zero()` emits a typed C compound initializer only for complete, wholly unmanaged imported structs; ordinary Haxe records and managed layouts fail closed. `c.CStringBufferRef.to(bytes)` keeps Bytes as the owner, selects only the existing Bytes runtime closure, obtains a checked `char *`, and permits exactly one direct native consumer; imports that do not use managed storage remain runtime-free. Reachable Float32 emits target-side format assertions; explicit narrowing/widening retains rounding intent. The neutral build plan deduplicates reached include/library/pkg-config/framework facts with source owners; generated C never recreates header-owned definitions. Independent compiled probes verify pointlib size, alignment, offsets, constants, zero initialization, Float32 boundaries, mutable-text capacity and aliasing, and behavior under strict C11/C++17, O0/O2, and sanitizers. Callbacks, optional variadic arguments, general pointers and retained lifetimes, opaque ownership, unions, packing, C++, public exports, full bindgen, and stable ABI fail closed. |
| Runtime feature planning (`runtime-feature-planning`) | `runtime/RuntimeAbiContract.hx`, `runtime/RuntimeFeature{Model,Catalog,Registry,Planner,Packager}.hx`, `runtime/hxrt/features.json`, `docs/{architecture,hxrt,runtime-feature-planning}.md`, `docs/specs/runtime-features.schema.json`, `test/{runtime/{runtime-feature-graph,gc},differential/{array-runtime,bytes-runtime},string_output,primitive_differential,span_lowering}/**` | Compile-verified deterministic typed graph, whole-program no-runtime proof, versioned internal ABI, and bounded compiler-selected closures | Stable validated feature IDs, HxcIR-reconciled source reasons, dependency-first closure, and exact artifact/symbol/library/define selection are implemented without an unconditional core. Schema-2 empty plans contain a checked absence proof; alloc-only, typed-array, Bytes, object-descriptor, collector, and full-string native closures compile independently. The schema-3 feature manifest records semantic roots, internal ABI 0.15.0, same-major compatibility, forbidden application-export runtime structs, exact source hashes, and C11/C++17 provenance. Primitive, fixed-array/span, and direct-import programs remain runtime-free. Literal output selects only `runtime-base + status + string-literal + io`; the bounded ordinary-Haxe Array slice selects only `runtime-base + status + alloc + array`; bounded Bytes selects only `runtime-base + status + alloc + string-literal + bytes`; an explicit collector plan selects only `runtime-base + status + alloc + object + gc`. Runtime-none rejects any selected root set before output. Ordinary escaping-class HxcIR integration, generated public application ABI support, and neighboring runtime semantics remain later work. |
| Runtime ABI and implementation (`runtime-abi-and-implementation`) | `runtime/hxrt/include/{hxc_runtime.h,hxrt/**}`, `runtime/hxrt/src/*.c`, `runtime/hxrt/features.json` | Scaffold-only runtime with bounded compiler-selected literal-output, Array, Bytes, descriptors, and collector slices | The provisional 0.15.0 internal runtime ABI/status/allocator/string/array/Bytes/output/object/collector code is split into separately owned feature headers and sources and compiles in hosted/freestanding C plus C++17 header-consumer lanes. Hosted literal output remains allocation-free. Array and Bytes retain their bounded shared-identity storage contracts. The collector adds stable payload identity, exact explicit root chains and foreign pins, cyclic reclamation, pressure thresholds, and allocation/collection/pause reports without conservative memory scanning. Strict C11 O0/O2, custom freestanding allocation, multiple execution-context roots, C++17, and ASan/UBSan evidence pass. Full strings, other Array/Bytes shapes, ordinary escaping-class compiler integration, status naming, ABI query, dynamic values, reflection, exceptions, real threads, and platform adapters remain fail-closed. Fixed arrays/spans/direct classes stay runtime-free, and no public ABI stability is claimed. |
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `examples/hello/**`, `test/runtime/runtime-feature-graph/**`, `test/{aggregate_lowering,arithmetic_semantics,c_ast,c_import,class_layout,constructor_lowering,virtual_dispatch,declaration_plan,differential/{array-runtime,bytes-runtime},project_emitter,body_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/import/aggregate/class-reference/constructor/dispatch/generic/span/literal-output/Array/Bytes/hello projects | Local auto mode reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ warning-as-error lanes. Generated projects run under their registered optimization/sanitizer matrices; direct imports link to pointlib and compile an independent ABI probe, while private aggregate/class/constructor/dispatch/enum headers have C++17 consumers. Runtime packages, generated managed Array and Bytes ownership, runtime-free import/span/class/constructor inspection, literal output, and exact hello output retain bounded evidence. Native-only umbrella requests deliberately skip generated-Haxe compilation so that lane remains Haxe-independent and within its timeout. Independent `.cpp` consumers prove only C-header compatibility; they are not generated C++ output or public ABI evidence. Broader platforms, stable exports, and installed consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, `examples/hello/case.json`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic snapshots through the direct-import, virtual-dispatch, and hello vertical slices | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus evaluation order/control flow, arithmetic, fixed-array/span, static initialization, closed generic specialization, bounded constructor, virtual-dispatch and direct-import lowering, E2.T07 literal output, and the declared E2.T10 hello example are mapped explicitly. Every expected artifact is registry-owned and reproducible only through the diffing updater, which cannot bless in CI. Direct AST/IR, generated programs, independent runtime/ABI seeds, oracle fixtures, and product examples retain their limited claims. Additional examples require schema-valid declared assertions and may not become implicit tests. |
...
| Requirements and ledgers (`requirements-and-ledgers`) | this PRD, `docs/specs/beads-plan.json`, `docs/specs/stdlib-ledger.{json,csv}`, `docs/specs/stdlib-ledger.schema.json`, diagnostics/fixture/primitive registries | Compile-verified generated stdlib ownership contract; other requirement ledgers vary by row | The PRD and Beads plan carry 160 stable requirements. The exact Haxe 5 pin produces a schema-2 typed stdlib source/API inventory with explicit exclusions, ownership, profile/environment/runtime/test/diagnostic policy, one exact conformant `Std.int` row and 33 exact partial rows for bounded output, Array, `haxe.io.Bytes`, `String`, `StringBuf`, and `Std.string` shapes, generated CSV review output, and fail-closed pin drift. Broad standard-library behavior remains unsupported. Keep task mappings, diagnostics, runtime features, and capability evidence synchronized as scope changes. |
| Example portfolio (`example-portfolio`) | `examples/hello/**`, its central generated baseline, and E2/E9 plan entries | One bounded generated example implemented | `examples/hello/Main.hx` travels through the production custom target to readable strict C11, selects only `runtime-base + status + string-literal + io` with the one source reason propagated to every selected feature, matches Haxe Eval byte-for-byte, and runs warning-clean under required GCC and Clang at O0/O2. This is the exact E2.T10 literal-output vertical slice, not general Haxe or String support. No-runtime, interop, shared-library, C++ shim, todo, WebAssembly, fresh-install, and polished first-run journeys remain future work; independent native fixtures are not examples. |
...
Each requirement has a stable ID so Codex can cite it in Beads descriptions,
acceptance evidence, ADRs, diagnostics, tests, and capability ledgers. Priority
is product priority, not an effort estimate. The suggested epic is a default
ownership location; cross-cutting dependencies remain explicit. A future
`docs/specs/requirements.json` export must be generated from and remain
semantically synchronized with these tables; that export is not present in the
M0 checkout, so these tables and the checked-in Beads plan are the current
authorities.
...
| W8 — Product CLI | PRD/task plan and an executable-only neutral build-adapter seed; no CLI source or config seed is present | Implement `hxc` commands, config precedence, full toolchain/sysroot/cross orchestration, inspect/doctor/clean, JSON schemas | Automation-friendly CLI drives end-to-end workflows | E8.T01–E8.T11 |
...
| Primitive representations, conversions, and operations | Accepted: exact fixed/ABI identities, binary64 Float, explicit binary32 `c.Float32`, tagged scalar nullability, explicit exact/wrapping/checked/saturating/binary32 conversions, wrapping Int32 arithmetic, masked shifts, zero-safe division/modulo, and compiler-first operation selection; ordinary semantics are profile-invariant and runtime-free | ADR 0008 / E2.T01 / E2.T05 / E6.T03 / `primitive-semantics.json` | E2/E6 lowering consumes one schema-3 typed contract and may use only direct C or a request-local static-inline primitive helper; Float32 narrowing/widening remain direct and emit target format guards, safe UInt paths remain direct, `fmod` carries only its exact build fact, and native ABI facts remain probe-owned. |
...
| Build-system adapters | Implemented M1 seed: one executable-only typed manifest is normative and optional CMake/Meson files are derived views; full toolchain/export/cross orchestration remains E7/E8 | E1.T08/E7/E8 / ADR | The seed closes the emitter interface without claiming the complete product build backend. |
