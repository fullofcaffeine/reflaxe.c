...
| Profiles, runtime policy, environment, and build mode (`configuration-policies`) | `CProfile.hx`, `CRuntimePolicy.hx`, `CEnvironment.hx`, `CBuildMode.hx`, `ProfileResolver.hx`, `BuildModeResolver.hx`, config schema/template | Seeded with compile-backed build-mode validation | `portable|metal`, `auto|minimal|none`, environment presets, and `debug|release|minsizerel` are represented. The span bounds-policy matrix proves build mode cannot weaken semantics and invalid direct values fail with `HXC0003`; complete define/project precedence, capability matrices, canonical reports, and provenance-aware manifest integration remain incomplete. |
