...
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `docs/{body-lowering,evaluation-order,function-lowering}.md`, `test/{typed_ast,all_sources,body_lowering,function_lowering,evaluation_order}/**` | Compile-verified fail-closed sequenced primitive executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, collects the reachable direct-static graph plus captured primitive fields, and lowers admitted primitive signatures, arguments, conversions, calls, locals, static storage, assignments, lazy/value control flow, unsigned increments, blocks, and returns through validated HxcIR and structural C. Unsupported nodes and signatures emit exact source-positioned `HXC1001` without output; a fully admitted graph emits an owned runtime-free private header/source-set project with `int main(void)` and analyzed empty runtime/ABI/stdlib sidecars. |
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,body_lowering,function_lowering,evaluation_order}/**` | Compile-verified deterministic semantic core with production primitive consumer | Schema 2 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, all dispatch kinds, ABI integer/nullability identities, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative source spans, constant/return/call-signature checks, structurally resolvable place-type checks, reordered-input canonical dumps, stable `HXC1001` unsupported-node and `HXC9000` invariant negatives, and no-implicit-runtime assertions pass. E2.T02 consumes it for primitive bodies, E2.T03 for ordered arguments/conversions/direct calls, and E2.T04 for primitive globals plus explicit lazy/conditional block graphs and stable-value temporaries; broader semantic analyses and runtime-using lowering remain. |
| Primitive semantic contract (`primitive-semantics`) | `semantics/CPrimitive{Types,Semantics,TypeMapper,Contract}.hx`, ADR 0008, `docs/primitive-semantics.md`, `docs/specs/primitive-semantics{,.schema}.json`, `test/primitive_semantics/**` | Compile-verified typed contract; independent native algorithm and generated-body proof | Real pinned-compiler typed fields map Void/Bool/Int/UInt/Float, exact/ABI integers, and scalar/reference/native-pointer nullability identically in portable and metal. The registered deterministic contract fixes safe widening/wrapping/checked/`Std.int` behavior and selects no runtime feature. Strict C11 GCC/Clang probes at O0/O2 cover exact widths, binary64 prerequisites, signed/unsigned reconstruction, NaN/infinity/overflow/negative-zero, nullable scalars, and pointer integers. E2.T02 now uses the ordinary primitive mappings for constants, locals, and returns; the broader conversion surface remains later work. |
| Primitive body lowering (`body-lowering`) | `lowering/{CBodyLowering,CBodyEmitter,HaxeSourceSpan}.hx`, `docs/{body-lowering,evaluation-order}.md`, `test/{body_lowering,evaluation_order}/**`, canonical body/evaluation-order cases | Compile-verified real TypedExpr-to-HxcIR/C sequenced body slice | Real pinned-Haxe primitive functions lower constants, parameters, initialized locals/reads, local/global assignment, referenced constant-initialized primitive static fields, direct calls, conversions, lazy Boolean/value conditionals, unsigned prefix/postfix increments, nested blocks, and void/value returns through validated HxcIR to structural strict C11. Every load and consumed call is materialized; only proven-total pure values over stable inputs remain inline. Per-compilation naming covers globals, locals, temporaries, and labels deterministically; exact Haxe ranges survive into HxcIR, optional structural `#line`, and `HXC1001`. Repeated/reversed/profile renders match; local available and separately required GCC/Clang CI lanes execute runtime-free C at O0/O2 against Eval. |
...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,CStaticFunctionProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `docs/project-emission.md`, `test/{project_emitter,function_lowering,evaluation_order}/**` | Compile-verified adversarial deterministic structural emitter with sequenced production consumer | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and explicit runtime/ABI/stdlib states. Structural fixtures retain honest placeholders. The admitted primitive executable uses analyzed empty states and a positive no-runtime proof naming primitive storage and explicit sequencing; generic unproven lowered-program status remains rejected. The writer prevalidates normalized ownership paths, unowned collisions, malformed JSON, non-canonical line endings, and descendant symlinks, delegates every write/stale cleanup to Reflaxe, and preserves unchanged artifact mtimes. Raw artifacts remain byte-identical across roots/order/locale/CRLF/server probes; production primitive roots are also byte-identical and strict GCC/Clang executable. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/{c_ast,declaration_plan,project_emitter,body_lowering,function_lowering,evaluation_order}/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed, AST, planned-header, structural project, generated sequenced functions, and primitive production project | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each compiles/runs the runtime-free AST goldens, independently compiles planned/emitted headers, links/runs structural projects and native seeds, and exercises typed-Haxe-generated primitive bodies, static functions, and evaluation-order differential projects at O0/O2. Sanitizers, broader generated Haxe, platform matrices, ownership/failure paths, generated C++-compatible export headers, and installed external consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy, deterministic snapshots, and active evaluation-order differential lane | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus the E2.T04 evaluation-order suite are mapped in place; every checked-in expected artifact is registry-owned and reproducible only through the explicit diffing updater, which cannot bless in CI. Direct AST/IR, generated primitive programs, independent runtime/ABI seeds, and oracle fixtures retain their precise limited claims; the evaluation-order case alone currently compares generated C with Eval. Future examples require schema-valid declared assertions and may not become implicit tests. |
...
| Primitive representations and conversions | Accepted: exact fixed/ABI identities, binary64 Float, tagged scalar nullability, pointer reference nullability, and explicit exact/wrapping/checked/saturating conversions; ordinary mappings are profile-invariant and primitive operations are runtime-free | ADR 0008 / E2.T01 / `primitive-semantics.json` | E2 lowering consumes one typed contract and may use only direct C or a program-local primitive helper; native ABI facts remain probe-owned. |
