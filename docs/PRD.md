...
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,aggregate_lowering,arithmetic_semantics,body_lowering,c_import,class_layout,constructor_lowering,virtual_dispatch,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-10 semantic core with production consumers | Schema 10 distinguishes immutable block-local values from mutable places and records ordered instructions, typed block/tag-switch edges, operation/dispatch kinds, ABI integer/nullability identities, explicit binary32 rounding/binary64 widening, arrays/spans, aggregates, enums, classes, header-owned nominal imports/constants, distinct Haxe and borrowed C string literals, exact native dispatch, absent/virtual/runtime header intent, hierarchy layouts/slots/tables, failure successors, fixed-array zero initialization, initialization, lifetime, and cleanup. The validator proves repository-relative spans, reference/type/order invariants, exact import literal invariants and virtual override representations, bounded fixed-array automatic storage, and dispatch-plan consistency. E2.T05-T09 consume bounded primitive/output/span/static families; E3 consumes records, enums, classes, construction, and dispatch; the bounded E6 slice consumes exact direct imports and the Float32 carrier. Reordered dumps, stable negative diagnostics, and no-implicit-runtime assertions pass. Broader semantic analyses and runtime use remain. |
...
| Haxe enum lowering (`tagged-enum-lowering`) | `lowering/{CBodyEnum,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/enum-lowering.md`, `test/enum_lowering/**`, canonical enum cases | Compile-verified runtime-free bounded enum slice | Fieldless concrete Haxe enums use private native C enums where representation permits; payload enums use readable discriminant enums, constructor payload structs, a union, and an outer value struct. HxcIR retains source-order construction, checked payload projection, and exhaustive tag-switch edges. Closed primitive generic instances participate in the shared E3.T03 specialization key/reason/limit/report contract, while recursive local edges use finite pointer storage; recursive signatures remain fail-closed pending escape/lifetime analysis. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private tags, size/alignment/offsets, construction, and recursive layout under required GCC/G++ and Clang/Clang++ O0/O2 lanes. Public ABI, broader generic class/reference arguments, reference/aggregate payloads, identity/equality, reflection, and escaping recursive values remain unsupported. |
...
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/{function_lowering,generic_specialization,evaluation_order,string_output}/**`, canonical function/generic/evaluation-order/string-output cases | Compile-verified production sequenced primitive and closed-generic executable slice with literal output | The typed reachable direct-static graph is collected cycle-safely from the captured program; signatures, globals, and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private declarations precede definitions. Closed generic instances use the same graph with applied signatures and private specialization identities. Closed recursive cycles in the remaining unconditional single-block subset are compiler-proven C11 `_Noreturn`; multi-block bodies are excluded conservatively. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other closed-cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Default/optional/rest parameters fail with scoped source-positioned `HXC1001`. Primitive and closed-generic portable, metal, and explicit runtime-none roots emit byte-identical runtime-free strict C; compiler-known literal output emits the exact selective runtime closure. Broader dispatch, collection/pattern control flow, objects, open/reference generics, nonliteral strings, exports, and standard library remain fail-closed. |
...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,CStaticFunctionProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `runtime/RuntimeFeaturePackager.hx`, `docs/project-emission.md`, `test/{arithmetic_semantics,c_import,project_emitter,function_lowering,generic_specialization,virtual_dispatch,evaluation_order,string_output}/**`, `test/runtime/runtime-feature-graph/**` | Compile-verified adversarial deterministic structural emitter with sequenced production consumer | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and explicit runtime/ABI/stdlib states. Primitive executables consume positive no-runtime proofs and report selected program-local helpers only when selected; direct-import projects additionally retain reached header/library/package/framework facts and imported value/type counts without inventing runtime or ABI proof. Closed generic projects add a content-addressed specialization report, reachable instance-call projects add a conditional dispatch report, and literal output consumes the validated four-feature runtime plan. The packager reads nothing for an empty plan and has no writer. Broader unproven statuses remain rejected. The writer retains adversarial validation and deterministic Reflaxe-managed output; production roots are strict GCC/Clang executable. |
...
| Direct C imports (`direct-c-import`) | `interop/CImportRegistry.hx`, `std/c/{CString,Float32}.hx`, `docs/typed-c-authoring.md`, `test/c_import/**`, `test/native/pointlib/**` | Compile-verified bounded runtime-free C-library consumption | Reached hand-authored externs with exact `@:c.name` and authoritative includes lower fixed-arity C calls, scalars including exact binary32 `float`, typedef identities, fieldless enum/constants, by-value imported structs/fields, and allocation-free literal borrowed C strings through schema-10 HxcIR and structural C AST. Reachable Float32 emits target-side format assertions; explicit narrowing/widening retains rounding intent. The neutral build plan deduplicates reached include/library/pkg-config/framework facts with source owners; generated C never recreates header-owned definitions. Independent compiled probes verify pointlib size, alignment, offsets, constants, Float32 boundaries, and behavior under GCC and Clang O0/O2. Callbacks, variadics, pointers and retained lifetimes, opaque ownership, unions, packing, C++, public exports, full bindgen, and stable ABI fail closed. |
| Runtime feature planning (`runtime-feature-planning`) | `runtime/RuntimeAbiContract.hx`, `runtime/RuntimeFeature{Model,Catalog,Registry,Planner,Packager}.hx`, `runtime/Runtime{Requirement,NoRuntimeEligibility}Analyzer.hx`, `runtime/hxrt/features.json`, `docs/{architecture,hxrt,runtime-feature-planning}.md`, `docs/specs/runtime-features.schema.json`, `test/{runtime/runtime-feature-graph,differential/array-runtime,string_output,primitive_differential,span_lowering}/**` | Compile-verified deterministic typed graph, whole-program no-runtime proof, versioned internal ABI, and one compiler-selected closure | Stable validated feature IDs, definitions, reservations, HxcIR-reconciled source-root reasons, transitive edges, policy/environment/availability checks, manual constraints, dependency-first closure, and exact artifact/symbol/library/define selection are implemented without an unconditional core. Reversed inputs are byte-identical; cycles and invalid policy combinations fail deterministically; schema-2 empty plans contain a structured reachability/absence proof; tampered proofs fail before reads; alloc-only, typed-array, and full-string native closures compile/run independently. The schema-3 feature manifest records per-feature selection roots, semantic rationale, documentation/evidence, internal ABI 0.5.0, same-major compatibility, forbidden application-export runtime structs, exact source hashes/source-set digest, and strict C11/C++17 build provenance; packaging verifies each hash, and generated runtime-using C rejects a changed major while accepting a changed minor. Primitive, fixed-array/span, and direct-import programs link with no runtime files or symbols even when unreachable runtime-shaped declarations are seen. Runtime-none literal output reports every sorted source blocker and root-to-leaf dependency chain before output; ordinary output selects only `runtime-base + status + string-literal + io`. Generated public application ABI support and other generated-Haxe runtime semantics remain later work. |
| Runtime ABI and implementation (`runtime-abi-and-implementation`) | `runtime/hxrt/include/{hxc_runtime.h,hxrt/**}`, `runtime/hxrt/src/*.c`, `runtime/hxrt/features.json` | Scaffold-only runtime with bounded compiler-selected literal output | The provisional 0.5.0 internal runtime ABI/status/allocator/string/array/output code is split into separately owned feature headers and sources and compiles in hosted/freestanding C plus a C++17 public-header-consumer lane. `runtime-base`, status definitions, the allocation-free literal carrier, and hosted `io` are compiler-selectable only as the E2.T07 closure; full allocator/string operations, typed resizable arrays, status naming, and ABI query remain native-seed-only. Generated literal output writes exact length-delimited bytes, flushes, and aborts on non-OK status. The array seed has primitive/reference, overflow, relocation-alias, lifecycle-rollback, and allocator-failure evidence without object/GC dependencies; fixed arrays/spans stay runtime-free. No public ABI stability is claimed. General generated strings/arrays, I/O, objects, managed memory, dynamic values, reflection, exceptions, threads, and platform adapters remain later work. |
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `examples/hello/**`, `test/runtime/runtime-feature-graph/**`, `test/{aggregate_lowering,arithmetic_semantics,c_ast,c_import,class_layout,constructor_lowering,virtual_dispatch,declaration_plan,differential/array-runtime,project_emitter,body_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/import/aggregate/class-reference/constructor/dispatch/generic/span/literal-output/hello projects | Local auto mode reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ warning-as-error lanes. Generated projects run at O0/O2; the direct-import lane links generated C to pointlib and compiles an independent layout/constant probe, while aggregate/class/constructor/dispatch/enum suites also compile independent C++17 private-header consumers. Runtime packages, sanitizer-eligible arithmetic/array cases, import/span/class/constructor zero-`hxrt` inspection, literal output, and exact hello output retain their bounded evidence. Independent `.cpp` consumers prove only C-header compatibility; they are not generated C++ output or public ABI evidence. Broader platforms, stable exports, and installed consumers remain later gates. |
