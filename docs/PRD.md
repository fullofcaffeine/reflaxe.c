...
compile-time-only `c.StaticAssert.require`. E2.T08 now admits operations only on
`CArray`, `Span`, and `ConstSpan`: direct nonempty literal initialization, local
borrowing, checked indexing, and compiler-lowered span iteration. Those methods
have no wrapper allocation or runtime representation; every other operational
surface remains fail-closed. Empty `c.Lib`, `c.Build`, or `c.Export` marker types
are not admitted: build and export intent uses validated declaration metadata
until a concrete typed value API earns its surface. `CArray<T, N>` preserves
`N` as the type-level length identity while deriving the current literal count
from the typed AST; exact witness-generation ergonomics remain E3 work.
...
| Fixed-array and span lowering (`span-lowering`) | `std/c/{CArray,Span,ConstSpan}.hx`, `lowering/{CBodyLowering,CBodyEmitter}.hx`, `docs/span-lowering.md`, `test/span_lowering/**`, canonical span cases | Compile-verified local fixed-array/span runtime-free slice | Direct nonempty primitive literals become structural automatic C arrays; mutable and const borrows become typed pointer-plus-`size_t` views; typed span `for` protocol becomes guarded index HxcIR with no iterator object. Exact static and typed `size_t` loop-guard proofs suppress only redundant checks, while other admitted indexes retain signed/element-count fail-stop checks tagged with profile/build provenance. Repeated/reversed renders, all two-profile/three-build combinations, strict GCC/Clang O0/O2 execution, negative/upper abort paths, invalid/general/empty/lookalike negatives, and runtime-none no-`hxrt` artifact/symbol inspection pass. General arrays, empty arrays, escaping views, broader elements/operations, and stable public ABI remain fail-closed. |
