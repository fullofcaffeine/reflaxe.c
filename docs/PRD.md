...
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `docs/{arithmetic-semantics,body-lowering,evaluation-order,function-lowering}.md`, `test/{typed_ast,all_sources,arithmetic_semantics,body_lowering,function_lowering,evaluation_order}/**` | Compile-verified fail-closed sequenced UB-safe primitive executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, collects the reachable direct-static graph plus captured primitive fields, and lowers admitted signatures, arguments, conversions, calls, storage, assignments, lazy/value and statement conditionals, primitive loops/switches/jumps, arithmetic/comparisons, shifts, bit operations, updates, blocks, and returns through validated HxcIR and structural C. It selects a deterministic request-local helper closure before symbol finalization. Unsupported nodes and signatures emit exact source-positioned `HXC1001` without output; a fully admitted graph emits an owned runtime-free private header/source-set project with `int main(void)` and analyzed runtime/ABI/stdlib sidecars. |
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,arithmetic_semantics,body_lowering,function_lowering,evaluation_order}/**` | Compile-verified deterministic semantic core with production primitive consumer | Schema 2 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, operation/dispatch kinds, ABI integer/nullability identities, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative spans, switch subject/case-family checks, type/order validation, reordered-input dumps, stable `HXC1001`/`HXC9000` negatives, and no-implicit-runtime assertions pass. E2.T05 consumes explicit direct/program-local intent for UB-safe arithmetic, masked shifts, zero-safe division/modulo, bit operations, and `Std.int`; E2.T06 consumes primitive branch/jump/switch graphs with explicit non-fallthrough targets. Broader semantic analyses and runtime-using lowering remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/{arithmetic_semantics,c_ast,declaration_plan,project_emitter,body_lowering,function_lowering,evaluation_order}/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed plus generated sequenced UB-safe primitive production projects | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each runs the existing structural/native matrix plus typed-Haxe-generated primitive bodies, functions, evaluation-order projects, and arithmetic boundaries at O0/O2. The arithmetic lane also runs eligible undefined-behavior/floating-divide sanitizers and rejects retained out-of-line helpers in optimized assembly. Broader generated Haxe, platform matrices, ownership/failure paths, generated C++-compatible export headers, and installed external consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy, deterministic snapshots, and active evaluation-order/arithmetic differential lanes | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus the E2.T04/E2.T06 evaluation-order/control-flow and E2.T05 arithmetic evidence are mapped in place; every checked-in expected artifact is registry-owned and reproducible only through the explicit diffing updater, which cannot bless in CI. Direct AST/IR, generated primitive programs, independent runtime/ABI seeds, and oracle fixtures retain their precise limited claims; only mapped evaluation-order/control-flow and arithmetic cases currently compare generated C with Eval. Future examples require schema-valid declared assertions and may not become implicit tests. |
