...
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/function_lowering/**`, canonical function-lowering cases | Compile-verified first production primitive executable slice | The typed reachable direct-static graph is collected cycle-safely; signatures and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; all private prototypes precede definitions; and recursive/mutually recursive graphs compile. Default/optional/rest parameters fail with scoped source-positioned `HXC1001`. Portable, metal, and explicit runtime-none production roots emit byte-identical strict C, exact `int main(void)`, analyzed empty runtime/ABI/stdlib sidecars, and no `hxrt` artifact or symbol. Broader calls, control flow, objects, strings, exports, and standard library remain fail-closed. |
