...
| Generic specialization (`generic-specialization`) | `lowering/{CGenericSpecialization,CGenericSpecializationContract,CGenericSpecializationReport,CStaticFunctionGraph,CBodyEnum,CBodyLowering}.hx`, `emit/{CProjectEmitter,GeneratedFile}.hx`, `docs/generic-specialization.md`, `docs/specs/generic-specialization-report.schema.json`, `test/generic_specialization/**`, canonical generic-specialization cases | Compile-verified deterministic bounded runtime-free monomorphization slice | Reachable direct static generic calls infer closed non-null Bool/Int/UInt/Float or admitted enum arguments, expand aliases, keep finite same-nominal nesting distinct, and share full length-prefixed UTF-8 semantic keys; SHA-256 is only a compact instance/symbol suffix and every registry rejects a full-key collision. Request-local worklist registration closes recursive discovery. Hard 64-function, 64-type, and 524,288 estimated-C-byte limits fail with source-positioned `HXC1001` rather than boxing. Schema-1 `hxc.specializations.json` records normalized arguments, sorted source reasons, recursion, isolated function bytes/hashes, conservative dependency-closed enum bytes, and total generated payload. Repeated roots, reversed modules, locale/profile/policy changes, and rejected warm-server requests are byte-identical where required; exact Dynamic/open/65th-function/expanding-type/code-size negatives leave no output; strict GCC/Clang C11 O0/O2 executables remain runtime-free. Generic classes/references, general containers, descriptor bodies, public ABI, reflection, closures, and dynamic dispatch remain unsupported. |
