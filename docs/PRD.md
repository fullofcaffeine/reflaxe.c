...
| Profiles, runtime policy, environment, and build mode (`configuration-policies`) | `CProfile.hx`, `CRuntimePolicy.hx`, `CEnvironment.hx`, `CBuildMode.hx`, `ProfileResolver.hx`, `BuildModeResolver.hx`, `docs/configuration.md` | Scaffold-only with a compile-backed build-mode slice | `portable|metal`, `auto|minimal|none`, environment presets, and `debug|release|minsizerel` are represented. The span bounds-policy matrix proves build mode cannot weaken semantics and invalid direct values fail with `HXC0003`; no project configuration schema/template exists, and complete precedence, capability matrices, canonical reports, and provenance-aware manifest integration remain incomplete. |
...
| Reflaxe adapter (`reflaxe-adapter`) | `CReflaxeCompiler.hx`, `frontend/Typed{ProgramInput,AstNormalizer,AstInventory}.hx`, `emit/**`, `docs/typed-ast-input.md`, `docs/project-emission.md`, `test/{typed_ast,project_emitter,function_lowering,evaluation_order}/**`, `test/all_sources/**` | Compile-verified deterministic typed-input and output-ownership adapter | `filterTypes` captures and normalizes complete request input before Reflaxe filters or preprocesses callbacks; modules, declarations, primary/secondary ownership, externs, abstracts, typedefs, metadata, fields, original field expressions, and entry-point facts enter a raw-object-retaining model. Unordered module/declaration sets are sorted while field, enum, metadata, and expression-root order is retained explicitly. Reordered-input, repeated-cold, and compiler-server inventories are byte-identical after normalizing Reflaxe's exact transient build-cache hook. The manual output adapter validates typed content-addressed files, prior Reflaxe ownership, unowned collisions, and descendant symlinks before delegating every write/stale deletion. The admitted primitive static-function graph now reaches production-owned private header/source-set/sidecars; broader callbacks, generics, public output, and runtime-using projects remain. |
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{arithmetic-semantics,body-lowering,evaluation-order,function-lowering,span-lowering,static-initialization}.md`, `test/{typed_ast,all_sources,arithmetic_semantics,body_lowering,function_lowering,evaluation_order,span_lowering,static_initialization}/**` | Compile-verified fail-closed sequenced UB-safe primitive/span executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, plans dependency-ordered class/static initialization, collects the direct-static graph reachable from entry and initializer bodies plus captured primitive fields, and lowers admitted signatures, arguments, conversions, calls, storage, assignments, lazy/value and statement conditionals, primitive loops/switches/jumps, arithmetic/comparisons, shifts, bit operations, updates, local literal-backed fixed arrays/spans, blocks, and returns through validated HxcIR and structural C. It selects a deterministic request-local helper closure before symbol finalization. Unsupported nodes and signatures emit exact source-positioned `HXC1001`, while unorderable static cycles emit `HXC1002`; both leave no output. A fully admitted graph emits an owned runtime-free private header/source-set project with `int main(void)`, an initialization wrapper only when the plan is non-empty, and analyzed initialization/runtime/ABI/stdlib sidecars. |
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,arithmetic_semantics,body_lowering,function_lowering,evaluation_order,span_lowering,static_initialization}/**` | Compile-verified deterministic semantic core with production primitive/span consumer | Schema 2 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, operation/dispatch kinds, ABI integer/nullability identities, fixed arrays/spans and their initialization/bounds policies, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative spans, switch subject/case-family checks, type/order validation, reordered-input dumps, stable `HXC1001`/`HXC9000` negatives, and no-implicit-runtime assertions pass. E2.T05 consumes explicit direct/program-local intent for UB-safe arithmetic; E2.T06 consumes primitive branch/jump/switch graphs; E2.T08 consumes static, loop-guarded, and profile/build-tagged checked bounds intent; E2.T09 consumes deferred globals whose private initializer performs exactly one validated `initialize-global`. Broader semantic analyses and runtime-using lowering remain. |
...
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/{function_lowering,evaluation_order}/**`, canonical function/evaluation-order cases | Compile-verified production sequenced primitive executable slice | The typed reachable direct-static graph is collected cycle-safely from the captured program; signatures, globals, and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private declarations precede definitions. Closed recursive cycles in the remaining unconditional single-block subset are compiler-proven C11 `_Noreturn`; multi-block bodies are excluded conservatively. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other closed-cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Default/optional/rest parameters fail with scoped source-positioned `HXC1001`. Portable, metal, and explicit runtime-none production roots emit byte-identical strict C, exact `int main(void)`, analyzed empty runtime/ABI/stdlib sidecars, and no `hxrt` artifact or symbol. Broader dispatch, collection/pattern control flow, objects, strings, exports, and standard library remain fail-closed. |
...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,CStaticFunctionProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `runtime/RuntimeFeaturePackager.hx`, `docs/project-emission.md`, `test/{arithmetic_semantics,project_emitter,function_lowering,evaluation_order}/**`, `test/runtime/runtime-feature-graph/**` | Compile-verified adversarial deterministic structural emitter with sequenced production consumer | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and explicit runtime/ABI/stdlib states. Structural fixtures retain honest placeholders. The admitted primitive executable consumes the typed planner's positive no-runtime proof and reports selected program-local helpers only when the compilation actually selected them; floating modulo carries its exact math-library build fact. The runtime packager independently converts exact selected provisional feature records to owned `GeneratedFile` values and reads nothing for an empty plan. Generic unproven and runtime-using generated-Haxe statuses remain rejected. The writer retains adversarial path/ownership/content validation and deterministic Reflaxe-managed output; production primitive roots are byte-identical and strict GCC/Clang executable. |
...
| Runtime feature planning (`runtime-feature-planning`) | `runtime/RuntimeFeature{Model,Catalog,Registry,Planner,Packager}.hx`, `runtime/hxrt/features.json`, `docs/{architecture,runtime-feature-planning}.md`, `docs/specs/runtime-features.schema.json`, `test/runtime/runtime-feature-graph/**` | Compile-verified deterministic typed graph and selective provisional packaging | Stable validated feature IDs, definitions, reservations, source-root reasons, transitive edges, policy/environment/availability checks, manual constraints, dependency-first closure, and exact artifact/symbol/library/define selection are implemented without an unconditional core. Reversed inputs are byte-identical; cycles and invalid policy combinations fail deterministically; empty plans require a positive proof; alloc-only and string seed closures compile/run independently with omitted files and symbols absent. The production primitive compiler consumes the same planner only for its empty plan. Runtime-requirement inference, blocker analysis, compiler-selectable features, stable manifest ABI, and generated-Haxe runtime use remain later work. |
| Runtime ABI and implementation (`runtime-abi-and-implementation`) | `runtime/hxrt/include/{hxc_runtime.h,hxrt/**}`, `runtime/hxrt/src/*.c`, `runtime/hxrt/features.json` | Scaffold-only, selectively split native seed | The provisional runtime ABI/status/allocator/string seed is split into separately owned feature headers and sources and compiles in hosted/freestanding C plus C++17 header lanes. It remains `native-seed-only`: generated Haxe cannot select or link it, the umbrella's Int32 helpers are not feature-packaged, and no public ABI stability is claimed. Hardened allocator integration, full Haxe strings, arrays, objects, managed memory, dynamic values, reflection, exceptions, threads, and platform adapters remain later work. |
