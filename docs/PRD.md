...
| Closed anonymous-record lowering (`closed-anonymous-record-lowering`) | `lowering/{CBodyAggregate,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/aggregate-lowering.md`, `test/aggregate_lowering/**`, canonical aggregate cases | Compile-verified runtime-free direct value-record slice | Closed anonymous records of admitted primitive or nested record fields deduplicate across aliases by canonical UTF-8 structural shape and emit dependency-first private named C structs. HxcIR retains source-order object-literal evaluation before canonical named construction, explicit by-value copies, immutable projections, and local field address/dereference operations. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private size/alignment/offset agreement under required GCC/G++ and Clang/Clang++ O0/O2 lanes. `Void` fields, identity/equality, mutation requiring alias analysis, open/reflective/Dynamic shapes, public ABI, and metal packed typed-C layouts remain fail-closed. |
...
| Deterministic symbol registry (`symbol-registry`) | `naming/CSymbol{Request,Registry,RegistryError}.hx`, `naming/TypedCNameFinalizer.hx`, `docs/symbol-naming.md`, `test/{symbol_registry,project_emitter}/**` | Compile-verified schema-1 naming core and structural sidecar | Per-compilation batch finalization covers packages/modules/types/fields/methods/locals/temporaries, overloads, generic specializations, anonymous ordinals, compiler tables/descriptors/initializers, exports, and runtime-private roles against real C namespaces. Exact `@:c.name` values are validated without rewriting; generated `hxc_`/`hxc_api_` defaults escape reserved/library forms and use path-independent SHA-256 disambiguation. Reversed discovery, both-origin HXC5002 collision diagnostics, typed-contract-to-planner handoff, host-path rejection, snapshot ownership, and zero-runtime effects pass. The typed project emitter now writes finalized structural `hxc.symbols.json`; full typed-AST request collection, production writing, stable export ABI, and Haxe-generated C evidence remain with E3/E7/lowering work. |
