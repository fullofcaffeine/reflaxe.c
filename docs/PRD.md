...
| Reflaxe adapter (`reflaxe-adapter`) | `CReflaxeCompiler.hx`, `frontend/Typed{ProgramInput,AstNormalizer,AstInventory}.hx`, `emit/**`, `docs/typed-ast-input.md`, `docs/project-emission.md`, `test/{typed_ast,project_emitter,function_lowering,evaluation_order,string_output}/**`, `test/all_sources/**` | Compile-verified deterministic typed-input and output-ownership adapter | `filterTypes` captures and normalizes complete request input before Reflaxe filters or preprocesses callbacks; modules, declarations, primary/secondary ownership, externs, abstracts, typedefs, metadata, fields, original field expressions, and entry-point facts enter a raw-object-retaining model. Unordered module/declaration sets are sorted while field, enum, metadata, and expression-root order is retained explicitly. Reordered-input, repeated-cold, and compiler-server inventories are byte-identical after normalizing Reflaxe's exact transient build-cache hook. The manual output adapter validates typed content-addressed files, prior Reflaxe ownership, unowned collisions, and descendant symlinks before delegating every write/stale deletion. The admitted graph reaches production-owned private header/source-set/sidecars and exact selectively packaged literal-output runtime files; broader callbacks, generics, public output, and other runtime-using projects remain. |
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{aggregate-lowering,arithmetic-semantics,body-lowering,enum-lowering,evaluation-order,function-lowering,hxc-ir,span-lowering,static-initialization,string-runtime}.md`, `test/{typed_ast,all_sources,aggregate_lowering,arithmetic_semantics,body_lowering,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified fail-closed sequenced primitive/span/closed-record/bounded-enum/literal-output executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, plans dependency-ordered class/static initialization, collects the direct-static graph reachable from entry and initializer bodies plus captured primitive fields, and lowers admitted signatures, arguments, conversions, calls, storage, assignments, lazy/value and statement conditionals, primitive loops/switches/jumps, arithmetic/comparisons, shifts, bit operations, updates, local literal-backed fixed arrays/spans, closed anonymous value records, bounded direct-value Haxe enums, compiler-known UTF-8 strings, literal-only hosted `Sys.println`/default `trace`, blocks, and returns through validated HxcIR and structural C. Closed record shapes deduplicate by canonical structure. Fieldless enums may use native C enums; payload enums use private tagged unions, checked projection, exhaustive tag switches, concrete primitive specialization, and finite function-local recursive storage. Unsupported nodes and signatures emit exact source-positioned `HXC1001`, while unorderable static cycles emit `HXC1002`; both leave no output. A fully admitted graph emits an owned private header/source-set project with `int main(void)`, an initialization wrapper only when required, and analyzed sidecars. Primitive, closed-record, and bounded-enum programs remain runtime-free; literal output packages only `runtime-base + status + string-literal + io`. |
...
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/{function_lowering,evaluation_order,string_output}/**`, canonical function/evaluation-order/string-output cases | Compile-verified production sequenced primitive executable slice with literal output | The typed reachable direct-static graph is collected cycle-safely from the captured program; signatures, globals, and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private declarations precede definitions. Closed recursive cycles in the remaining unconditional single-block subset are compiler-proven C11 `_Noreturn`; multi-block bodies are excluded conservatively. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other closed-cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Default/optional/rest parameters fail with scoped source-positioned `HXC1001`. Primitive portable, metal, and explicit runtime-none roots emit byte-identical runtime-free strict C; compiler-known literal output emits the exact selective runtime closure. Broader dispatch, collection/pattern control flow, objects, nonliteral strings, exports, and standard library remain fail-closed. |
