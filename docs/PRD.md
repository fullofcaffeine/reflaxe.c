...
| Reflaxe adapter (`reflaxe-adapter`) | `CReflaxeCompiler.hx`, `frontend/Typed{ProgramInput,AstNormalizer,AstInventory}.hx`, `emit/**`, `docs/typed-ast-input.md`, `docs/project-emission.md`, `test/{typed_ast,project_emitter}/**`, `test/all_sources/**` | Compile-verified deterministic typed-input and output-ownership adapter | `filterTypes` captures complete request input before Reflaxe filters; modules, declarations, primary/secondary ownership, externs, abstracts, typedefs, metadata, fields, expression roots, and entry-point facts enter a raw-object-retaining model. Unordered module/declaration sets are sorted while field, enum, metadata, and expression-root order is retained explicitly. Reordered-input, repeated-cold, and compiler-server inventories are byte-identical after normalizing Reflaxe's exact transient build-cache hook. The manual output adapter validates typed content-addressed files, prior Reflaxe ownership, unowned collisions, and descendant symlinks before delegating every write/stale deletion. The body slice now normalizes real compiler source spans; callback/generic behavior and production Haxe-generated multi-file output remain. |
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `docs/body-lowering.md`, `test/{typed_ast,all_sources,body_lowering}/**` | Compile-verified fail-closed primitive body lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main body even when Haxe retains only its entry-point call, and lowers the admitted parameter-free primitive subset through validated HxcIR and structural C. Unsupported nodes emit exact source-positioned `HXC1001`; a supported body reaches the later E2.T03 `HXC1000` function/call/entry-point boundary. Neither path emits a production artifact. |
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/hxc_ir/**` | Compile-verified deterministic semantic core with first frontend/C consumer | Schema 2 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, all dispatch kinds, ABI integer/nullability identities, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative source spans, constant/return checks, structurally resolvable place-type checks, reordered-input canonical dumps, stable `HXC1001` unsupported-node and `HXC9000` invariant negatives, and no-implicit-runtime assertions pass. E2.T02 consumes it for primitive bodies; later semantic analyses, broader frontend/C lowering, runtime-plan finalization, and production generated-program evidence remain. |
...
| Primitive body lowering (`body-lowering`) | `lowering/{CBodyLowering,CBodyEmitter,HaxeSourceSpan}.hx`, `docs/body-lowering.md`, `test/body_lowering/**`, canonical body-lowering cases | Compile-verified first real TypedExpr-to-HxcIR/C slice | Real pinned-Haxe parameter-free functions lower constants, initialized locals/reads, cleanup-free nested blocks, and void/value returns through validated HxcIR to structural strict C11. Per-compilation batch naming is deterministic and shadow-safe; exact Haxe ranges survive into HxcIR, optional structural `#line`, and `HXC1001`. Repeated/reversed/profile renders match; local available and separately required GCC/Clang CI lanes execute mapped/unmapped runtime-free C at O0/O2. The test-only function/harness envelope is not production entry-point or project emission. |
...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `docs/project-emission.md`, `test/project_emitter/**` | Compile-verified adversarial deterministic structural project emitter | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and honest runtime/ABI/stdlib placeholders. The writer prevalidates normalized ownership paths, unowned collisions, malformed JSON, non-canonical line endings, and descendant symlinks, delegates all writes/stale cleanup to Reflaxe, and preserves unchanged artifact mtimes. Raw artifacts remain byte-identical across isolated roots, reversed discovery, fixed locale, CRLF inputs, and warm compiler-server reuse; failures name the first UTF-8-ordered artifact and byte offset. A full-to-renamed-symbol transition removes every prior owned payload while unlisted user data survives, and scans reject host paths, timestamps, random-ID shapes/keys, unstable JSON keys, and carriage returns. Emitted structural C compiles/runs under strict GCC/Clang. The real body slice is not yet connected to this emitter; production reaches exact `HXC1001` or later `HXC1000` with no output, so runtime/ABI and production-project proofs remain absent. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/{c_ast,declaration_plan,project_emitter,body_lowering}/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed, AST, planned-header, structural project, and generated-body goldens | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each compiles/runs the runtime-free declarator and expression/statement AST goldens, independently compiles every planned and project-emitter header, links/runs the emitted structural project and declaration consumer, runs the hosted C11 runtime, compiles the freestanding path, links/runs a C++17 runtime-header consumer, and exercises independent C-library and opaque-handle C++-shim fixtures. The body suite independently compiles/runs real typed-Haxe-generated primitive bodies with available compiler families. Sanitizers, broader generated Haxe, platform matrices, ownership/failure paths, generated C++-compatible export headers, and installed external consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic M0 snapshots | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites, including typed-AST normalization and real primitive body-lowering cases, are mapped in place; every checked-in expected artifact is registry-owned and reproducible through an explicit check/update command that prints semantic diffs and cannot bless in CI. Direct AST/IR, the generated-body slice, independent native runtime/ABI, and oracle seeds retain their precise limited claims. Future examples require schema-valid declared assertions and may not become implicit tests. |
