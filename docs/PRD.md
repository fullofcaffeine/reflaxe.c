...
| Runtime feature planning (`runtime-feature-planning`) | `runtime/RuntimeFeature{Model,Catalog,Registry,Planner,Packager}.hx`, `runtime/hxrt/features.json`, `docs/{architecture,runtime-feature-planning}.md`, `docs/specs/runtime-features.schema.json`, `test/{runtime/runtime-feature-graph,string_output}/**` | Compile-verified deterministic typed graph with one compiler-selected closure | Stable validated feature IDs, definitions, reservations, source-root reasons, transitive edges, policy/environment/availability checks, manual constraints, dependency-first closure, and exact artifact/symbol/library/define selection are implemented without an unconditional core. Reversed inputs are byte-identical; cycles and invalid policy combinations fail deterministically; empty plans require a positive proof; alloc-only and full-string native closures compile/run independently. The production compiler keeps primitive programs empty and selects only `runtime-base + status + string-literal + io` for literal hosted output, with exact per-call reasons and policy diagnostics. Broader inference, blocker analysis, stable manifest ABI, and other generated-Haxe runtime use remain later work. |
