...
| Structural C AST and printer (`c-ast-printer`) | `ast/CAST.hx`, `ast/CASTPrinter.hx`, `test/all_sources/**`, `test/c_ast/**` | Compile-verified declarator model | Base specifiers and grammar-level pointer/array/function/parenthesized/abstract declarators are structural; typed nodes cover C11 array bounds, prototypes, native/exact types, atomics, alignment, anonymous aggregates, incomplete types, and dialect-gated attributes. A deterministic Haxe-built golden covers precedence-sensitive typedef/parameter/field/function-pointer forms and compiles/runs under strict GCC and Clang. Expression/statement precedence completeness, source maps/provenance, whole-AST invariants, extension-native probes, and the broad E1.T10 corpus remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/c_ast/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed and AST golden | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each compiles/runs the structural AST golden and hosted C11 runtime, compiles the freestanding path, links/runs a C++17 public-header consumer, and exercises independent C-library and opaque-handle C++-shim fixtures. Sanitizers, Haxe-language generated output, broader platform matrices, ownership/failure paths, and installed external consumers remain later gates. |
