...
| Target-facing C API (`target-c-api`) | `std/c/**`, `TypedCContractMacro`, typed-C, span, and direct-import fixtures | Compile-verified contract seed plus bounded fixed-array/span and direct-import operation slices | Pointer, span, ownership, result, allocator, syntax, calling-convention, visibility, integer, and volatile abstractions type-check as compiler contracts. Namespaced declaration/build metadata produces a deterministic schema-2 typed snapshot whose merged build facts retain sorted declaration provenance; negative fixtures prove source-positioned validation and empty runtime effects. The fixed-array slice operationally lowers direct nonempty primitive `CArray` literals, bounded compiler-known zero initialization, and local `Span`/`ConstSpan`; the bounded E6 slice lowers exact reached scalar/typedef/enum/constant/by-value-struct extern calls and literal borrowed `CString`. Other `c.*` operations, production public-header emission, broad native layout derivation, unsafe operation, and stable public ABI are not implemented. |
| Direct C imports (`direct-c-import`) | `interop/CImportRegistry.hx`, `std/c/CString.hx`, `docs/typed-c-authoring.md`, `test/c_import/**`, `test/native/pointlib/**` | Compile-verified bounded runtime-free C-library consumption | Reached hand-authored externs with exact `@:c.name` and authoritative includes lower fixed-arity C calls, scalars, typedef identities, fieldless enum/constants, by-value imported structs/fields, and allocation-free literal borrowed C strings through schema-9 HxcIR and structural C AST. The neutral build plan deduplicates reached include/library/pkg-config/framework facts with source owners; generated C never recreates header-owned definitions. Independent compiled probes verify pointlib size, alignment, offsets, constants, and behavior under GCC and Clang O0/O2. Callbacks, variadics, pointers and retained lifetimes, opaque ownership, unions, packing, C++, public exports, full bindgen, and stable ABI fail closed. |
...
| Primitive representations, conversions, and operations | Accepted: exact fixed/ABI identities, binary64 Float, tagged scalar nullability, explicit exact/wrapping/checked/saturating conversions, wrapping Int32 arithmetic, masked shifts, zero-safe division/modulo, and compiler-first operation selection; ordinary semantics are profile-invariant and runtime-free | ADR 0008 / E2.T01 / E2.T05 / `primitive-semantics.json` | E2 lowering consumes one schema-2 typed contract and may use only direct C or a request-local static-inline primitive helper; safe UInt paths remain direct, `fmod` carries only its exact build fact, and native ABI facts remain probe-owned. |
