...
| Concrete class instance layouts (`class-layout`) | `lowering/{CBodyClass,CBodyLowering,CBodyEmitter}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/class-layout.md`, `test/class_layout/**`, canonical class-layout cases | Compile-verified runtime-free private class-reference/layout slice | Ordinary concrete non-generic classes emit nominal private C structs with source-order own fields, an embedded first base subobject, and a strict-C anchor only for an otherwise empty root. HxcIR retains nullable instance pointers, explicit profile/build null proofs, inherited field places, identity operations, absent-unless-required header intent, and null-preserving derived-to-base representation conversions. Structural `_Static_assert` claims plus independent C/C++17 consumers verify private size/alignment/offset/base paths and field/reference behavior under required GCC/G++ and Clang/Clang++ O0/O2 lanes. Production ABI reports retain no public exports. E3.T06 may select one root-only program-local table pointer for a reachable polymorphic hierarchy; unrelated and final-direct hierarchies remain header-free. General allocation, interface/dynamic dispatch, generic classes, object runtime metadata, reflection/type tests, ownership/GC, and public ABI remain fail-closed. |
| Bounded constructor lowering (`constructor-lowering`) | `lowering/{CBodyConstructor,CBodyLowering,CBodyEmitter,CStaticFunctionGraph}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/constructor-lowering.md`, `test/constructor_lowering/**`, canonical constructor-lowering cases | Compile-verified bounded construction with selective managed parameter ownership | Direct unconditional local `new` expressions for concrete non-generic classes receive default-initialized automatic storage and ordinary private C constructor functions. Proven `new` and `super` calls append omitted trailing typed defaults before HxcIR; explicit null remains distinct from omission, non-managed tagged record optionals pass by value, and the IR validator sees only exact fixed-arity calls. Literal-backed String parameters preserve nominal abstract identity in constructor symbols and pass as immutable views; a first final field initialization copies the program-long literal view without allocation or ownership operations. Fieldless enum parameters preserve their exact instance identity and pass as nominal C tags. Tagged payload enums whose finalized graph has no managed lifetime also pass by value with their active payload intact; a first final field initialization copies either unmanaged enum representation without lifetime work. Constructor symbol admission forces enum lifecycle finalization first. Prepared `Array<T>` constructor parameters retain their exact specialization and remain call borrows; a first final `this.field = parameter` initialization retains one field owner for reference-counted Arrays, collector-backed Arrays use exact roots and tracing, and fresh literal arguments receive a caller cleanup owner. HxcIR preserves pinned-Haxe argument/field/`super`/body order, non-null `self`, trivial-chain elision, explicit infallible versus `status(exception)` conventions, nested propagation, and reverse partial/initialized cleanup. Embedded final children may use a validated non-owning automatic alias, including across lazy Boolean blocks, without gaining an independent lifetime. Eval, repeated/reversed/two-profile/runtime-none where eligible, strict GCC/Clang O0/O2 C11, sanitizers, intentional fail-stop, and C++17 header evidence pass. E3.T06 may bind a selected program-local table before constructor code. Runtime-created or owned Strings, managed or recursive payload-enum constructor values, returned, escaping, passed-to-owning, reassigned, self-stored, or conditionally constructed references; managed Array field replacement or storage through another object; dependency cycles; native layouts; generic/interface construction; unproven managed/callable parameter families; heap allocation; catch/finally; interface/dynamic dispatch; object runtime metadata; and public ABI remain fail-closed. |
