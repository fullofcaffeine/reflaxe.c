...
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{aggregate-lowering,arithmetic-semantics,body-lowering,class-layout,enum-lowering,evaluation-order,function-lowering,generic-specialization,hxc-ir,span-lowering,static-initialization,string-runtime}.md`, `test/{typed_ast,all_sources,aggregate_lowering,arithmetic_semantics,body_lowering,class_layout,enum_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified fail-closed sequenced primitive/span/closed-record/class-reference/bounded-enum/closed-generic/literal-output executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, plans dependency-ordered class/static initialization, collects the direct-static graph reachable from entry and initializer bodies plus captured primitive fields, and lowers admitted signatures, arguments, conversions, calls, storage, assignments, lazy/value and statement conditionals, primitive loops/switches/jumps, arithmetic/comparisons, shifts, bit operations, updates, local literal-backed fixed arrays/spans, closed anonymous value records, concrete class references and fields, bounded direct-value Haxe enums, closed generic static functions, compiler-known UTF-8 strings, literal-only hosted `Sys.println`/default `trace`, blocks, and returns through validated HxcIR and structural C. Closed record shapes deduplicate by canonical structure. Concrete non-generic classes use private base-prefix structs, nullable pointers, checked dereference, identity comparison, and null-preserving structural upcasts without constructors, allocation, or dispatch. Fieldless enums may use native C enums; payload enums use private tagged unions, checked projection, exhaustive tag switches, closed primitive specialization, and finite function-local recursive storage. Generic functions and enums share collision-checked semantic keys, bounded request-local discovery, and `hxc.specializations.json` code-size/reason evidence. Unsupported nodes and signatures emit exact source-positioned `HXC1001`, while unorderable static cycles emit `HXC1002`; both leave no output. A fully admitted graph emits an owned private header/source-set project with `int main(void)`, an initialization wrapper only when required, and analyzed sidecars. Primitive, closed-record, concrete-class-reference, bounded-enum, and closed-generic programs remain runtime-free; literal output packages only `runtime-base + status + string-literal + io`. |
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,aggregate_lowering,arithmetic_semantics,body_lowering,class_layout,enum_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-5 semantic core with production consumers | Schema 5 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block and tag-switch edges, operation/dispatch kinds, ABI integer/nullability identities, fixed arrays/spans and their initialization/bounds policies, aggregate, enum, and class declarations/instances/places, explicit class header intent and null checks, safe representation upcasts, checked tag projection, finite direct instance layout, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative spans, type/order validation, reordered-input dumps, stable `HXC1001`/`HXC9000` negatives, and no-implicit-runtime assertions pass. E2.T05-T09 consume the bounded primitive, output, span, and deferred-global families; E3.T01 consumes direct closed-record shapes; E3.T02 consumes bounded native/tagged enum shapes, exhaustive matching, and function-local recursive storage; E3.T04 consumes private concrete class layouts, checked references, identity, fields, and safe upcasts. Broader semantic analyses and runtime use remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `examples/hello/**`, `test/runtime/runtime-feature-graph/**`, `test/{aggregate_lowering,arithmetic_semantics,c_ast,class_layout,declaration_plan,differential/array-runtime,project_emitter,body_lowering,function_lowering,generic_specialization,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/aggregate/class-reference/generic/span/literal-output/hello projects | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each runs exact alloc-only/array/full-string/literal-I/O packages, the array differential/lifecycle/sanitizer contract, structural/native matrix, typed-Haxe-generated primitive/aggregate/class-reference/generic/evaluation/arithmetic/span/literal-output projects, and the checked-in generated hello baseline at O0/O2. Aggregate lanes compare private generated-record layout and copies between C and C++17; class lanes compare private base-prefix layout, fields, null-preserving upcasts, and identity; the generic lane proves runtime-free closed instances and recursion. The alloc-only lane rejects retained string symbols; arithmetic and array run eligible sanitizers; span and class layout inspect zero-`hxrt` links; literal output checks exact NUL-bearing stdout and fail-stop I/O; hello checks exact `Hello from hxc\n`. Independent `.cpp` consumers prove C-header compatibility only; they are not generated C++ output or public ABI evidence. Broader generated Haxe, platforms, stable exports, and installed consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, `examples/hello/case.json`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic snapshots through the hello vertical slice | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus evaluation order/control flow, arithmetic, fixed-array/span, static initialization, closed generic specialization, E2.T07 literal output, and the declared E2.T10 hello example are mapped explicitly; every checked-in test or example expected artifact is registry-owned and reproducible only through the diffing updater, which cannot bless in CI. Direct AST/IR, generated programs, independent runtime/ABI seeds, oracle fixtures, and product examples retain their precise limited claims. Hello compares generated C with the pinned Haxe Eval oracle. Additional examples still require schema-valid declared assertions and may not become implicit tests. |
