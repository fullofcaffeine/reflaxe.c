...
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `plan/CStaticInitialization*.hx`, `docs/{arithmetic-semantics,body-lowering,evaluation-order,function-lowering,hxc-ir,span-lowering,static-initialization,string-runtime}.md`, `test/{typed_ast,all_sources,arithmetic_semantics,body_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified fail-closed sequenced primitive/span/literal-output executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, plans dependency-ordered class/static initialization, collects the direct-static graph reachable from entry and initializer bodies plus captured primitive fields, and lowers admitted signatures, arguments, conversions, calls, storage, assignments, lazy/value and statement conditionals, primitive loops/switches/jumps, arithmetic/comparisons, shifts, bit operations, updates, local literal-backed fixed arrays/spans, compiler-known UTF-8 strings, literal-only hosted `Sys.println`/default `trace`, blocks, and returns through validated HxcIR and structural C. It selects request-local helpers and exact runtime reasons before finalization. Unsupported nodes and signatures emit exact source-positioned `HXC1001`, while unorderable static cycles emit `HXC1002`; both leave no output. A fully admitted graph emits an owned private header/source-set project with `int main(void)`, an initialization wrapper only when required, and analyzed sidecars. Primitive-only programs remain runtime-free; literal output packages only `runtime-base + status + string-literal + io`. |
| HxcIR semantic core (`hxc-ir`) | `ir/{HxcIR,HxcIRValidator,HxcIRDumper,HxcUtf8,HxcJsonString}.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,arithmetic_semantics,body_lowering,function_lowering,evaluation_order,span_lowering,static_initialization,string_output}/**` | Compile-verified deterministic schema-3 semantic core with production consumers | Schema 3 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, operation/dispatch kinds, ABI integer/nullability identities, fixed arrays/spans and their initialization/bounds policies, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. It adds `IRTString` plus exact validated Unicode-scalar UTF-8 constants with explicit byte length, deterministic NUL escaping, and literal hosted-output calls carrying native-status abort edges. Repository-relative spans, type/order validation, reordered-input dumps, stable `HXC1001`/`HXC9000` negatives, and no-implicit-runtime assertions pass. E2.T05 consumes UB-safe primitive intent; E2.T06 consumes primitive branch/jump/switch graphs; E2.T07 consumes the bounded String/output nodes; E2.T08 consumes bounds intent; E2.T09 consumes deferred globals. Broader semantic analyses and runtime use remain. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `examples/hello/**`, `test/runtime/runtime-feature-graph/**`, `test/{arithmetic_semantics,c_ast,declaration_plan,differential/array-runtime,project_emitter,body_lowering,function_lowering,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/span/literal-output/hello projects | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each runs exact alloc-only/array/full-string/literal-I/O packages, the array differential/lifecycle/sanitizer contract, structural/native matrix, typed-Haxe-generated primitive/evaluation/arithmetic/span/literal-output projects, and the checked-in generated hello baseline at O0/O2. The alloc-only lane rejects retained string symbols; arithmetic and array run eligible sanitizers; span inspects zero-`hxrt` links; literal output checks exact NUL-bearing stdout and fail-stop I/O; hello checks exact `Hello from hxc\n`. The `.cpp` runtime consumer intentionally proves the public C header and output signature remain C++17-compatible; it is not generated C++ output. Broader generated Haxe, platforms, stable exports, and installed consumers remain later gates. |
