# PRD: `reflaxe.c` / `hxc` — Haxe 4.3.7 to idiomatic C
...
- `.haxerc` pinning Haxe 4.3.7 with scoped library resolution.
...
The C checkout now pins Haxe `4.3.7`, Lix `17.0.2`, and Reflaxe commit
`73a983112e039daad46b37912ab238df6bf0cf53`. Reflaxe compiler sources are
vendored and content-hashed for source and staged-package reproducibility; this
...
- Haxe 4.3.7 pin for the first compatibility contract.
...
This section defines the intended representation, not a license to skip conformance tests. Each rule MUST be backed by differential fixtures against the chosen Haxe 4.3.7 reference targets.
...
| Structural validation and archive integrity (`structural-validation`) | `scripts/ci/check_prd.py`, `structure_check.py`, `verify_manifest.py`, runtime smoke, CI workflow, package manifest | Verified native seed | Local gates verify PRD/registry/plan synchronization, structure, reference evidence, native fixtures, and package checksums. Haxe/Reflaxe type-check, live Beads apply, generated-output, sanitizer, platform, and release gates remain explicitly unverified. |
