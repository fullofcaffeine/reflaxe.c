...
| Bounded constructor lowering (`constructor-lowering`) | `lowering/{CBodyConstructor,CBodyLowering,CBodyEmitter,CStaticFunctionGraph}.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/constructor-lowering.md`, `test/constructor_lowering/**`, canonical constructor-lowering cases | Compile-verified runtime-free nonescaping stack-construction slice | Direct unconditional local `new` expressions for concrete non-generic classes receive default-initialized automatic storage and ordinary private C constructor functions. Proven `new` and `super` calls append omitted trailing typed defaults before HxcIR; explicit null remains distinct from omission, non-managed tagged record optionals pass by value, and the IR validator sees only exact fixed-arity calls. HxcIR preserves pinned-Haxe argument/field/`super`/body order, non-null `self`, trivial-chain elision, explicit infallible versus `status(exception)` conventions, nested propagation, and reverse partial/initialized cleanup. Embedded final children may use a validated non-owning automatic alias, including across lazy Boolean blocks, without gaining an independent lifetime. Eval, repeated/reversed/two-profile/runtime-none, strict GCC/Clang O0/O2 C11, sanitizers, intentional fail-stop, and C++17 header evidence pass. E3.T06 may bind a selected program-local table before constructor code. Returned, escaping, passed-to-owning, reassigned, self-stored, or conditionally constructed references; dependency cycles; native layouts; generic/interface construction; unproven managed/callable parameter families; heap allocation; catch/finally; interface/dynamic dispatch; object runtime metadata; and public ABI remain fail-closed. |
