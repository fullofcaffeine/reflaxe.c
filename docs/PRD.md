...
| Per-build compiler state (`per-build-context`) | `CompilationContext.hx`, `test/typed_ast/**`, `test/symbol_registry/**` | Compile-verified typed-input/naming state seed | A fresh context owns one normalized `TypedProgramInput` and one empty per-compilation `CSymbolRegistry`; adversarial rich-to-small compiler-server requests match cold output without module/context leakage, while reversed registry discovery is byte-identical. Resolved policy/runtime reasons, type/layout registries, pass state, diagnostics, outputs, full typed-AST symbol request collection, and later pipeline caches still need implementation. |
...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,CStaticFunctionProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `docs/project-emission.md`, `test/{arithmetic_semantics,project_emitter,function_lowering,evaluation_order}/**` | Compile-verified adversarial deterministic structural emitter with sequenced production consumer | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and explicit runtime/ABI/stdlib states. Structural fixtures retain honest placeholders. The admitted primitive executable records a positive no-runtime proof and reports selected program-local helpers only when the compilation actually selected them; floating modulo carries its exact math-library build fact. Generic unproven lowered-program status remains rejected. The writer retains adversarial path/ownership/content validation and deterministic Reflaxe-managed output; production primitive roots are byte-identical and strict GCC/Clang executable. |
...
| Runtime ABI and implementation (`runtime-abi-and-implementation`) | `runtime/hxrt/include/hxc_runtime.h`, `runtime/hxrt/src/hxc_runtime.c` | Scaffold-only | A provisional allocator/string/status/Int32 ABI compiles and runs natively but is not selected by generated programs. The runtime feature graph/manifest, hardened allocation contracts, arrays, objects, managed memory, dynamic values, reflection, exceptions, threads, and platform adapters remain later work. |
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/{arithmetic_semantics,c_ast,declaration_plan,project_emitter,body_lowering,function_lowering,evaluation_order,span_lowering}/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed plus generated sequenced UB-safe primitive/span production projects | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each runs the existing structural/native matrix plus typed-Haxe-generated primitive bodies, functions, evaluation-order projects, arithmetic boundaries, and fixed-array/span projects at O0/O2. The arithmetic lane also runs eligible undefined-behavior/floating-divide sanitizers and rejects retained out-of-line helpers; the span lane executes fail-stop bounds paths and inspects links for zero `hxrt` symbols. Broader generated Haxe, platform matrices, ownership/failure paths, generated C++-compatible export headers, and installed external consumers remain later gates. |
