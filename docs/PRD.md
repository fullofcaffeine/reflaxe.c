...
| Per-build compiler state (`per-build-context`) | `CompilationContext.hx`, `test/typed_ast/**` | Compile-verified typed-input state seed | A fresh context owns one normalized `TypedProgramInput`, and adversarial rich-to-small compiler-server requests match cold output without module/context leakage. Resolved policy/runtime reasons, symbol/type/layout registries, pass state, diagnostics, outputs, and later pipeline caches still need implementation. |
