...
| Reflaxe adapter (`reflaxe-adapter`) | `CReflaxeCompiler.hx`, `frontend/Typed{ProgramInput,AstNormalizer,AstInventory}.hx`, `emit/**`, `docs/typed-ast-input.md`, `docs/project-emission.md`, `test/{typed_ast,project_emitter,function_lowering}/**`, `test/all_sources/**` | Compile-verified deterministic typed-input and output-ownership adapter | `filterTypes` captures complete request input before Reflaxe filters; modules, declarations, primary/secondary ownership, externs, abstracts, typedefs, metadata, fields, expression roots, and entry-point facts enter a raw-object-retaining model. Unordered module/declaration sets are sorted while field, enum, metadata, and expression-root order is retained explicitly. Reordered-input, repeated-cold, and compiler-server inventories are byte-identical after normalizing Reflaxe's exact transient build-cache hook. The manual output adapter validates typed content-addressed files, prior Reflaxe ownership, unowned collisions, and descendant symlinks before delegating every write/stale deletion. The admitted primitive static-function graph now reaches production-owned private header/source-set/sidecars; broader callbacks, generics, public output, and runtime-using projects remain. |
| Whole-program compiler (`whole-program-compiler`) | `CCompiler.hx`, `lowering/**`, `docs/{body,function}-lowering.md`, `test/{typed_ast,all_sources,body_lowering,function_lowering}/**` | Compile-verified fail-closed primitive executable lowering | `CCompiler` consumes the context-owned normalized program, locates the real typed static main even when Haxe retains only its entry-point call, collects the reachable direct-static graph, and lowers admitted primitive signatures, arguments, conversions, calls, locals, blocks, and returns through validated HxcIR and structural C. Unsupported nodes and signatures emit exact source-positioned `HXC1001` without output; a fully admitted graph emits an owned runtime-free private header/source-set project with `int main(void)` and analyzed empty runtime/ABI/stdlib sidecars. |
| HxcIR semantic core (`hxc-ir`) | `ir/HxcIR.hx`, `HxcIRValidator.hx`, `HxcIRDumper.hx`, `docs/hxc-ir.md`, `test/{hxc_ir,body_lowering,function_lowering}/**` | Compile-verified deterministic semantic core with production primitive consumer | Schema 2 structurally distinguishes immutable block-local values from mutable places; records ordered instructions, typed block edges, all dispatch kinds, ABI integer/nullability identities, exact/wrapping/checked/saturating conversions with failure successors, allocation/implementation intent, initialization/lifetime transitions, and reverse inner-to-outer cleanup execution. Repository-relative source spans, constant/return/call-signature checks, structurally resolvable place-type checks, reordered-input canonical dumps, stable `HXC1001` unsupported-node and `HXC9000` invariant negatives, and no-implicit-runtime assertions pass. E2.T02 consumes it for primitive bodies and E2.T03 for ordered arguments, conversions, and direct calls; broader semantic analyses and runtime-using lowering remain. |
...
| Primitive body lowering (`body-lowering`) | `lowering/{CBodyLowering,CBodyEmitter,HaxeSourceSpan}.hx`, `docs/body-lowering.md`, `test/body_lowering/**`, canonical body-lowering cases | Compile-verified real TypedExpr-to-HxcIR/C body slice | Real pinned-Haxe primitive functions lower constants, parameters, initialized locals/reads, cleanup-free nested blocks, direct calls, conversions, and void/value returns through validated HxcIR to structural strict C11. Per-compilation batch naming is deterministic and shadow-safe; exact Haxe ranges survive into HxcIR, optional structural `#line`, and `HXC1001`. Repeated/reversed/profile renders match; local available and separately required GCC/Clang CI lanes execute mapped/unmapped runtime-free C at O0/O2. |
| Static function lowering (`function-lowering`) | `lowering/CStaticFunctionGraph.hx`, `emit/CStaticFunctionProjectEmitter.hx`, `docs/function-lowering.md`, `test/function_lowering/**`, canonical function-lowering cases | Compile-verified first production primitive executable slice | The typed reachable direct-static graph is collected cycle-safely; signatures and symbol requests finalize before bodies; left-to-right arguments and implicit primitive conversions are explicit HxcIR instructions; and all private prototypes precede definitions. Closed recursive cycles in the current unconditional single-block subset are compiler-proven C11 `_Noreturn` and omit unreachable returns. Direct self-tail calls materialize registry-named typed arguments before a structural loop; other cycle members receive deterministic private translation units behind the shared header, so GCC and Clang remain warning-clean with `-Winfinite-recursion` active at `-O2`. Default/optional/rest parameters fail with scoped source-positioned `HXC1001`. Portable, metal, and explicit runtime-none production roots emit byte-identical strict C, exact `int main(void)`, analyzed empty runtime/ABI/stdlib sidecars, and no `hxrt` artifact or symbol. Broader calls, control flow, objects, strings, exports, and standard library remain fail-closed. |
...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/{c_ast,declaration_plan,project_emitter,body_lowering,function_lowering}/**`, `test/native/pointlib/**`, `test/native/cpp_shim/**` | Verified native seed, AST, planned-header, structural project, generated bodies/functions, and primitive production project | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each compiles/runs the runtime-free AST goldens, independently compiles planned/emitted headers, links/runs structural projects and native seeds, and exercises typed-Haxe-generated primitive bodies plus static functions at O0/O2. The function suite also strictly compiles/runs the owned production project. Sanitizers, broader generated Haxe, platform matrices, ownership/failure paths, generated C++-compatible export headers, and installed external consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic M0 snapshots | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites, including typed-AST, primitive-body, and static-function production cases, are mapped in place; every checked-in expected artifact is registry-owned and reproducible only through the explicit diffing updater, which cannot bless in CI. Direct AST/IR, generated primitive programs, independent runtime/ABI seeds, and oracle fixtures retain their precise limited claims. Future examples require schema-valid declared assertions and may not become implicit tests. |
