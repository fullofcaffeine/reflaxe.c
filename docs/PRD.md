...
| Runtime feature planning (`runtime-feature-planning`) | `runtime/RuntimeFeature{Model,Catalog,Registry,Planner,Packager}.hx`, `runtime/Runtime{Requirement,NoRuntimeEligibility}Analyzer.hx`, `runtime/hxrt/features.json`, `docs/{architecture,runtime-feature-planning}.md`, `docs/specs/runtime-features.schema.json`, `test/{runtime/runtime-feature-graph,string_output,primitive_differential,span_lowering}/**` | Compile-verified deterministic typed graph, whole-program no-runtime proof, and one compiler-selected closure | Stable validated feature IDs, definitions, reservations, HxcIR-reconciled source-root reasons, transitive edges, policy/environment/availability checks, manual constraints, dependency-first closure, and exact artifact/symbol/library/define selection are implemented without an unconditional core. Reversed inputs are byte-identical; cycles and invalid policy combinations fail deterministically; schema-2 empty plans contain a structured reachability/absence proof; tampered proofs fail before reads; alloc-only and full-string native closures compile/run independently. Primitive and fixed-array/span programs link with no runtime files or symbols even when unreachable runtime-shaped types are seen. Runtime-none literal output reports every sorted source blocker and root-to-leaf dependency chain before output; ordinary output selects only `runtime-base + status + string-literal + io`. Stable manifest ABI and other generated-Haxe runtime semantics remain later work. |
