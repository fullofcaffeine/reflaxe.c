...
| Generated-file boundary (`generated-file-boundary`) | `emit/{GeneratedFile,CProjectEmitter,ProjectEmissionError,ReflaxeOutputWriter}.hx`, `docs/project-emission.md`, `test/project_emitter/**` | Compile-verified deterministic structural project emitter | Closed typed plans package canonical header/source layouts, merged build facts, SHA-256-addressed artifacts, finalized symbols, and honest runtime/ABI/stdlib placeholders. The writer prevalidates normalized ownership paths, unowned collisions, malformed JSON, and descendant symlinks, delegates all writes/stale cleanup to Reflaxe, and preserves unchanged artifact mtimes. Fresh unrelated roots and reversed discovery are byte-identical; a listed stale file is removed while unlisted user data survives; emitted structural C compiles/runs under strict GCC/Clang. Production Haxe lowering still reaches `HXC1000` with no output, so runtime/ABI proofs and generated-Haxe evidence remain absent. |
