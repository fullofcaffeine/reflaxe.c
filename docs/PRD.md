...
| Native smoke fixtures (`native-smoke-fixtures`) | `runtime/hxrt/test/**`, `scripts/ci/runtime_smoke.py`, `test/runtime/runtime-feature-graph/**`, `test/{arithmetic_semantics,c_ast,declaration_plan,project_emitter,body_lowering,function_lowering,evaluation_order,span_lowering,string_output}/**`, `test/native/{pointlib,cpp_shim}/**` | Verified selective native seed plus generated primitive/span/literal-output projects | Local auto mode explicitly reports optional compiler-family skips and requires at least one complete pair. CI separately requires real GCC/G++ and Clang/Clang++ lanes with warnings as errors; each runs exact alloc-only/full-string/literal-I/O packages, the structural/native matrix, and typed-Haxe-generated primitive, evaluation-order, arithmetic, span, and literal-output projects at O0/O2. The alloc-only lane rejects retained string symbols; arithmetic runs eligible sanitizers and optimized-shape checks; span inspects zero-`hxrt` links; literal output checks exact NUL-bearing stdout and fail-stop I/O. The `.cpp` runtime consumer intentionally proves the public C header and output signature remain C++17-compatible; it is not generated C++ output. Broader generated Haxe, platforms, stable exports, and installed consumers remain later gates. |
| Fixture taxonomy and snapshot policy (`fixture-policy`) | `docs/testing.md`, `docs/specs/fixture-{case,taxonomy}*.json`, `scripts/test/snapshots.py`, `scripts/ci/check_fixture_policy.py`, canonical `test/{positive,negative,ast,snapshot,runtime,differential,abi,performance}/` lanes | Verified policy and deterministic snapshots through literal-output evidence | Eight evidence lanes have explicit directory, runner, and expected-output contracts. Existing M0 suites plus evaluation order/control flow, arithmetic, fixed-array/span, static initialization, and E2.T07 literal-output evidence are mapped in place; every checked-in expected artifact is registry-owned and reproducible only through the explicit diffing updater, which cannot bless in CI. Direct AST/IR, generated programs, independent runtime/ABI seeds, and oracle fixtures retain their precise limited claims; mapped evaluation-order/control-flow, arithmetic, static initialization, and literal output compare generated C with a pinned Haxe oracle. Future examples require schema-valid declared assertions and may not become implicit tests. |
