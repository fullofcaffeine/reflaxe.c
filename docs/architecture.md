...
constructor validates and content-addresses the finished text after formatting;
a broad “C printing” clock alone cannot tell those two costs apart.
...
Schema 7 makes cross-request digest reuse observable without turning it into
semantic compiler state. A warm Haxe compiler server retains only the previous
successful output generation's immutable path, artifact role, complete text,
UTF-8 byte length, and SHA-256 digest. A later artifact is a hit only when the
path, role, and full text are equal; unequal text always uses the normal UTF-8
and SHA-256 path. The cache records hit, miss, retained-entry, and retained-byte
counts. It never retains `TypedExpr`, Haxe types or positions, HxcIR, CAST,
`CompilationContext`, or an output manager.

Publication follows Reflaxe's output lifecycle. The request starts with a fresh
lookup, generated files receive their addresses, Reflaxe writes the complete
artifact set, applies its stale-file policy, and writes its ownership metadata.
Only the subsequent `onOutputComplete` callback replaces the one retained
cache generation. A rejected compile, output error, or explicitly aborted
request therefore leaves the preceding complete generation intact. The
`reflaxe_c_test_disable_generated_digest_cache` verification define bypasses
reuse without changing generated bytes, keeping the cold path authoritative.

Schema 8 makes the first semantic backend reuse boundary equally explicit.
Control-flow planning answers questions such as which branches meet again and
which loops can become structured C. Those answers depend only on one complete,
validated HxcIR function--not on C names, project layout, runtime packaging, or
the current Haxe compiler context. A warm request can therefore reuse the
previous validated plan only when all three key parts match:

1. the HxcIR schema number;
2. the logical function identity; and
3. the complete canonical HxcIR text for that function, including its stable
   source spans.

The comparison uses the full text rather than only a digest, so a collision
cannot turn changed semantics into a hit. The current request still rebuilds
and independently validates its HxcIR program. On a hit, CAST emission consumes
the prior immutable control-flow plan with the current request's names,
representations, and project facts. On a miss, the normal planner runs and
validates a replacement. The cache retains no `TypedExpr`, Haxe `Type`,
`Position`, compiler context, generated C, output owner, or machine-local path.

`HxcIRDumper.dumpSnapshot` produces report text and exact function fragments in
one exhaustive traversal. Key-only builds retain the function fragments but
not a second complete-program string. `onOutputComplete` replaces the cache
with the new complete function generation; a failed request cannot publish
partial plans. Retained-key size is reported in Haxe string code units, not
claimed as a byte count, because the cache does not perform another UTF-8
encoding merely to measure itself. The
`reflaxe_c_test_disable_control_flow_plan_cache` define preserves the
authoritative uncached path, while
`reflaxe_c_control_flow_plan_cache_report` exposes exact lifecycle counters to
focused server tests.

An all-hit request still emits the broad `body control-flow planning` detail
span for its lookups. Its analysis, indexing, dominator, loop, construction,
and validation child spans are absent because those computations did not run;
...
internal runtime ABI 0.15.0, and exact source/build provenance. Every artifact has a reviewed SHA-256, packaging
