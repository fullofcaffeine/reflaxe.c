...
authority for selected visual IDs and their source records.
The package manifest repeats each selected asset path and digest as a transport
...
reproducible stored ZIP. The archive contains its package manifest, semantic
files, and all selected visual assets. It sorts names, fixes
...
| complete package transport | `caxecraft.package.json` | this manifest receipts every semantic file and selected asset that must travel together |
