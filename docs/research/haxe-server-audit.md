...
The frontend now gives named record lowering an exact declaration-position
plan. It accepts cold positions only inside the typedef's source span and can
recover a warm position only from a bounded plain-data cache keyed by the exact
source bytes and complete define set. The recovered Haxe `Position` is rebuilt
for the current request and current filename; macro types, positions, and host
paths never enter the persistent record. Missing or mismatched evidence fails
with `HXC9000` before HxcIR construction rather than coarsening or guessing.

The focused server suite compares HxcIR, generated bytes, and unsupported-type
diagnostics with fresh-process oracles across unchanged requests, source and
public-type edits, request restoration, restart, server-off, and a second
worktree. This removes the provenance blocker for the next backend-reuse
experiment. It does not claim that full source-map emission is shipped; E8.T08
still owns `#line`, sidecar maps, and native diagnostic remapping.

The integrated Caxecraft rerun then reported zero cold-to-warm HxcIR function
changes, zero module changes, and no non-function or schema/dispatch skeleton
change. The fixed `Vitals.hx` implementation edit changed only
`Vitals.applyAttack`, the `caxecraft.domain.Vitals` HxcIR module,
`Vitals.c`, and `hxc.manifest.json`. Its cold, warm, and edited requests took
42.71s, 22.20s, and 22.29s while one-minute host load began above the machine's
12 logical CPUs. Those times are contention diagnostics, not a new baseline;
the exact changed sets are the stable result.
