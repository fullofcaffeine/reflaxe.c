...
The checked-in runtime is deliberately incomplete. At present, generated Haxe
can select only the hosted literal-output closure. The allocator, resizable
array, full string, status-name, and ABI-query slices have independent native
evidence but are `native-seed-only`; their presence is not a claim that general
Haxe allocation, `Array`, or `String` lowering works.
...
The runtime ABI is internal and versioned, currently 0.5.0. Generated
...
No checked-in feature provides threads, atomics, thread-local error state, or a
collector. Current records contain no hidden global mutable runtime state; the
hosted C streams used by `io` follow the C library's behavior. Thread and GC
feature IDs remain reserved and fail closed. A future collector must follow
[ADR 0005](adr/0005-precise-nonmoving-collector.md), including exact compiler-
emitted roots and no arbitrary foreign/interior-pointer rooting.
...
Native-seed-only checked allocator and move-only allocation-owner contract. It
supports hosted and caller-supplied allocator evidence, aligned storage,
failure-atomic resize, and checked size arithmetic. Generated allocation remains
unsupported. See [allocator ownership](allocator-abi.md).

<!-- hxrt-feature:array -->
### `array`

Native-seed-only resizable unboxed array storage built on `alloc`. It owns
deterministic growth, alias-safe insert/resize behavior, reverse destruction,
and optional all-or-none element lifecycle callbacks. Fixed arrays and spans
stay direct and runtime-free. See [array runtime](array-runtime.md).
...
The only current generated-Haxe semantic root. Reachable
...
| `include/hxrt/allocator.h`, `src/allocator.c` | Native-seed allocator callbacks, owner lifecycle, checked arithmetic, aligned hosted implementation. |
| `include/hxrt/array.h`, `src/array.c` | Native-seed resizable typed storage and element lifecycle. |
...
not runtime implementation. They may request `native-seed-fixture` planning to
test slices that generated Haxe is forbidden to select.
